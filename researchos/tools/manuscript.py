...
from .citation_alignment import (
    citation_alignment_issues,
    citation_support_text_by_key,
...
# Note cards are retrieval anchors, not a substitute for evidence verification.
# Each writing surface starts with a bounded, section-specific set so the model
# can scan diverse evidence without broad-reading every paper note.  A section
# supplement then directs targeted ``grep_search`` / ``read_file`` recovery.
NOTE_CARD_BUDGETS: dict[str, dict[str, int]] = {
    "abstract": {"initial": 0, "minimum_claim_usable": 0, "maximum": 0},
    # These are context ceilings and recovery triggers, not citation quotas.
    # A narrow claim may use fewer cards or query a different slice.
    "introduction": {"initial": 6, "minimum_claim_usable": 3, "maximum": 10},
    "related_work": {"initial": 8, "minimum_claim_usable": 4, "maximum": 12},
    "methodology": {"initial": 5, "minimum_claim_usable": 2, "maximum": 8},
    "experiments": {"initial": 4, "minimum_claim_usable": 2, "maximum": 6},
    "analysis": {"initial": 6, "minimum_claim_usable": 3, "maximum": 9},
    "conclusion": {"initial": 3, "minimum_claim_usable": 2, "maximum": 5},
}
...
SECTION_WRITING_CONTRACTS = {
    ...
    "introduction": {
        ...
        "required_content": [
            ...
            "The smallest distinct set of concrete gaps needed to motivate the work, tied to prior work or practice.",
    ...
    "related_work": {
        ...
        "required_content": [
            "A minimal coherent set of prior-work streams organized by rationale, not by authors.",
...
MANUSCRIPT_SECTION_MIN_CITATIONS = {
    # These are absence guards, not quality targets.  They catch a
    # claim-bearing section that ignored the literature entirely while the
    # citation/claim alignment audit remains responsible for semantic fit.
    # The effective floor is capped by the traceable bibliography size.
    "introduction": 1,
    "related_work": 1,
    # Method and Analysis may be supported entirely by the project's design
    # and realized evidence. Requiring a citation there encouraged an
    # unrelated token citation rather than better information use.
    "methodology": 0,
    "experiments": 0,
    "analysis": 0,
    "conclusion": 0,
...
class BuildManuscriptResourceIndexParams(BaseModel):
    ...
    include_previews: bool = Field(
        default=False,
        description=(
            "Whether to copy short source previews into the index. Disabled by default because consumers can "
            "read exact artifacts and repeated previews dilute later model context."
        ),
    )
...
class InitializeManuscriptStateParams(BaseModel):
    ...
    storyline_path: str = Field(
        default="drafts/writing_storyline.md",
        description="LLM-authored cross-section argument and evidence storyline.",
    )
    resource_index_path: str = Field(
        default="drafts/manuscript_resource_index.json",
        description="Resource index generated by build_manuscript_resource_index.",
...
class BuildSectionEvidenceSupplementParams(BaseModel):
    section_id: str = Field(description="Manuscript section id, e.g. related_work, methodology, analysis.")
    resource_index_path: str = Field(
        default="drafts/manuscript_resource_index.json",
        description="Resource index generated by build_manuscript_resource_index.",
    )
    output_path: str = Field(
        default="",
        description="Relative output path. Defaults to drafts/section_outlines/{section_id}_evidence_supplement.md.",
    )
    min_cards: int = Field(
        default=0,
        description="Optional override for claim-usable card target. Zero uses the section policy.",
    )
    include_weak_cards: bool = Field(
        default=True,
        description="Include weak cards as background/upgrade leads when claim-usable cards are insufficient.",
    )
    reason: str = Field(
        default="section_evidence_insufficient",
        description="Why the supplement is being generated.",
    )
    stage: str = Field(
        default="manuscript",
        description="Calling surface for the retrieval record, e.g. manuscript or skill_note_review.",
    )
...
class AuditWritingCraftParams(BaseModel):
    ...
    target_venue: str = Field(
        default="",
        description="Target venue used only to resolve an internal writing profile; not an official-rule source.",
    )
    writing_style_path: str = Field(
        default="drafts/writing_style.json",
        description="Optional T8 style selection used to resolve the venue-aware drafting profile.",
    )
    storyline_path: str = Field(
        default="drafts/writing_storyline.md",
        description="Optional Writer argument/evidence storyline for deterministic coverage checks.",
    )
...
class InitializeManuscriptStateTool(Tool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        try:
            ...
            storyline = _read_optional_text(self.policy, params.storyline_path)
            index = _read_optional_json(self.policy, params.resource_index_path)
            ...
            state = build_paper_state(
                ...
                storyline_path=params.storyline_path,
            ...
            state["input_fingerprints"] = build_paper_state_input_fingerprints(
                ws,
                {
                    ...
                    "storyline": params.storyline_path,
            ...
            # The full note-card fields are useful once, while constructing
            # section-specific retrieval plans. Persisting all of them inside
            # paper_state duplicated the resource index and caused every
            # section worker to reread the entire corpus before issuing its
            # own targeted evidence query.
            ...
            shared_facts = state.get("shared_facts") if isinstance(state.get("shared_facts"), dict) else {}
            if isinstance(shared_facts, dict):
                indexed_cards = shared_facts.get("paper_note_cards", [])
                shared_facts["paper_note_card_count"] = len(indexed_cards) if isinstance(indexed_cards, list) else 0
                shared_facts["paper_note_cards"] = []
                shared_facts["paper_note_index_path"] = "drafts/manuscript_resource_index.json"
                shared_facts["paper_note_catalog_semantics"] = (
                    "externalized_query_exact_note_sections_for_claims"
                )
...
class BuildSectionEvidenceSupplementTool(Tool):
    name = "build_section_evidence_supplement"
    description = (
        "Generate a section-scoped evidence supplement from indexed paper-note cards. "
        "Use this when a manuscript section has insufficient citations/evidence; it surfaces "
        "relevant note-card sections and weak/background upgrade leads without writing prose."
    )
    parameters_schema = BuildSectionEvidenceSupplementParams
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        params = BuildSectionEvidenceSupplementParams(**kwargs)
        try:
            section_id = normalize_section_id(params.section_id)
            if section_id not in CORE_SECTIONS:
                return ToolResult(ok=False, content=f"unknown section_id: {params.section_id}", error="unknown_section")
            if params.output_path:
                output_rel = params.output_path
                output_path = self.policy.resolve_write(output_rel)
            else:
                output_rel = f"drafts/section_outlines/{section_id}_evidence_supplement.md"
                output_path = self.policy.resolve_write(output_rel)
            ...
            if index_candidate.exists():
                ...
                index = build_resource_index(self.policy.workspace_dir, include_previews=False)
            supplement = build_section_evidence_supplement(
                self.policy.workspace_dir,
                section_id=section_id,
                resource_index=index,
                min_cards=params.min_cards,
                include_weak_cards=params.include_weak_cards,
                reason=params.reason,
                stage=params.stage,
            )
            output_path.parent.mkdir(parents=True, exist_ok=True)
            output_path.write_text(supplement["markdown"], encoding="utf-8")
        except ToolAccessDenied as exc:
            return ToolResult(ok=False, content=str(exc), error="access_denied")
        except Exception as exc:
            return ToolResult(ok=False, content=f"section evidence supplement failed: {exc}", error="supplement_failed")

        return ToolResult(
            ok=True,
            content=(
                f"Wrote section evidence supplement to {output_rel}: "
                f"{supplement['claim_usable_count']} claim-usable cards, "
                f"{supplement['weak_or_background_count']} weak/background cards."
            ),
            data={
                "path": output_rel,
                "section_id": section_id,
                "claim_usable_count": supplement["claim_usable_count"],
                "weak_or_background_count": supplement["weak_or_background_count"],
                "note_card_budget": supplement["note_card_budget"],
                "retrieval_actions": supplement["retrieval_actions"],
            },
        )


def _section_query_receipt_utilization(
    workspace: Path,
    *,
    task_id: str,
    run_id: str = "",
    used_citation_keys: set[str],
) -> dict[str, list[str]]:
    """Summarize evidence bundles actually returned to one section task.

    Receipts from other stages or earlier section tasks are deliberately
    excluded.  This prevents a large shared archive from being mistaken for
    evidence that entered the current Writer decision.
    """

    receipt_root = workspace / "literature" / "evidence_queries"
    receipt_paths: list[str] = []
    retrieved_sources: set[str] = set()
    claim_usable_sources: set[str] = set()
    retrieved_keys: set[str] = set()
    normalized_task_id = str(task_id or "").strip()
    if normalized_task_id and receipt_root.is_dir():
        for path in sorted(receipt_root.glob("**/*.json"), key=lambda item: item.as_posix()):
            try:
                payload = json.loads(path.read_text(encoding="utf-8"))
            except (OSError, ValueError, json.JSONDecodeError):
                continue
            if not isinstance(payload, dict) or str(payload.get("caller_task_id") or "").strip() != normalized_task_id:
                continue
            normalized_run_id = str(run_id or "").strip()
            if normalized_run_id:
                receipt_runs = {
                    str(item).strip()
                    for item in payload.get("retrieval_events", [])
                    if str(item).strip()
                }
                caller_run_id = str(payload.get("caller_run_id") or "").strip()
                if normalized_run_id not in receipt_runs and caller_run_id != normalized_run_id:
                    continue
            try:
                receipt_paths.append(path.relative_to(workspace).as_posix())
            except ValueError:
                receipt_paths.append(path.as_posix())
            ...
            for hit in hits:
                ...
                if source_path:
                    ...
                    if bool(hit.get("claim_usable_for_purpose")):
                        claim_usable_sources.add(source_path)
                citation_key = str(hit.get("citation_key") or "").strip()
                if citation_key:
                    retrieved_keys.add(citation_key)
    return {
        "receipts": sorted(set(receipt_paths)),
        "retrieved_source_paths": sorted(retrieved_sources),
        "retrieved_claim_usable_source_paths": sorted(claim_usable_sources),
        "retrieved_citation_keys": sorted(retrieved_keys),
        "retrieved_and_explicitly_used_citation_keys": sorted(retrieved_keys & used_citation_keys),
    }
...
class UpdateManuscriptSectionStateTool(Tool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        try:
            ...
            # Preserve the distinction between evidence made available to the
            # section and evidence that survived into the actual prose.  This
            # is deliberately citation-key based: it does not pretend that
            # context inclusion proves reasoning use, while remaining stable
            # and inspectable across resume/retry.
            context_texts: list[str] = []
            for context_rel in (
                f"drafts/section_outlines/{section_id}.md",
                f"drafts/section_outlines/{section_id}_evidence_supplement.md",
            ):
                context_path = self.policy.workspace_dir / context_rel
                if context_path.is_file():
                    context_texts.append(context_path.read_text(encoding="utf-8", errors="replace"))
            included_citation_keys = set().union(*(_extract_latex_cites(value) for value in context_texts)) if context_texts else set()
            used_citation_keys = _extract_latex_cites(text)
            query_utilization = _section_query_receipt_utilization(
                self.policy.workspace_dir,
                task_id=str(self.policy.task_id or ""),
                run_id=str(self.policy.run_id or ""),
                used_citation_keys=used_citation_keys,
            )
            retrieved_citation_keys = set(query_utilization["retrieved_citation_keys"])
            included_citation_keys.update(retrieved_citation_keys)
            bibliography_path = self.policy.workspace_dir / "literature" / "related_work.bib"
            bibliography_keys = (
                set(extract_bib_keys_from_text(bibliography_path.read_text(encoding="utf-8", errors="replace")))
                if bibliography_path.is_file()
                else set()
            )
            utilization = {
                "semantics": "manuscript_section_evidence_inclusion_and_explicit_use",
                "available_bibliography_key_count": len(bibliography_keys),
                "included_context_citation_keys": sorted(included_citation_keys),
                "explicitly_used_citation_keys": sorted(used_citation_keys),
                "included_but_not_explicitly_used_citation_keys": sorted(included_citation_keys - used_citation_keys),
                "evidence_query_receipts": query_utilization["receipts"],
                "retrieved_source_paths": query_utilization["retrieved_source_paths"],
                "retrieved_claim_usable_source_paths": query_utilization["retrieved_claim_usable_source_paths"],
                "retrieved_citation_keys": query_utilization["retrieved_citation_keys"],
                "retrieved_and_explicitly_used_citation_keys": query_utilization[
                    "retrieved_and_explicitly_used_citation_keys"
                ],
                "boundary": (
                    "Explicit use means a citation key appears in the written section. "
                    "A query hit means the tool returned it to this section task, not that the model relied on it. "
                    "Uncited synthesis, logical inference, and model knowledge are not falsely labeled as unused evidence."
                ),
            }
            entry["evidence_utilization"] = utilization
            information_utilization = state.setdefault(
                "information_utilization",
                {
                    "semantics": "manuscript_information_availability_inclusion_and_explicit_use_audit",
                    "sections": {},
                },
            )
            if isinstance(information_utilization, dict):
                section_utilization = information_utilization.setdefault("sections", {})
                if isinstance(section_utilization, dict):
                    section_utilization[section_id] = utilization
            state["current_section"] = section_id
            state["last_written_section"] = section_id
            _refresh_mutable_literature_state(self.policy.workspace_dir, state)
...
class AuditWritingCraftTool(Tool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        try:
            ...
            alignment_matrix = _read_optional_json(self.policy, params.alignment_matrix_path)
            cdr_ledger = _read_optional_json(self.policy, params.cdr_claim_ledger_path)
            writing_style = _read_optional_json(self.policy, params.writing_style_path)
            try:
                storyline = _read_optional_text(self.policy, params.storyline_path)
            except ToolAccessDenied:
                storyline = ""
            profile_style = dict(writing_style) if isinstance(writing_style, dict) else {}
            if params.venue_style != "auto":
                profile_style["venue_style"] = params.venue_style
            writing_profile = resolve_venue_writing_profile(params.target_venue, profile_style)
            ...
            citation_provenance = build_citation_provenance_audit(
                self.policy.workspace_dir,
                paper=paper,
                paper_path=params.paper_path,
                related_work_bib_path=params.related_work_bib_path,
            )
            provenance_hard_failures = citation_provenance["summary"]["hard_fail_count"]
            audit_doc["json"]["checks"].append(
                {
                    "name": "citation_provenance",
                    "level": "PASS" if provenance_hard_failures == 0 else "FAIL",
                    "passed": provenance_hard_failures == 0,
                    "detail": (
                        "Every in-text citation is mapped to a real bibliography entry and a readable note source; "
                        "no disallowed or weak-only strong-claim citation was found."
                        if provenance_hard_failures == 0
                        else "Citation provenance hard failures: "
                        + "; ".join(citation_provenance["hard_failures"][:8])
                    ),
                }
            )
            ...
            provenance_path = output_path.parent / "citation_provenance_audit.json"
            provenance_markdown_path = provenance_path.with_suffix(".md")
            provenance_path.write_text(
                json.dumps(citation_provenance, ensure_ascii=False, indent=2) + "\n",
                encoding="utf-8",
            )
            provenance_markdown_path.write_text(
                render_citation_provenance_audit(citation_provenance),
                encoding="utf-8",
            )
        ...
        return ToolResult(
            ...
            data={
                ...
                "citation_provenance_json": str(provenance_path.relative_to(self.policy.workspace_dir)),
                "citation_provenance_markdown": str(provenance_markdown_path.relative_to(self.policy.workspace_dir)),
...
def build_resource_index(workspace: Path, *, include_previews: bool = False) -> dict[str, Any]:
...
def build_section_plan(index: dict[str, Any], *, target_venue: str = "", paper_type: str = "auto") -> dict[str, Any]:
    ...
    def section(
        ...
        # Workspace bootstrap creates directory guides so users can discover
        # where to put optional material.  A directory containing only that
        # guide is not usable evidence or implementation context: presenting
        # it to Writer caused needless directory scans during a section run.
        # Keep genuine contentful directories inspectable, but treat empty
        # roots as unavailable optional material.
        def is_contentful_optional(path: str) -> bool:
            entry = artifact_entries.get(path, {})
            if str(entry.get("kind") or "") != "directory":
                return path in artifact_paths
            return int(entry.get("substantive_file_count") or 0) > 0

        available_optional = [path for path in optional if is_contentful_optional(path)]
        optional_directories = [
            path
            for path in available_optional
            if str(artifact_entries.get(path, {}).get("kind") or "") == "directory"
        ]
        sections.append(
            {
                "id": name,
                ...
                "available_optional_inputs": [
                    path for path in available_optional if path not in optional_directories
                ],
                "optional_directory_inputs": optional_directories,
                "missing_optional_input_count": sum(1 for path in optional if path not in available_optional),
    ...
    *,
    ...
    for section_id in SECTION_WRITING_SEQUENCE:
        ...
        sections[section_id] = {
            ...
            "available_optional_inputs": list(plan_entry.get("available_optional_inputs", [])),
            "optional_directory_inputs": list(plan_entry.get("optional_directory_inputs", [])),
            "missing_optional_input_count": int(plan_entry.get("missing_optional_input_count") or 0),
    ...
    return {
        ...
        "input_fingerprints": {},
        # Literature may grow through one bounded section-level supplement.
        # Its current identity is refreshed when the section state is
        # committed; immutable outline/claim/experiment inputs remain strict.
        "mutable_input_labels": ["related_work_bib"],
        ...
        "shared_facts": {
            ...
            "storyline_heading_count": len(re.findall(r"(?m)^##\s+\S", storyline_text or "")),
            "bib_keys": bib_keys,
        ...
        "rules": [
            ...
            "Citation keys must exist in the current related_work.bib; shared_facts.bib_keys is refreshed after each section write.",
...
def _refresh_mutable_literature_state(workspace: Path, state: dict[str, Any]) -> None:
    """Refresh the bounded T8 literature view without rebuilding manuscript state."""

    shared = state.setdefault("shared_facts", {})
    if not isinstance(shared, dict):
        shared = {}
        state["shared_facts"] = shared
    bib_keys = _extract_bib_keys(workspace / "literature" / "related_work.bib")
    citation_refs = _extract_citation_reference_summary(workspace)
    shared["bib_keys"] = bib_keys
    shared["citation_ref_by_note_id"] = citation_refs["citation_ref_by_note_id"]
    shared["note_id_by_bib_key"] = citation_refs["note_id_by_bib_key"]
    shared["unmapped_note_ids"] = citation_refs["unmapped_note_ids"]
    shared["paper_note_card_count"] = sum(
        1 for _card in iter_literature_note_cards(workspace, include_shallow=True)
    )
    shared["paper_note_cards"] = []
    shared["paper_note_index_path"] = "drafts/manuscript_resource_index.json"
    shared["paper_note_catalog_semantics"] = (
        "externalized_query_exact_note_sections_for_claims"
    )

    fingerprints = state.setdefault("input_fingerprints", {})
    if isinstance(fingerprints, dict):
        path = workspace / "literature" / "related_work.bib"
        item: dict[str, Any] = {
            "path": "literature/related_work.bib",
            "exists": path.is_file(),
        }
        if path.is_file():
            item["sha256"] = _sha256_path(path)
        fingerprints["related_work_bib"] = item
...
def build_section_outlines(
    ...
    *,
    ...
    for section_id in SECTION_WRITING_SEQUENCE:
        ...
        local_storyline = _extract_storyline_for_section(storyline_text, section_id)
        lines = [
            ...
            "",
            "## Storyline Responsibilities",
            local_storyline or "- No section-specific storyline passage was found; preserve the global problem-to-solution argument in drafts/writing_storyline.md.",
            "",
            "## Required Inputs",
        ]
        ...
        available_optional = [
            str(path)
            for path in plan_entry.get("available_optional_inputs", [])
            if str(path).strip()
        ]
        if available_optional:
            lines.extend(["", "## Available Optional Files"])
            for path in available_optional:
                lines.append(f"- `{path}`")
            lines.append("- Read only a file that can materially change this section; the list is not a reading checklist.")
        optional_directories = [
            str(path)
            for path in plan_entry.get("optional_directory_inputs", [])
            if str(path).strip()
        ]
        if optional_directories:
            lines.extend(["", "## Inspectable Resource Directories"])
            for path in optional_directories:
                lines.append(f"- `{path}`")
            lines.append(
                "- A directory is an availability root, not a text artifact. Never pass it to `read_file`; "
                "use one bounded `list_files` call, then read only a concrete file needed for the current argument."
            )
        missing_optional_count = int(plan_entry.get("missing_optional_input_count") or 0)
        if missing_optional_count:
            lines.extend(
                [
                    "",
                    "## Unavailable Optional Materials",
                    f"- {missing_optional_count} optional source(s) were absent when the plan was built. Do not probe their missing paths or treat them as required.",
                ]
            )
...
def _section_note_cards(section_id: str, note_cards: list[Any], *, limit: int) -> list[dict[str, Any]]:
    ...
    # Prefer coverage of different method/source lanes before filling remaining
    # slots by score.  A common failure mode was sending many near-duplicate
    # notes from the same method family to a section and missing the boundary
    # or bridge literature that makes a claim appropriately scoped.
    selected: list[dict[str, Any]] = []
    seen_lanes: set[str] = set()
    for card in ranked:
        lane = _note_card_diversity_lane(card)
        ...
        if len(selected) >= limit:
            return selected
    selected_ids = {str(card.get("note_id") or card.get("path") or "") for card in selected}
    for card in ranked:
        card_id = str(card.get("note_id") or card.get("path") or "")
...
def _note_card_diversity_lane(card: dict[str, Any]) -> str:
    ...
    if str(card.get("bridge_point") or "").strip():
        return "bridge"
    if str(card.get("boundary_conditions") or "").strip() or str(card.get("gaps") or "").strip():
        return "boundary_or_gap"
    path = str(card.get("path") or card.get("source_file") or "").strip()
    return f"source:{path}" if path else f"title:{str(card.get('title') or '').casefold()}"


def note_card_budget_for_section(section_id: str) -> dict[str, int]:
    """Return a copy of the deterministic writing budget for one section."""

    normalized = normalize_section_id(section_id)
    return dict(NOTE_CARD_BUDGETS.get(normalized) or NOTE_CARD_BUDGETS["related_work"])


def build_section_evidence_supplement(
    workspace: Path,
    *,
    section_id: str,
    resource_index: dict[str, Any],
    min_cards: int = 0,
    include_weak_cards: bool = True,
    reason: str = "section_evidence_insufficient",
    stage: str = "manuscript",
) -> dict[str, Any]:
    ...
    if requested_min > 0:
        requested_min = max(1, min(requested_min, budget["maximum"]))
    else:
        requested_min = budget["minimum_claim_usable"]
    selection_limit = min(
        budget["maximum"],
        max(budget["initial"], requested_min),
    )
    claim_usable = _section_note_cards(section_id, note_cards, limit=selection_limit)
    selected_ids = {str(card.get("note_id") or card.get("path") or "") for card in claim_usable}
    weak_cards: list[dict[str, Any]] = []
    if include_weak_cards and len(claim_usable) < requested_min:
        weak_cards = _section_weak_or_background_cards(
            section_id,
            note_cards,
            exclude_ids=selected_ids,
            limit=max(0, requested_min - len(claim_usable)) + 3,
        )
    fields = _note_card_fields_for_section(section_id)
    output_rel = f"drafts/section_outlines/{section_id}_evidence_supplement.md"
    retrieval_actions = _section_evidence_retrieval_actions(
        section_id=section_id,
        cards=claim_usable,
    ...
    lines = [
        f"# Section Evidence Supplement: {SECTION_TITLES.get(section_id, section_id)}",
        "",
        f"- reason: {reason}",
        f"- stage: {stage}",
        f"- section_id: {section_id}",
        f"- source: drafts/manuscript_resource_index.json -> paper_note_cards",
        f"- required note-card sections: {', '.join(fields)}",
        (
            "- note-card budget: "
            f"initial={budget['initial']}; minimum_claim_usable={budget['minimum_claim_usable']}; "
            f"maximum={budget['maximum']}; selected={len(claim_usable)}"
        ),
        "- rule: claim-usable cards may support citation-backed prose after exact note-section verification; weak/background cards are only for boundary text or resource upgrade decisions.",
        "",
        "## Section Need",
    ]
    lines.extend(_note_card_retrieval_lines(section_id, note_cards)[:4])
    lines.extend(["", "## Claim-Usable Note Cards"])
    if claim_usable:
        for card in claim_usable:
            _append_section_card(lines, card, fields, weak=False)
    else:
        lines.append("- None found. Do not invent citations; weaken the section claim or run a literature/resource upgrade before writing claim-bearing prose.")
    lines.extend(["", "## Weak / Background / Upgrade Leads"])
    if weak_cards:
    ...
    lines.extend(
        [
            "",
            "## Writer Actions",
            *[f"- {action}" for action in retrieval_actions],
            f"- Suggested output path for this supplement: `{output_rel}`",
            "",
        ]
    )
    return {
        "section_id": section_id,
        "claim_usable_count": len(claim_usable),
...
def _section_evidence_retrieval_actions(
    *,
    section_id: str,
    cards: list[dict[str, Any]],
    requested_min: int,
    ...
    """Return a bounded recovery ladder that the writer can execute literally."""

    # Give the action a stable target list instead of depending on short card
    # excerpts, which may omit a heading even though the full note has it.
    heading_labels = {
        "introduction": "§6, §9, §13, §18, §19",
        "related_work": "§2, §6, §9, §13, §14, §18, §19",
        "methodology": "§2, §14, §15, §18",
        "experiments": "§3, §12, §16",
        "analysis": "§13, §14, §18, §19",
        "conclusion": "§9, §18, §19",
        "abstract": "已有正文和 evidence pack（abstract 不引入 citation）",
    }.get(section_id, "§2, §9, §13")
    actions = [
        f"先针对本章读取 {len(cards)} 张已选 claim-usable 卡的精确笔记 section（目标：{heading_labels}），不要批量分页阅读所有 paper notes。",
        "对每张卡先用 grep_search 定位相应 heading，再用 read_file 读取该 heading 的局部内容，并把 citation 与句级 claim 对照。",
    ]
    if len(cards) < requested_min:
        actions.append(
            "当前 claim-usable 卡不足：先从本章的 weak/background leads 回查完整笔记；它们只能补充背景、边界或升级线索，不能直接支撑强断言。"
        )
    actions.extend(
        [
            "若精确 section 回查后仍没有直接证据，弱化/删除该 claim 或在 limitation 中说明缺口，并记录需要的证据类型。",
            "只有上述步骤仍留下明确、可描述的证据缺口时，才启动一次有 query、目标 section、语言和数量上限的定向文献补检；新来源必须先进入笔记卡后才能引用。",
        ]
    )
    return actions


def _section_weak_or_background_cards(
    section_id: str,
    note_cards: list[Any],
    *,
    exclude_ids: set[str],
    ...
    candidates = [
        ...
        and str(card.get("note_id") or card.get("path") or "") not in exclude_ids
        and not _paper_note_card_claim_usable(card)
    ]

    def score(card: dict[str, Any]) -> tuple[int, float, str]:
        field_hits = sum(1 for field in fields if str(card.get(field) or "").strip())
        try:
            quality = float(card.get("citation_quality_score") or 0.0)
        except (TypeError, ValueError):
            quality = 0.0
        return (field_hits, quality, str(card.get("title") or ""))

    return sorted(candidates, key=score, reverse=True)[: max(0, limit)]


def _append_section_card(lines: list[str], card: dict[str, Any], fields: tuple[str, ...], *, weak: bool) -> None:
    citation = str(card.get("citation_ref") or "").strip() or f"[note:{card.get('note_id')}]"
    title = _shorten(card.get("title"), 120)
    path = str(card.get("path") or "").strip()
    evidence = str(card.get("evidence_level") or "unknown")
    use = str(card.get("citation_use") or "unknown")
    score = card.get("citation_quality_score")
    warning = str(card.get("quality_warning") or ("weak_background_only" if weak else "")).strip()
    prefix = "background-only" if weak else "claim-usable"
    lines.append(f"- {citation} {title} | {prefix} | evidence={evidence} | use={use} | score={score} | note={path}")
    if warning:
        lines.append(f"  - warning: {warning}")
    cues = _note_card_section_cues(card, fields, limit=4)
    if not cues:
        lines.append("  - no matching section excerpt indexed; open the note before relying on it.")
    for cue in cues:
        lines.append(f"  - {cue}")
...
def audit_writing_craft(
    *,
    ...
    profile = dict(writing_profile or resolve_venue_writing_profile("", {"venue_style": venue_style}))
    profile_id = str(profile.get("id") or "unknown")
    budgets = section_word_budget_ranges(profile)
    storyline_headings = storyline_required_headings(profile)
    add(
        "venue_writing_profile",
        "WARN",
        profile_id != "unknown",
        (
            f"resolved profile={profile_id}; narrative_mode={profile.get('narrative_mode', 'unspecified')}; "
            "section targets are internal drafting diagnostics, not official venue limits."
        ),
    )
    ...
    add(
        ...
        "WARN",
        contribution_count == 0 or expected_count == 0 or contribution_count == expected_count,
        (
            f"detected introduction contribution bullets={contribution_count}; "
            f"alignment contribution lanes={expected_count}; counts are not a quality target and prose should use the smallest coherent set"
        ),
    )
    if gap_count:
    ...
    citation_density_issues = _manuscript_section_citation_issues(
        section_texts,
        available_bib_keys=set(extract_bib_keys_from_text(related_work_bib)),
    )
    add(
        ...
        (
            ...
            else "Claim-bearing sections pass bibliography-bounded citation absence guards; semantic alignment is checked separately."
        ),
    )
    ...
    word_count = _section_word_count(abstract)
    abstract_target = budgets.get("abstract")
    if abstract_target:
        lower, upper = abstract_target
    ...
    section_word_counts = {
        section_id: _section_word_count(text)
        for section_id, text in section_texts.items()
        if section_id in CORE_SECTIONS
    }
    for section_id, (lower, upper) in budgets.items():
        if section_id == "abstract":
            continue
        count = section_word_counts.get(section_id, 0)
        present = bool(section_texts.get(section_id, "").strip())
        add(
            f"section_budget_{section_id}",
            "WARN",
            (not present) or lower <= count <= upper,
            (
                f"{section_id} words={count}; internal profile target={lower}-{upper}; "
                "diagnostic only, not an official venue page or word limit."
            ),
        )

    normalized_storyline = storyline_text.casefold()
    missing_storyline_headings = [
        heading for heading in storyline_headings if heading.casefold() not in normalized_storyline
    ]
    add(
        "writing_storyline_coverage",
        "WARN",
        bool(storyline_text.strip()) and not missing_storyline_headings,
        (
            "storyline contains all profile headings"
            if storyline_text.strip() and not missing_storyline_headings
            else "missing storyline headings: " + ", ".join(missing_storyline_headings[:8])
            if storyline_text.strip()
            else "drafts/writing_storyline.md is absent or empty; create it during the outline phase."
        ),
    )
    ...
    markdown = _format_craft_audit_markdown(checks, rows, venue_style, profile=profile, section_word_counts=section_word_counts)
    return {
        "markdown": markdown,
        "json": {
            "version": "1.0",
            ...
            "venue_writing_profile": profile,
            "section_word_counts": section_word_counts,
            "missing_storyline_headings": missing_storyline_headings,
            "alignment_cids": cids,
            "checks": checks,
        },
    }
...
_CITATION_PROVENANCE_STRONG_CLAIM_RE = re.compile(
    r"\b(?:prove[sd]?|demonstrat(?:e|es|ed)|show(?:s|ed)?|find(?:s|ing)?|"
    r"reveal(?:s|ed)?|validate[sd]?|confirm(?:s|ed)?|outperform(?:s|ed)?|"
    r"improve(?:s|d)?|significant(?:ly)?|causal|causality|mechanism|"
    r"empirical(?:ly)?|experiment(?:s|al)?|effect(?:s|ive|iveness)?)\b|"
    r"证明|表明|显示|发现|验证|证实|显著|因果|机制|实证|实验|提升|改善|导致|影响",
    flags=re.IGNORECASE,
)
_CITATION_PROVENANCE_BOUNDARY_RE = re.compile(
    r"\b(?:abstract|metadata|background|preliminary|limited|scope|boundary|not evidence)\b|"
    r"摘要|元数据|背景|初步|有限|范围|边界|不作为证据",
    flags=re.IGNORECASE,
)


def build_citation_provenance_audit(
    workspace: Path,
    *,
    paper: str,
    ...
    """Audit every in-text citation against BibTeX, map, note, and evidence use.

    The result is deliberately narrower than a scholarly peer review.  It
    makes provenance failures mechanically non-negotiable: a cited key must
    resolve to a bibliography entry and a local source note, `do_not_cite`
    sources are prohibited, and abstract/low-quality sources cannot carry a
    strong empirical or causal claim without an explicit evidence boundary.
    Semantic adequacy beyond those checks still requires expert review.
    """

    bib_path = workspace / related_work_bib_path
    bib_text = bib_path.read_text(encoding="utf-8", errors="replace") if bib_path.exists() else ""
    bib_entries = {str(item.get("key") or "").strip(): item for item in parse_bib_entries(bib_text)}
    citation_map_path = workspace / "literature" / "citation_map.json"
    try:
        citation_map = json.loads(citation_map_path.read_text(encoding="utf-8"))
    except Exception:
        citation_map = {}
    raw_entries = citation_map.get("entries") if isinstance(citation_map, dict) else []
    map_by_key: dict[str, list[dict[str, Any]]] = {}
    if isinstance(raw_entries, list):
        for item in raw_entries:
            ...
            if key:
                map_by_key.setdefault(key, []).append(item)

    occurrences = iter_citation_occurrences(paper)
    cited_keys = sorted({item.key for item in occurrences})
    hard_failures: list[str] = []
    warnings: list[str] = []
    records: list[dict[str, Any]] = []
    quality_issues = bibtex_quality_issues(bib_text, require_author=True)
    for issue in quality_issues:
        issue_key = issue.split(":", 1)[0]
        if issue_key in cited_keys or ":" not in issue:
            hard_failures.append(f"BibTeX quality: {issue}")

    contexts_by_key: dict[str, list[str]] = {}
    for occurrence in occurrences:
        contexts_by_key.setdefault(occurrence.key, []).append(plain_latex_text(occurrence.context))

    for key in cited_keys:
        contexts = contexts_by_key.get(key, [])
        strong_context = any(_CITATION_PROVENANCE_STRONG_CLAIM_RE.search(context) for context in contexts)
        bounded_context = all(_CITATION_PROVENANCE_BOUNDARY_RE.search(context) for context in contexts if context) if contexts else False
        record: dict[str, Any] = {
            "key": key,
            "occurrence_count": len(contexts),
            "contexts": [_shorten(context, 300) for context in contexts[:3]],
            "strong_context": strong_context,
            "explicit_evidence_boundary": bounded_context,
            "status": "PASS",
            "issues": [],
            "warnings": [],
        }
        if key not in bib_entries:
            message = f"{key}: cited in TeX but absent from {related_work_bib_path}"
            hard_failures.append(message)
            record["issues"].append(message)
            record["status"] = "FAIL"
            records.append(record)
            continue
        map_entries = map_by_key.get(key, [])
        if not map_entries:
            message = f"{key}: cited key has no paper-note mapping in literature/citation_map.json"
            hard_failures.append(message)
            record["issues"].append(message)
            record["status"] = "FAIL"
            records.append(record)
        ...
        for mapping in map_entries:
            ...
            note_path = workspace / "literature" / source_file if source_file else None
            exists = bool(note_path and note_path.exists() and note_path.is_file())
            note_text = note_path.read_text(encoding="utf-8", errors="replace") if exists and note_path else ""
            citation_use = _markdown_field(note_text, "Citation Use") or "unknown"
            score = _parse_float(_markdown_field(note_text, "Citation Quality Score"))
            evidence_level = str(mapping.get("evidence_level") or _evidence_level_from_note(note_text) or "unknown")
            source = {
                "note_id": str(mapping.get("note_id") or ""),
        ...
        record["sources"] = source_statuses
        if not usable_sources:
            message = f"{key}: mapped paper note file is missing or unreadable"
            hard_failures.append(message)
            record["issues"].append(message)
        for source in usable_sources:
            use = str(source["citation_use"] or "unknown").strip().lower()
            score = source.get("citation_quality_score")
            evidence = str(source["evidence_level"] or "unknown").upper()
            if use in {"do_not_cite", "do-not-cite", "excluded", "unrelated"}:
                message = f"{key}: mapped note is marked {use}"
                hard_failures.append(message)
                record["issues"].append(message)
            if strong_context and ("ABSTRACT" in evidence or "METADATA" in evidence) and not bounded_context:
                message = f"{key}: {evidence} source is used for a strong claim without an explicit evidence boundary"
                hard_failures.append(message)
                record["issues"].append(message)
            if strong_context and isinstance(score, float) and score < 0.55 and not bounded_context:
                message = f"{key}: low-quality note (score={score:.2f}) is used for a strong claim"
                hard_failures.append(message)
                record["issues"].append(message)
            if use in {"background_only", "background_context"}:
                message = f"{key}: mapped note is background-only; keep citation to contextual framing rather than a core claim"
                warnings.append(message)
                record["warnings"].append(message)
            if score is None:
                message = f"{key}: citation-quality score unavailable; manual source review remains required"
                warnings.append(message)
                record["warnings"].append(message)
        if record["issues"]:
            record["status"] = "FAIL"
    ...
    return {
        ...
        "input_fingerprints": craft_audit_input_fingerprints(
            workspace,
            paper_path=paper_path,
            related_work_bib_path=related_work_bib_path,
        ),
        "summary": {
            ...
            "unique_citation_count": len(cited_keys),
            "citation_map_available": bool(raw_entries),
        ...
        "warnings": warnings,
        "scope_note": (
            "This audit verifies citation provenance and obvious evidence-boundary misuse. "
            "It does not replace expert review of the full scholarly interpretation."
        ),
    }


def render_citation_provenance_audit(audit: dict[str, Any]) -> str:
    summary = audit.get("summary") if isinstance(audit.get("summary"), dict) else {}
    lines = [
        "# Citation Provenance Audit",
        "",
        "This report verifies final-manuscript citation provenance and evidence boundaries. It is not a substitute for expert semantic review.",
        "",
        "## Summary",
        f"- Citation occurrences: {summary.get('citation_occurrence_count', 0)}",
        f"- Unique citation keys: {summary.get('unique_citation_count', 0)}",
        f"- Citation map available: {summary.get('citation_map_available', False)}",
        f"- Hard failures: {summary.get('hard_fail_count', 0)}",
        f"- Warnings: {summary.get('warning_count', 0)}",
        "",
        "## Hard Failures",
    ]
    hard_failures = audit.get("hard_failures") if isinstance(audit.get("hard_failures"), list) else []
    lines.extend(f"- {item}" for item in hard_failures[:60])
    if not hard_failures:
        lines.append("- None")
    lines.extend(["", "## Warnings"])
    warnings = audit.get("warnings") if isinstance(audit.get("warnings"), list) else []
    lines.extend(f"- {item}" for item in warnings[:60])
    if not warnings:
        lines.append("- None")
    ...
    for record in records:
        if not isinstance(record, dict):
            continue
        lines.append(
            f"- {record.get('key')}: {record.get('status')} "
            f"(occurrences={record.get('occurrence_count', 0)}, strong_context={record.get('strong_context', False)})"
        )
    return "\n".join(lines) + "\n"
...
def _write_style_variant_craft_audits(
    ...
    *,
    ...
    for style in ("is", "ccf_a"):
        ...
        variant_style = {"venue_style": style}
        if style == "is":
            variant_style["venue_profile"] = "informs_story"
        variant_profile = resolve_venue_writing_profile(params.target_venue, variant_style)
        audit_doc = audit_writing_craft(
            paper=paper,
            ...
            cdr_ledger=cdr_ledger,
            venue_style=style,
            writing_profile=variant_profile,
...
def _fallback_alignment_lanes(contribution_claims: Any) -> list[dict[str, Any]]:
    ...
    # Preserve one lane per actual contribution when possible.  The former
    # minimum of three manufactured placeholder contributions for a project
    # that had only one or two claims and then forced Writer to rationalize
    # them downstream.
    target_count = min(4, max(1, len(claims) if claims else 1))
...
def _section_word_count(text: str) -> int:
    """Count prose tokens for internal drafting diagnostics, not submission limits."""

    plain = plain_latex_text(text or "")
    latin_words = re.findall(r"[A-Za-z]+(?:[-'][A-Za-z]+)?", plain)
    cjk_characters = re.findall(r"[\u3400-\u4dbf\u4e00-\u9fff]", plain)
...
def _manuscript_section_citation_issues(
    section_texts: dict[str, str],
    *,
    ...
    for section_id, minimum in MANUSCRIPT_SECTION_MIN_CITATIONS.items():
        ...
        effective_minimum = min(minimum, len(available_bib_keys)) if available_bib_keys is not None else minimum
        if len(cited) < effective_minimum:
            issues.append(
                f"{section_id} has {len(cited)} unique citations; "
                f"absence_guard={effective_minimum}, available_traceable_keys={len(available_bib_keys or set())}"
            )
    return issues
...
def _mechanical_punctuation_style_hits(text: str) -> list[str]:
    ...
    semicolon_count = len(re.findall(r";|；", prose))
    if semicolon_count >= 8:
        hits.append(f"{semicolon_count} semicolon joins")
...
def _format_craft_audit_markdown(
    checks: list[dict[str, Any]],
    rows: list[dict[str, Any]],
    venue_style: str,
    *,
    profile: dict[str, Any] | None = None,
    section_word_counts: dict[str, int] | None = None,
) -> str:
    ...
    lines = [
        ...
        f"- Venue writing profile: `{(profile or {}).get('id', 'unknown')}` (internal drafting diagnostics; not official venue limits)",
    ...
    if section_word_counts:
        ...
        for section_id, count in section_word_counts.items():
...
def _artifact_entry(workspace: Path, path: Path, *, include_preview: bool) -> dict[str, Any]:
    ...
    if path.is_dir():
        ...
        return {
            ...
            "substantive_file_count": sum(1 for child in files if child.name != "_DIR_GUIDE.md"),
        }
    entry = {
        "path": rel,
...
def _extract_storyline_for_section(storyline_text: str, section_id: str) -> str:
    """Carry the most relevant parts of the LLM storyline across T8 stages."""

    if not storyline_text.strip():
        return ""
    heading_map = {
        "introduction": {"research problem", "technical bottleneck", "root technical reason", "core insight", "contribution claims"},
        "related_work": {"technical bottleneck", "root technical reason", "contribution claims"},
        "methodology": {"core insight", "method mapping"},
        "experiments": {"method mapping", "evidence and ablation map"},
        "analysis": {"alternative explanations", "failure modes and limitations", "evidence and ablation map"},
        "conclusion": {"contribution claims", "failure modes and limitations", "reviewer questions"},
        "abstract": {"research problem", "core insight", "contribution claims"},
    }
    wanted = heading_map.get(section_id, set())
    matches = list(re.finditer(r"(?m)^##\s+(.+?)\s*$", storyline_text))
    selected: list[str] = []
    for index, match in enumerate(matches):
        title = " ".join(match.group(1).casefold().split())
        if title not in wanted:
            continue
        end = matches[index + 1].start() if index + 1 < len(matches) else len(storyline_text)
        selected.append(storyline_text[match.start():end].strip())
    if not selected:
        return storyline_text.strip()[:2500]
    return "\n\n".join(selected)[:5000]
