...
class ReaderAgent(Agent):
    ...
    def initial_user_message(self, ctx: ExecutionContext) -> str:
        ...
        return prepend_resume_prefix(
            ...
            "先调用 build_synthesis_workbench（不传 llm_insights）建立完整审计 workbench 与紧凑 synthesis_context 索引，不要逐文件无差别通读；"
            "在统一推理时先读 synthesis_context.json 与 outline，而且每个大索引只读一次；运行时会保留完整首读结果，重复读取不会增加信息。"
            "不要反复读取 synthesis_context.json、synthesis_workbench.json 或 bridge paper_catalog；只有运行时明确提示历史已被裁剪时才重新读取。"
            "不要把完整 workbench 读入会话；只对关键含糊点定向回查 note section，"
