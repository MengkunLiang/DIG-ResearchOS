...
- Section drafts: draft `abstract`, `introduction`, `related_work`, `methodology`, `experiments`, `analysis`, `limitations`, and `conclusion` as separate files under `drafts/sections/`.
...
- Limitations: direct-full evidence boundaries, skipped pilot/novelty-final risks if applicable, external validity, cost, and failure cases.
...
- If T5/T6 were skipped, state the evidence boundary in Limitations and avoid claiming pilot-validated novelty-final evidence.

## Tool Boundary

- Use `build_manuscript_resource_index`, `plan_manuscript_sections`, `plan_manuscript_evidence`, `assemble_manuscript`, and `audit_manuscript_claims` for mechanical workflow.
