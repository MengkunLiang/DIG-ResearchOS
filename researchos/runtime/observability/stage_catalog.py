...
_PROFILES: dict[str, StageProfile] = {
    ...
    "T4.5": StageProfile("T4.5 · 研究方案审计与正式化", "将已选方向经相似工作审计、主张正式化和质量审阅，形成可执行研究方案", "该方向在已知工作中是否可区分，并能否形成有明确机制、实验映射和证伪边界的研究方案？", ("审计相似工作", "明确机制与必需基线", "正式化研究主张与实验", "审阅方案质量"), "T4.5 包含查新审计、研究正式化和 orientation-aware 质量审阅三个连续子阶段；只有全部通过后才进入 T5。"),
    "T4.5-FORMALIZE": StageProfile("T4.5 · 研究正式化", "用统一研究蓝图连接问题、技术方案、claims 与实验", "已通过审计的候选能否形成可证伪、可执行且有现实意义的研究方案？", ("写入研究蓝图", "登记可证伪 claims", "映射实验", "编写正式 Proposal")),
    "T4.5-REVIEW": StageProfile("T4.5 · 方案质量审阅", "按已选 orientation 定向复核并修复研究方案", "技术贡献、设计理由、评测、现实意义与所有结构化产物是否一致？", ("检查研究论证", "检查 claim-实验映射", "定向修复", "记录质量评分")),
    ...
    "T5-RESOURCE-PREP-WAIT": StageProfile("T5 · 外部资源自动准备", "等待外部执行器检索、审查并记录公开资源", "公开数据、代码、benchmark 与 baseline 能否在不改变研究边界的前提下获得？", ("上下文核对", "检索与静态审查", "资源来源与准备度记录")),
    "T5-PROTOCOL-GATE": StageProfile("T5 · 实验协议就绪确认", "区分可自动准备的资源与完整实验授权", "当前研究约束下，哪些公开资源和可执行设置可先自动补齐，哪些变更会改变研究结论范围？", ("展示已编译协议", "提供自动资源准备分支", "仅在研究边界变化时等待确认"), "公开资源检索、固定版本获取、静态审查与稳定默认 seed 不要求人工上传或逐项决定；完整实验仍不得静默改变 T4.5 已定义的研究任务、机制、必需 baseline、benchmark 范围或 claim 边界。"),
    "T5-EXPR-MATERIAL-GATE": StageProfile("T5 · 已有实验材料盘点", "盘点已有数据、代码仓库、模型权重和基线材料", "哪些项目材料已经可用，哪些应由受限执行器自动检索和审查？", ("盘点已有材料", "提示自动资源准备入口", "等待你的选择")),
