...
def build_config_audit_summary(config_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "configuration_layers": [
            ...
            "输出校验的默认自动修复次数由 config/system_config/runtime.yaml 的 agent_behavior.max_validation_retries 控制；agent_params 或用户级 agent 设置可为单个 agent 覆盖。T3 深读队列未完成属于确定性续读，不消耗这个次数。T3.6 的 section/assemble/review 写作阶段，以及 T4.5-FORMALIZE / T4.5-REVIEW，使用 source-aware 定向修复：相关 artifact 必须真实变化，同一诊断无变化会立即暂停，持续变化但不收敛也会在 8 轮总安全窗后暂停。T4.5 的安全窗还会以 task、已选 Candidate 与上游证据基线写入 workspace，resume 不会重置它。T3.6 只可对语言混合、紧凑 taxonomy 内容吸收和中英 review 维度等预先列举的 prose 误判进行独立 LLM 复核；T4.5 对其预先列举的 Proposal/hypothesis prose 项适用同一机制。两者均要求当前 artifact 的逐字证据和哈希绑定 receipt，绝不覆盖 schema、引用/证据、BibTeX、TeX/PDF、verdict、lineage、实验映射、反重复或审计语言硬门。",
            ...
            "context fallback、truncation 和可选 rate_limit 与连接字段保存在同一个 config/model_settings.yaml；truncation 默认跟随有效 context，rate_limit 默认关闭且与上下文容量无关；旧 endpoint/profile 文件仅为历史部署保留兼容读取。",
        ],
        "model_settings": {
