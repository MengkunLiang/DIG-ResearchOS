...
def build_config_audit_summary(config_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "configuration_layers": [
            ...
            "输出校验的默认自动修复次数由 config/system_config/runtime.yaml 的 agent_behavior.max_validation_retries 控制；agent_params 或用户级 agent 设置可为单个 agent 覆盖。T3 深读队列未完成属于确定性续读，不消耗这个次数。T3.6 的 section/assemble/review 写作阶段，以及 T4.5-FORMALIZE / T4.5-REVIEW，例外：质量 Gate 会将具体失败原因和最小修复范围注入相应 Writer，相关 source artifact 持续变化时不按固定次数停止。T3.6 只可对语言混合、紧凑 taxonomy 内容吸收和中英 review 维度等预先列举的 prose 误判进行独立 LLM 复核；T4.5 对其预先列举的 Proposal/hypothesis prose 项适用同一机制。两者均要求当前 artifact 的逐字证据和哈希绑定 receipt，绝不覆盖 schema、引用/证据、BibTeX、TeX/PDF、verdict、lineage、实验映射、反重复或审计语言硬门。只有同一错误且未修改与该诊断相关的 source artifact、且未获有效语义 receipt 时才暂停以避免无声循环。",
