    ...
    return {
        ...
        "active_global_controls": {
            "model_settings_yaml": [
                ...
                "(optional)context_window_override",
        ...
        "configuration_layers": [
            "日常只改 config/model_settings.yaml 中的 provider、api_base、api_key、model、fallback、可选 context_window_override、context_window_fallback、truncation 和可选 rate_limit，或运行 `researchos configure-llm`；未声明容量时先自动探测、失败才用 fallback；max_input_tokens 仅是高级 gateway 覆盖；所有 Agent 使用同一模型。",
            ...
            "输出校验的默认自动修复次数由 config/system_config/runtime.yaml 的 agent_behavior.max_validation_retries 控制；agent_params 或用户级 agent 设置可为单个 agent 覆盖。T3 深读队列未完成属于确定性续读，不消耗这个次数。T3.6 的 section/assemble/review 写作阶段，以及 T4.5-FORMALIZE / T4.5-REVIEW，使用持久化的 source-aware 定向修复：同一诊断且关联 artifact 无变化会立即暂停；只要相关 source 出现实质变化，resume 可以继续定向修复，不设任意的累计次数上限。T3.6 只可对语言混合、紧凑 taxonomy 内容吸收和中英 review 维度等预先列举的 prose 误判进行独立 LLM 复核；T4.5 对其预先列举的 Proposal/hypothesis prose 项适用同一机制。两者均要求当前 artifact 的逐字证据和哈希绑定 receipt，绝不覆盖 schema、引用/证据、BibTeX、TeX/PDF、verdict、lineage、实验映射、反重复或审计语言硬门。",
            ...
            "可选 context override、context fallback、truncation 和可选 rate_limit 与连接字段保存在同一个 config/model_settings.yaml；未声明 override 时先探测真实容量，truncation 默认跟随有效 context，rate_limit 默认关闭且与上下文容量无关；旧 endpoint/profile 文件仅为历史部署保留兼容读取。",
        ],
        "model_settings": {
            ...
            "context_window_override": model_settings.get("context_window_override"),
