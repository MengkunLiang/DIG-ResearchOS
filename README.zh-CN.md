...
- T8 写作已经拆成 `T8-STYLE-GATE -> T8-RESOURCE -> T8-WRITE -> T8-SECTION-PLAN -> T8-SEC-* -> T8-DRAFT -> T8-SELF-CHECK -> review/revise -> T8-PAPER-CLAIM-AUDIT`，会先确认 IS/CCF-A/both 写作风格、写作语言和 LaTeX 模板 family/id（同样支持 `basic_zh`、`basic_en`、`ccf`、`utd`、`other`；CCF 默认 NeurIPS，UTD 默认 INFORMS），再生成资源索引、证据计划、图表计划、`paper_state.json` 和每章局部大纲，再用一个节点只写一个 section 的方式逐章生成正文，最后拼装审计；Limitations 已并入 Conclusion。T8 的每章大纲也包含 `Section Writing Contract`，规定 Abstract/Introduction/Related Work/Method/Experiments/Analysis/Conclusion 各自的 purpose、required_content、internal_shape、evidence_rules 和 avoid，避免正文写成同质化模板或文献流水账。`assemble_manuscript` 会尽量应用本地模板：`basic_zh/basic_en`、`utd/informs`、`ccf/neurips` 可直接装配，缺少模板时回退到基础 article/ctex，并在 TeX 注释中记录 fallback。
