...
- T3 deep read 后会运行 Reader LLM abstract sweep，向 `literature/paper_notes_abstract/` 补充 active shallow 论文和一部分 backlog 候选的 abstract-only 或 metadata-only 轻量笔记；确定性模板只作为失败兜底
