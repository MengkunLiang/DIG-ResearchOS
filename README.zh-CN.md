...
- T2 正常路径由检索工具返回值触发 runtime 自动保存 raw。Scout 负责设计 query 和 LLM `semantic_screen`，`expand_queries` 只做机械合并/去重，不内置领域知识。runtime 收尾会对所有 eligible DOI/OpenAlex 候选做 OpenAlex DOI/OA 详情补全、Crossref DOI 详情补全和多源摘要回填；对无 DOI/OpenAlex ID 的 seed/弱记录再做高置信 OpenAlex 标题兜底补全。OpenAlex/Crossref citation snowball 是 bounded one-hop 补充，不是 all-eligible backfill。之后 runtime 会确定性生成 dedup、verified、deep-read/shallow-read queue 和审计文件；每篇 `papers_verified.jsonl` 论文都必须有 deep_read 或 shallow_read/backlog 去向，不能因为缺少 `semantic_screen` 被静默丢弃。`search_log.md` 会记录 `eligible/attempted/skipped_by_cap/failed/remaining_missing_*`、citation `reference_items_seen/non_doi_references_skipped/title_references_resolved`、OpenAlex/Crossref snowball cap 等字段，用于判断摘要、PDF/OA hint、references 缺失到底是源缺失、网络失败还是截断。
...
- citation graph 会转成实际 T3 证据链：OpenAlex/Crossref references 写入 `citation_edges.json`，OpenAlex/Crossref bounded one-hop snowball 候选进入 raw/dedup/verified/queue；结构 hub 会占用小额 `citation_hub_slots` 保护槽或以普通 ranking 进入 active target。缺少 semantic screen 的 hub 会标记 `citation_hub_needs_reader_screening=true`，由 T3 Reader 复核；已有 LLM screen 明确排除的 hub 不强制 deep-read。
...
- T8/T9 的实验 claim 必须经过 evidence pack 闭环：`drafts/experiment_evidence_pack.json` 归一化外部执行器证据，`drafts/result_to_claim.json` 把指标转成可写 claim，`drafts/paper_claim_audit.md/json` 在 T9 前检查正文数字、强 claim、mock-only evidence 和 forbidden wording。`paper_claim_audit.json` 必须带 `input_fingerprints`，绑定当前 `paper.tex`、evidence pack 和 result-to-claim hash；旧 audit 不允许放行新正文，审计失败回 `T8-REVISE-2` 定向修订。
...
- pipeline 基本可运行
- 关键阶段已具备断点恢复
- T2 正常路径由检索工具返回值触发 runtime 自动保存 raw；Scout 可以在 `semantic_screen` 前调用 `backfill_paper_abstracts`，但 runtime finalize 也会兜底执行全 eligible DOI/OpenAlex 详情补全、多源摘要回填、Crossref 详情补全和 title-only seed/弱记录的高置信 OpenAlex 标题补全。OpenAlex/Crossref citation snowball 是 bounded one-hop 补充，并分别记录 cap、persisted、merged 和 failed。随后 runtime 确定性完成 dedup、verified、deep-read/shallow-read queue 和审计文件。启动时会用 `inspect_user_seeds` 区分真实 seed 与 `_DIR_GUIDE.md`/模板/空文件。`search_log.md` 会显示 attempted、failed、skipped_by_cap、remaining_missing_*、OpenAlex/Crossref snowball 统计和 title fallback 解析数，避免“检索到了但不知道为什么没摘要/PDF/引用”的黑箱状态。
...
- T9 已经改成“编译失败则修复并重试”的投稿包阶段；`prepare_submission_bundle` 会写 `submission/bundle/bundle_manifest.json`，记录当前 `drafts/paper.tex`、`literature/related_work.bib`、bundle 内 main/references 和 copied figures 的 hash，避免旧 bundle 在源文件变更后被误用。`latex_compile` 会保留 `submission/compile_report.json.attempts` / `attempt_count` 历史，同一 TeX 与依赖 fingerprint 的成功只有在 PDF/log/hash/mtime/size 全部匹配时才复用，同一 hash 的源级失败不会重复编译，必须修改 TeX 或依赖后再重试。resume 时如果已有投稿包通过 validator，会在环境检查和 LLM 前直接确定性完成。
- provider 稳定性仍会影响长任务
