...
- T2 正常路径由检索工具返回值触发 runtime 自动保存 raw。Scout 负责设计 query 和 LLM `semantic_screen`，`expand_queries` 只做机械合并/去重，不内置领域知识。runtime 收尾会先把全量 `papers_raw.jsonl` 去重成审计池，再按 `config/agent_params.yaml -> agents.scout.behavior.t2_finalize.active_pool_max` 选出 active candidate pool 写入 `papers_dedup.jsonl` / `papers_verified.jsonl`，超额候选写入 `papers_backlog.jsonl`。OpenAlex DOI/OA、Crossref DOI、多源摘要和标题兜底补全只面向 active pool，避免几百篇弱召回拖垮 API 和 LLM 预算。OpenAlex/Crossref citation snowball 是从 seed 或 `semantic_screen.can_enter_deep_read=true` 来源触发的一次性 bounded one-hop 补充，不是对全量 raw 的回填。每篇 active `papers_verified.jsonl` 论文都必须有 deep_read 或 shallow_read/backlog 去向，不能因为缺少 `semantic_screen` 被静默丢弃。
...
- T3 deep read 后会运行 Reader LLM abstract sweep：有 abstract 的 active shallow/backlog 候选写入 `literature/paper_notes_abstract/`，纯 metadata-only 候选批量写入 `literature/metadata_triage.md` 作为补资源/升级阅读线索；确定性模板只作为失败兜底
