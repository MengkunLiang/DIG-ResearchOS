...
- T2 正常路径由检索工具返回值触发 runtime 自动保存 raw。Scout 负责设计 query 和 LLM `semantic_screen`，`expand_queries` 只做机械合并/去重，不内置领域知识。runtime 收尾会先把全量 `papers_raw.jsonl` 去重成审计池，对排序后的前置候选做 bounded light backfill（标题/DOI/OpenAlex/OA/摘要 hint），再按 `config/agent_params.yaml -> agents.scout.behavior.t2_finalize.active_pool_max` 选出 active candidate pool 写入 `papers_dedup.jsonl` / `papers_verified.jsonl`，超额候选写入 `papers_backlog.jsonl`。`must_explore` 与 `should_explore` bridge 有不同 active cap；同一 bridge 即使命中 semantic screen、citation snowball 或 metadata fill，也不能绕过 cap，`no_cross/skip` 不强制进入 T3。OpenAlex/Crossref citation snowball 是从 seed 或 `semantic_screen.can_enter_deep_read=true` 来源触发的一次性 bounded one-hop 补充，不是对全量 raw 的回填；`snowball_max_candidates` 是 OpenAlex 与 Crossref 共享的全局尝试上限，不会各跑一遍 cap。每篇 active `papers_verified.jsonl` 论文都必须有 deep_read 或 shallow_read/backlog 去向，不能因为缺少 `semantic_screen` 被静默丢弃；active pool 外和 domain-profile 排除记录保留在 backlog 审计链，不会被普通 T3 abstract sweep 自动读回。T2 收尾会写 `literature/t2_finalize_manifest.json`，把 `project.yaml`、seed 文件/PDF、`bridge_domain_plan.json`、PDF 目录和配置 hash 绑定；这些输入变更后旧 T2 产物不能被 resume 误复用。
...
- T3.6 是可选综述论文支线：T3.5 后通过状态机级 immediate gate 问“是否撰写综述论文”，选择 yes 后按 taxonomy 规划、人工确认、逐 section 写作、拼装审阅、LaTeX 编译和导出 `survey_insights.json` 的方式执行；它不是把 `synthesis.md` 转成 TeX。`decision.json` 或 `corpus_decision.json` 缺失/损坏时会回到对应 gate；上游 synthesis/corpus 变更后旧 gate 决策也会失效。每个 section、`survey_state.json`、`survey_audit.json`、`survey_review_actions.json` 和 `survey_compile_report.json` 都有 input fingerprint 或 TeX/PDF/log/dependency hash 绑定；Abstract 禁止正式引用，section 禁止 placeholder 和未知 BibTeX key，旧 PDF/log 不能在 TeX 或 references 变化后放行。`survey_insights.json` 会合并 `survey_plan` 与 `survey_state.shared_facts` 中的 `resource_upgrade_needs`，把 taxonomy/challenge/future hints 传给 T4。T4 可以生成 `survey_driven` 候选，但 weak-only 候选必须标 `not_supported_by_current_evidence`，不能被选中或绑定最终 hypothesis。
...
- T8/T9 的实验 claim 必须经过 evidence pack 闭环：`drafts/experiment_evidence_pack.json` 归一化外部执行器证据，`drafts/result_to_claim.json` 把指标转成可写 claim，`drafts/paper_claim_audit.md/json` 在 T9 前检查正文数字、强 claim、mock-only evidence 和 forbidden wording。`paper_claim_audit.json` 必须带 `input_fingerprints`，绑定当前 `paper.tex`、evidence pack 和 result-to-claim hash；旧 audit 不允许放行新正文，审计失败回 `T8-REVISE-2` 定向修订。`craft_audit.json` 还会拦截 Abstract 正式引用、正文/注释中的 `C1`/`CID` 内部标号、`TODO/TBD/PLACEHOLDER/LLM review required` 等规划残留；T9 对 `submission/bundle/main.tex` 再做一次同类脏标记扫描，防止模板迁移阶段把内部标号带进最终投稿包。
...
- T3.6 会复用 `drafts/survey/survey_plan.json`、`survey_state.json`、`section_outlines/`、`sections/*.tex`、`survey_audit.json`、`survey_review_actions.json` 和 `survey_compile_report.json`，但只在 input fingerprint、TeX/PDF/log hash 和 dependency fingerprint 仍匹配当前文件时复用；中断后会按 section 继续，不需要重写整个 survey
...
- T3 deep read 后会运行 Reader LLM abstract sweep：默认只从 active verified/dedup 的 shallow 候选写入 `literature/paper_notes_abstract/`，纯 metadata-only 候选批量写入 `literature/metadata_triage.md` 作为补资源/升级阅读线索；active pool 外的 `papers_backlog.jsonl` 默认只做覆盖审计和人工/显式回捞。确定性模板只作为失败兜底
