...
想法
 ...
 -> 假设与实验计划
 -> 新颖性预审
 -> External Experiment Re-boost / Handoff / Automatic Skill Customization / Dry-run / Ingest / Audit / Claims
 -> PI Evaluate
...
- 多阶段断点恢复
- artifact 校验
- T4 假设生成会同时落盘 `ideation/_gate1_candidate_cards.md`、`ideation/selected_idea_brief.md`、`ideation/idea_scorecard.yaml`、`ideation/rejected_ideas.md`、`ideation/gate_decisions.json` 和 `ideation/idea_rationales.json`，记录每个 idea 的证据链、用户选择和决策链
- T2 前会先进入 `T2-PARAM-GATE`，显示当前检测到的任务类型、推荐档位、各档实际参数和自定义输入说明，再确认“保留候选数、精读目标、摘要轻读目标、写作语言/中文文献策略”，并写入 `literature/literature_params.json`；直接回车采用当前推荐项，也可直接输入“英文稿，不要中文论文，候选数300”。这不是网络请求 cap：对保留候选会尽可能补 DOI、摘要、OpenAlex/Crossref/Semantic Scholar/arXiv 和 PDF hints；内部 safety cap 只防异常失控。英文稿且用户明确排除中文文献时，非 seed 中文论文不会进入 active pool；中文/双语或显式允许中文文献时，T2 不再因为缺少权威来源标签硬过滤中文候选，而是标记 `authority_review_needed`，后续 Reader/Writer 需要结合真实期刊目录、全文和人工判断决定是否引用。T2 正常路径由检索工具返回值触发 runtime 自动保存 raw。Scout 负责设计 query 和 LLM `semantic_screen`，`expand_queries` 只做机械合并/去重，不内置领域知识。runtime 收尾会先把全量 `papers_raw.jsonl` 去重成审计池，对排序后的前置候选做 bounded light backfill，再按 `literature/literature_params.json` 或默认配置中的 `active_pool_max` 选出保留候选写入 `papers_dedup.jsonl` / `papers_verified.jsonl`，超额候选写入 `papers_backlog.jsonl`。`must_explore` 与 `should_explore` bridge 有不同保留上限；同一 bridge 即使命中 semantic screen、citation snowball 或 metadata fill，也不能绕过 cap，`no_cross/skip` 不强制进入 T3。每篇保留候选都必须有 deep_read、shallow_read 或 backlog/deferred 去向，不能因为缺少 `semantic_screen` 被静默丢弃。T2 收尾会写 `literature/t2_finalize_manifest.json`，把 `project.yaml`、seed 文件/PDF、`bridge_domain_plan.json`、PDF 目录和配置 hash 绑定；这些输入或 coverage 参数变更后旧 T2 产物不能被 resume 误复用。
...
- T3 论文阅读以当前队列的 `queue_rank` 为工作单位：`lookup_paper_record(queue_rank=...)` 取单篇 metadata，`save_paper_note(queue_rank=..., content=...)` 自动生成 note 路径、即时校验并刷新 `literature/notes_manifest.json`，并在 CLI 打印确定性 `T3 deep read progress: x/y`。`require_deep_read_target=true` 时必须读满 `deep_read_target` 才能进入 T3.5；只有明确配置为 false 才允许达到 `deep_read_min` 后放行。resume 时 `deep_read_queue_pending.jsonl` 会把当前待读项重排为 1..N，同时保留 `original_queue_rank` 指回完整 `deep_read_queue.jsonl`，避免 pending rank 和原始 rank 混淆导致重复阅读。`notes_manifest.json` 同样绑定 T2 队列、verified/dedup、domain_map、seed profile/PDF、`literature/pdfs/` 和配置 hash，并记录 `citation_quality_score/citation_use/citation_quality_rationale`。T3.5/T4/T8 只应把 `score>=0.55` 且 `core_evidence/supporting_context` 的论文作为核心论证材料；低分、`do_not_cite`、`citation_allowed=false` 或中文英文稿不匹配的材料只能做背景、风险或补资源线索。命中 `user_seeds/pdfs/` 的 seed 会进入 queue/access audit 时标记 `evidence_level=FULL_TEXT` 与 `access_level_hint=FULL_TEXT_LOCAL`；`evidence_level` 是证据强度枚举，不会新增 `seed_pdf` 枚举。每篇 `paper_notes/*.md` 都必须记录 `## 12. Reading Coverage`，PDF 可用时必须覆盖到最后一页，只有完整页码覆盖且最终无截断时才能标记 `[FULL-TEXT]`
...
- T3.6 是可选综述论文支线：T3.5 后通过状态机级 immediate gate 问“是否撰写综述论文”，选择 yes 后先进入 `T3.6-TEMPLATE-GATE` 确认写作语言与 LaTeX 模板，再按 taxonomy 规划、人工确认、逐 section 写作、拼装审阅、LaTeX 编译和导出 `survey_insights.json` 的方式执行；它不是把 `synthesis.md` 转成 TeX。模板 family 支持 `basic_zh`、`basic_en`、`ccf`、`utd`、`other`；CCF 默认 `template_id=neurips`，也可直接选 `iclr`、`icml`、`kdd`，UTD/CDS/Management Science/ISR/MISQ 默认 `template_id=informs`，选择会写入 `drafts/survey/writing_template.json` 并要求 `survey_plan.template_selection` 一致。综述质量标准来自通用综述写作原则，而不是当前题目模板：PLAN 必须写出 `writing_language`、`central_question`、`scope_boundaries`、`review_contribution`、`quality_plan`，并要求每个核心 section 有 `reader_question/section_argument`。默认写成紧凑专业综述，taxonomy 类放在 `Taxonomy` 和 `Comparative Analysis` 内部，而不是为每个类拆出独立 theme 大章；`survey_plan.sectioning_policy` 会在 PLAN 阶段被校验，缺失或 compact 模式下仍输出 `theme_*` 会被拒绝。`T3.6-SEC-THEME-*` 只是兼容/显式长综述槽位，默认 skipped；skipped 只表示不写独立大章，不表示删除内容，`compact_theme_content_absorbed` 审计会要求每个 taxonomy class 已被 Taxonomy 和 Comparative Analysis 吸收。`build_survey_state` 会把每节不同的 `Section Writing Contract` 写入 `section_outlines/*.md`，规定本节 purpose、required_content、internal_shape、evidence_rules 和 avoid，防止 Abstract/Introduction/Background/Taxonomy/Comparison/Challenges/Future/Conclusion 用同一套短模板；如果旧 plan/outline 带有精读数量、覆盖层、metadata triage 等内部过程表述，会在 outline 生成和 section/audit 阶段被替换或拦截。SECTION/ASSEMBLE 会拦截中英混写、篇幅过短、缺少本节标题、文献串烧、空泛 future/gap、placeholder、内部 `C1/CID/ResearchOS alignment` 标号、运行过程正文、引用过少、引用多样性不足、引用与当前论断明显错配、未知 BibTeX key 和实际引用条目的 BibTeX 质量问题；REVIEW 还必须检查 review contribution 与语言/深度，不能把语言混杂降级为低风险。`decision.json`、`writing_template.json` 或 `corpus_decision.json` 缺失/损坏时会回到对应 gate；上游 synthesis/corpus/template 变更后旧 gate 决策也会失效。每个 section、`survey_state.json`、`survey_audit.json`、`survey_review_actions.json` 和 `survey_compile_report.json` 都有 input fingerprint 或 TeX/PDF/log/dependency hash 绑定；`survey_assemble` 要求 audit 包含最新的 citation density、citation diversity、citation-claim alignment、runtime-process 和 bibliography quality 检查，旧 audit 不能继续放行。Abstract 源文件只能写纯正文，禁止正式引用、`\section{Abstract}` 和 `\begin{abstract}`，旧 PDF/log 不能在 TeX 或 references 变化后放行。`survey_insights.json` 会合并 `survey_plan` 与 `survey_state.shared_facts` 中的 `resource_upgrade_needs`，把 taxonomy/challenge/future hints 传给 T4。T4 可以生成 `survey_driven` 候选，但 weak-only 候选必须标 `not_supported_by_current_evidence`，不能被选中或绑定最终 hypothesis。
- T4 现在有正式状态机 Gate1：先由 `IdeationAgent` 生成 `_pass1_forward_candidates.json`、`_pass2_grounding_review.json`、`_candidate_directions.json`、`_gate1_candidate_cards.md` 和 `_gate1_selection_brief.md`；runtime 的 Gate1 默认展示 Markdown 候选卡片的路径/摘要，完整内容在 `ideation/_gate1_candidate_cards.md`，JSON 只作为机器可读附录路径，不直接刷给用户。CLI 可直接输入 `D1`、`D1+D3`、`merge D1+D3`、`new: ...` 或 `reanalyze: ...`。用户选择会写入绑定候选池 fingerprint 的 `ideation/_gate1_user_selection.json`，同时生成/刷新 `ideation/selected_idea_brief.md` 作为选择确认；再回到 T4 生成最终 `hypotheses.md`、`exp_plan.yaml`、scorecard、决策链和风险。最终 `hypotheses.md` 每个 H 必须包含技术机制、现实/管理/商业含义、评分依据、核心论文依赖和证伪/停止条件。最终 T4 产物必须晚于 Gate1 选择，候选池在等待期间变化会重新展示 gate；如果旧 workspace 已经有 ready 候选池却停在 T4 死锁/PAUSED，resume 会先补写候选卡片并展示 Gate1。用户选择“重新分析”时不会写 `_gate1_user_selection.json`，而是写 `_gate1_reanalysis_request.json` 并归档旧候选池，避免误入最终假设阶段。T4 的 deadlock hash 使用稳定 SHA256，并把 Gate1 selection fingerprint 纳入 post-Gate1 参数，避免旧的 pre-Gate1 失败记录阻塞后半段；T4/T4.5 的复用 freshness 会把 `literature/bridge_domain_plan.json` 纳入输入，避免 bridge 计划变化后复用旧 idea。
- 当前主链从 `T4.5` 进入外部实验链：`T5-REBOOST-GATE -> T5-HANDOFF -> T5-SKILL-CUSTOMIZATION-GATE -> T5-EXPR-MATERIAL-GATE -> T5-EXECUTOR-GATE -> T5-DRY-RUN/T5-EXTERNAL-WAIT -> T7-INGEST -> T7-AUDIT -> T7-POST-NOVELTY -> T7-CLAIMS`。ResearchOS 负责用 LLM API 做 context re-boost、编译协议、准备 Codex/Claude/manual prompt、复制并自动定制外部执行 skills、等待用户放置实验材料、摄取结果、审计证据、实验后 novelty 复核和生成 result-to-claim；真实实验由外部执行器在隔离路径完成。`PARTIAL_RESULTS_READY` 默认不能通过 `T5-EXTERNAL-WAIT` 进入 T7，除非显式允许 partial，并且后续 claim 必须降级处理。
...
- T8 写作已经拆成 `T8-STYLE-GATE -> T8-RESOURCE -> T8-WRITE -> T8-SECTION-PLAN -> T8-SEC-* -> T8-DRAFT -> T8-SELF-CHECK -> review/revise -> T8-PAPER-CLAIM-AUDIT`，会先通过 runtime immediate gate 确认 IS/CCF-A/both 写作风格、写作语言和 LaTeX 模板 family/id（同样支持 `basic_zh`、`basic_en`、`ccf`、`utd`、`other`；CCF 默认 NeurIPS，也可选 ICLR/ICML/KDD，UTD 默认 INFORMS），再生成资源索引、证据计划、图表计划、`paper_state.json` 和每章局部大纲，再用一个节点只写一个 section 的方式逐章生成正文，最后拼装审计；Limitations 已并入 Conclusion。T8 的 `manuscript_resource_index.json` 和 `paper_state.json` 会暴露 `citation_ref_by_note_id`，把 T3.5 的 `[note:<id>]` provenance 映射到正式 `\cite{bibkey}`；未映射 note 只能作为补资源/上下文提示，不能原样进入 TeX。T8 的每章大纲也包含 `Section Writing Contract`，规定 Abstract/Introduction/Related Work/Method/Experiments/Analysis/Conclusion 各自的 purpose、required_content、internal_shape、evidence_rules 和 avoid，避免正文写成同质化模板或文献流水账。`assemble_manuscript` 会尽量应用本地模板：`basic_zh/basic_en`、`utd/informs`、`ccf/neurips`、`ccf/iclr`、`ccf/icml`、`ccf/kdd` 可直接装配；UTD/INFORMS 默认使用本地官方 INFORMS ISRE 2024 模板包（`informs4.cls`、`informs2014.bst` 和 logo/support files），后续若投 Management Science 等特定 INFORMS 期刊，可再替换为对应官方期刊包；缺少模板时回退到基础 article，并在 TeX 注释中记录 fallback。
- T8/T9 的实验 claim 必须经过 evidence pack 闭环：`drafts/experiment_evidence_pack.json` 归一化外部执行器证据，`drafts/result_to_claim.json` 把指标转成可写 claim，`drafts/paper_claim_audit.md/json` 在 T9 前检查正文数字、强 claim、mock-only evidence 和 forbidden wording。`paper_claim_audit.json` 必须带 `input_fingerprints`，绑定当前 `paper.tex`、evidence pack 和 result-to-claim hash；旧 audit 不允许放行新正文，审计失败回 `T8-REVISE-2` 定向修订。`craft_audit.json` 还会拦截 Abstract 正式引用、Abstract section heading、citation 与当前 claim 明显错配、正文/注释中的 `C1`/`CID` 内部标号、`TODO/TBD/PLACEHOLDER/LLM review required` 等规划残留；T9 对 `submission/bundle/main.tex` 再做一次同类脏标记扫描，防止模板迁移阶段把内部标号带进最终投稿包。
- CLI 人工输入现在会区分真实回答和无输入；预算扩限 gate 支持 `1/2`、`继续/停止`、`确认/stop` 等输入
- LLM profile / tier / fallback / retry
- human gate
- skill 发现与 `run-skill`
- MCP server 加载与工具注册
- 外部执行器 re-boost / handoff / 自动 skill 定制 / 材料确认 / mock dry-run / result ingest / evidence audit
- 本机 `latexmk` LaTeX 编译；Docker 仅作为可选 CLI 隔离和 legacy 高级工具
...
例如 T4 不只产出 `ideation/hypotheses.md`、`ideation/exp_plan.yaml` 和 `ideation/risks.md`，还会产出 `ideation/_gate1_candidate_cards.md`、`ideation/selected_idea_brief.md`、`ideation/idea_scorecard.yaml`、`ideation/rejected_ideas.md`、`ideation/gate_decisions.json` 和 `ideation/idea_rationales.json`。这些文件用于追踪每个 idea 从哪里来、解决什么 gap、和哪些工作最像、为什么不同、为什么被选中或淘汰，以及后续什么时候应该继续或停止。
...
| `docs/` | 文档索引、快速开始、流水线/runtime 参考和设计归档 |
| `deploy/` | 面向用户的 Docker Compose 部署目录、Docker 环境变量示例和启动脚本；项目数据仍放在顶层 `workspaces/` |
| `infra/docker/` | 维护镜像用的底层 Dockerfile、构建脚本和兼容运行脚本 |
| `scripts/` | 可提交的维护工具，例如 artifact 校验、模型探测和恢复脚本 |
| `tests/` | 自动化 pytest；`tests/unit/` 是确定性单测，`tests/real/` 可能需要密钥或本地工具，`tests/manual/` 是本地忽略的手动联调 |
| `workspaces/`、显式旧路径 `workspace/` | 生成的本地 workspace，不应提交 |

完整目录约定见 [docs/project_structure.md](docs/project_structure.md)。
...
`requirements.txt` 是本地使用的唯一 Python 依赖文件，已经包含运行时、LLM 路由、PDF/BibTeX 处理和 pytest 开发测试依赖。它不再默认安装本地实验训练栈。

如果要运行 T3.6 综述编译或 T9 投稿编译，需要在宿主机安装 TeX 与 `latexmk`。Ubuntu/Debian 示例：
...
Docker 是可选部署方式。普通 Docker Compose 运行请使用 `deploy/`；
`infra/docker/` 是维护镜像用的底层目录，不是另一套 ResearchOS。

可选。适合把 ResearchOS CLI 放进干净 Python 容器运行，避免污染宿主机 Python 环境。默认主链、外部执行器协议和 LaTeX 编译都不要求 Docker。
...
宿主机文件位于 `workspaces/local-test2`。完整说明见 [deploy/README.md](./deploy/README.md) 和 [docs/docker.md](./docs/docker.md)。
...
| 变量 | 作用 |
| --- | --- |
| `DEEPSEEK_API_KEY` | checked-in 默认 profile 使用的 DeepSeek endpoint |
| `DEEPSEEK_BASE_URL` | DeepSeek 兼容基地址 |
...
开发时如果想快速发现真实主链问题，用 `run_smoke`。它会运行真实状态机，默认从 `T2` 开始，但把候选池、精读目标和摘要轻读量压小，并把状态机节点模型层级临时降到 `medium`。
...
`run_smoke` 会在目标 workspace 写入 `literature/literature_params.json` 和 `literature/literature_params_confirmation.json`；已有参数文件默认不覆盖，需要重写时加 `--force-smoke-params`。这个模式用于真实快速联调，不代表最终文献覆盖质量。
...
如果 T2/T3/T3.5 产物已经合格，只是综述草稿质量不达标，不需要重跑检索和阅读。保留 `literature/` 与 `user_seeds/`，移走或删除 `drafts/survey/` 中旧的 survey 产物，把 `state.yaml` 的 `current_task` 设为 `T3.6-PLAN`、`status` 设为 `PAUSED`，然后执行：

```bash
python -m researchos.cli resume --workspace ./workspaces/local-test2
```

这样会从综述计划重新开始，复用已有文献笔记和 synthesis。若只是调试单节点，可用 `run-task T3.6-PLAN`、`T3.6-SEC-TAXONOMY`、`T3.6-ASSEMBLE` 等；完整综述支线仍建议用 `run/resume`，因为其中包含 outline/corpus gate 和状态机失败回退。
...
python -m researchos.cli run-task T3.6 --workspace ./workspaces/local-test2
...
python -m researchos.cli run-task T3.6-STATE --workspace ./workspaces/local-test2
python -m researchos.cli run-task T3.6-SEC-TAXONOMY --workspace ./workspaces/local-test2
python -m researchos.cli run-task T3.6-ASSEMBLE --workspace ./workspaces/local-test2
python -m researchos.cli run-task T3.6-COMPILE --workspace ./workspaces/local-test2
...
python -m researchos.cli run-task T7-INGEST --workspace ./workspaces/local-test2  # 必须已有 dry-run 或 T5-EXTERNAL-WAIT 验收结果
python -m researchos.cli run-task T7-AUDIT --workspace ./workspaces/local-test2
python -m researchos.cli run-task T7-POST-NOVELTY --workspace ./workspaces/local-test2
python -m researchos.cli run-task T7-CLAIMS --workspace ./workspaces/local-test2
...
python -m researchos.cli run-task T5-EXECUTOR-GATE --workspace ./workspaces/local-test2
python -m researchos.cli run-task T5-DRY-RUN --workspace ./workspaces/local-test2
python -m researchos.cli run-task T7-INGEST --workspace ./workspaces/local-test2
python -m researchos.cli run-task T7-AUDIT --workspace ./workspaces/local-test2
python -m researchos.cli run-task T7-POST-NOVELTY --workspace ./workspaces/local-test2
python -m researchos.cli run-task T7-CLAIMS --workspace ./workspaces/local-test2
```

真实实验时，`T5-REBOOST-GATE` 会直接调用当前配置的 LLM provider，对 Pre-T5 材料做 context re-boost，并写出 `external_executor/handoff_pack.json#context_reboost` 与 `external_executor/reboost_report.json`；不再需要用户手动启动 Codex CLI 做 re-boost。`T5-HANDOFF` 会复制 13 个外部执行器模板 skill 到 `external_executor/skills/`，`T5-SKILL-CUSTOMIZATION-GATE` 随后继续调用 LLM provider 自动专属化这些副本并写出 `external_executor/skills/customization_report.json`；不再需要手动拉起 Codex 做 skill 定制。之后仍会进入 `external_executor/expr/` 材料放置 gate。`T5-EXECUTOR-GATE` 选择 `codex_cli` / `claude_code_window` / `manual` 后会进入 `T5-EXTERNAL-WAIT` 并暂停。外部 Codex/Claude/manual executor 读取 `external_executor/AGENTS.md`、项目定制 skills、`codex_prompt.md`、`claude_code_prompt.md` 或 `manual_instructions.md`，按 `expected_outputs_schema.json` 写 `external_executor/result_pack.json`、`executor_status.json`、`run_manifest.json` 等文件；然后执行 `researchos resume --workspace ...`，验收通过后才进入 `T7-INGEST`。
...
这里 `run --from` 会复制 `T2` 的前置输入，不复制旧 T2 检索产物。若省略 `--start-task`，默认从 `T2` 开始。开发联调时如果想从 T2 开始跑真实主链但减少候选数和阅读量，优先用 `run_smoke --from`。
...
pytest -q \
  ...
  tests/unit/debug_novelty_auditor_agent.py \
...
- T3.6 会复用 `drafts/survey/survey_plan.json`、`survey_state.json`、`section_outlines/`、`sections/*.tex`、`survey_audit.json`、`survey_review_actions.json` 和 `survey_compile_report.json`，但只在 input fingerprint、TeX/PDF/log hash 和 dependency fingerprint 仍匹配当前文件时复用；中断后会按 section 继续，不需要重写整个 survey。默认 compact 结构会让 `theme_1` 到 `theme_4` skipped；只有 `survey_plan.sectioning_policy.mode=standalone_theme_sections` 且通过用户确认时才写少量独立主题章。旧 state/outline 如果缺少 `Section Writing Contract` 或 compact coverage contract，会要求重建 `T3.6-STATE`，避免沿用旧的弱 outline。
...
- 外部实验链在进入摄取前检查 result pack 和材料完整性；T9 在进入 LLM 前检查本机 `latexmk` 或 `tectonic`，环境缺失时会暂停等待修复而不是消耗 LLM 步数
...
- T2 文献参数与检索覆盖确认
- T3.6 综述模板、outline、corpus 与完成后去向确认
- T4 Gate1 idea 选择/合并/重新分析
- T4.5 novelty 非通过时的人工审阅
- T5 材料放置与执行器选择；skill 定制已自动化
- T7.5 的 PI 评估后分流
- T8 写作风格与模板确认
- 提交前 / 最终决策类阶段
...
- T3 `fetch_paper_pdf` 会从本地 literature metadata、OpenAlex OA locations、Unpaywall、arXiv 和 DOI fallback 解析候选 PDF；OpenAlex DOI 查询使用 `works/doi:...` endpoint，避免 DOI 斜杠被当成路径。T1/T2 会修复旧中文 PDF seed 把期刊页眉/期号误当论文题名的问题：例如 `《管理世界》（月刊）` 会优先从 PDF 文件名恢复为真实题名，并带 `title_repair_reason` 供审计。`[FULL-TEXT]` 校验只认 `extract_pdf_text` 完整页码覆盖和最终无截断，例如 `1-4, 5-8, 9-10 / 10`；`all pages` 这类无总页数证明的泛化描述不会通过。
- T3 deep read 后会运行 Reader LLM abstract sweep：默认从保留候选中的 shallow 候选写入 `literature/paper_notes_abstract/`，纯 metadata-only 候选批量写入 `literature/metadata_triage.md` 作为补资源/升级阅读线索；综述均衡默认摘要轻读最多 120 个候选，强覆盖默认 180 个，`papers_backlog.jsonl` 只在数值预算仍有剩余时回捞有摘要/PDF 的候选补足可读覆盖。`save_paper_note` 会在控制台输出 deep-read `x/y`，abstract sweep 会按配置打印候选处理进度。确定性模板只作为失败兜底
...
- T4 Gate1 已拆成状态机级 `T4-GATE1` immediate gate；Gate1 主展示是 `ideation/_gate1_candidate_cards.md`，`_candidate_directions.json` 只作为机器附录。Gate2/计划确认仍在 T4 agent 内通过 artifact 与人类反馈记录完成。
...
- 中文管理学文献目前依赖 OpenAlex/Crossref/Semantic Scholar/INFORMS/Scopus 可用覆盖和用户 seed；系统没有内置 CNKI/万方官方 API，不能伪造中文论文 metadata。英文稿且用户明确排除中文文献时，非 seed 中文论文不会进入 active pool；中文/双语或显式允许中文文献时，T2 只做 `authority_review_needed` 风险标记，不把来源不明等同于不可读证据，后续引用仍需结合真实期刊目录、全文和人工判断。法规、标准和治理框架（如 EU AI Act、NIST AI RMF、ISO/IEC 42001/23894、中国算法治理规定）会进入 `seed_external_resources.jsonl` 作为 official-source verification 线索，不进入 `papers_dedup.jsonl`，正文引用前必须由官方来源或已检索文献支撑。
...
如果 LaTeX 编译、Docker 镜像或 CLI 报 `ModuleNotFoundError: No module named 'researchos'`，同样按上面两种方式修复当前源码路径或本地 editable 安装。不要执行 `pip install researchos`；那不是本项目的环境修复方式。
...
3. `workspaces/<name>/_runtime/logs/researchos-debug.log`
4. `workspaces/<name>/_runtime/traces/*.jsonl`
5. 对应 task 产物目录
