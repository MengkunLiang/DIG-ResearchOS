...
- T2 前会先进入 `T2-PARAM-GATE`，显示当前检测到的任务类型、推荐档位、各档实际参数和自定义输入说明，再确认“保留候选数、精读目标、摘要轻读目标、写作语言/中文文献策略”，并写入 `literature/literature_params.json`；直接回车采用当前推荐项，也可直接输入“英文稿，不要中文论文，候选数300”。这不是网络请求 cap：对保留候选会尽可能补 DOI、摘要、OpenAlex/Crossref/Semantic Scholar/arXiv 和 PDF hints；内部 safety cap 只防异常失控。英文稿默认不主动检索中文论文，也不让非 seed 中文论文进入 active pool；中文/双语或显式允许中文时，只接受 WJCI、SCI/SSCI、EI、北大核心、CSSCI、CSCD、AMI 顶级/权威/核心等显式权威来源或用户 seed，普通普刊宁缺毋滥。T2 正常路径由检索工具返回值触发 runtime 自动保存 raw。Scout 负责设计 query 和 LLM `semantic_screen`，`expand_queries` 只做机械合并/去重，不内置领域知识。runtime 收尾会先把全量 `papers_raw.jsonl` 去重成审计池，对排序后的前置候选做 bounded light backfill，再按 `literature/literature_params.json` 或默认配置中的 `active_pool_max` 选出保留候选写入 `papers_dedup.jsonl` / `papers_verified.jsonl`，超额候选写入 `papers_backlog.jsonl`。`must_explore` 与 `should_explore` bridge 有不同保留上限；同一 bridge 即使命中 semantic screen、citation snowball 或 metadata fill，也不能绕过 cap，`no_cross/skip` 不强制进入 T3。每篇保留候选都必须有 deep_read、shallow_read 或 backlog/deferred 去向，不能因为缺少 `semantic_screen` 被静默丢弃。T2 收尾会写 `literature/t2_finalize_manifest.json`，把 `project.yaml`、seed 文件/PDF、`bridge_domain_plan.json`、PDF 目录和配置 hash 绑定；这些输入或 coverage 参数变更后旧 T2 产物不能被 resume 误复用。
...
- T3 deep read 后会运行 Reader LLM abstract sweep：默认从保留候选中的 shallow 候选写入 `literature/paper_notes_abstract/`，纯 metadata-only 候选批量写入 `literature/metadata_triage.md` 作为补资源/升级阅读线索；综述 gate 可设置 `all_readable` 并允许从 `papers_backlog.jsonl` 回捞有摘要/PDF 的候选补足可读覆盖。`save_paper_note` 会在控制台输出 deep-read `x/y`，abstract sweep 会按配置打印候选处理进度。确定性模板只作为失败兜底
...
- 中文管理学文献目前依赖 OpenAlex/Crossref/Semantic Scholar/INFORMS/Scopus 可用覆盖和用户 seed；系统没有内置 CNKI/万方官方 API，不能伪造中文论文 metadata。英文稿默认不搜索/引用中文论文；如用户明确要中文文献，系统只允许权威来源线索（WJCI、SCI/SSCI、EI、北大核心、CSSCI、CSCD、AMI 顶级/权威/核心等）或用户 seed 进入 active pool。法规、标准和治理框架（如 EU AI Act、NIST AI RMF、ISO/IEC 42001/23894、中国算法治理规定）会进入 `seed_external_resources.jsonl` 作为 official-source verification 线索，不进入 `papers_dedup.jsonl`，正文引用前必须由官方来源或已检索文献支撑。
