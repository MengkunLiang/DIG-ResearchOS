...
- T3 论文阅读以当前队列的 `queue_rank` 为工作单位：`lookup_paper_record(queue_rank=...)` 取单篇 metadata，`save_paper_note(queue_rank=..., content=...)` 自动生成 note 路径、即时校验并刷新 `literature/notes_manifest.json`。`require_deep_read_target=true` 时必须读满 `deep_read_target` 才能进入 T3.5；只有明确配置为 false 才允许达到 `deep_read_min` 后放行。resume 时 `deep_read_queue_pending.jsonl` 会把当前待读项重排为 1..N，同时保留 `original_queue_rank` 指回完整 `deep_read_queue.jsonl`，避免 pending rank 和原始 rank 混淆导致重复阅读。`notes_manifest.json` 同样绑定 T2 队列、verified/dedup、domain_map、seed profile/PDF、`literature/pdfs/` 和配置 hash；换 seed、补 PDF 或改 deep-read 参数后，旧 note 不能直接让 T3 跳过。命中 `user_seeds/pdfs/` 的 seed 会进入 queue/access audit 时标记 `evidence_level=FULL_TEXT` 与 `access_level_hint=FULL_TEXT_LOCAL`，不会再显示成 metadata-only。每篇 `paper_notes/*.md` 都必须记录 `## 12. Reading Coverage`，PDF 可用时必须覆盖到最后一页，只有完整页码覆盖且最终无截断时才能标记 `[FULL-TEXT]`
