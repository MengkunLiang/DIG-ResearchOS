...
因为 runtime 已经为 T1-T9 预留了 task I/O 契约，但当前真正注册到 [researchos/agents/registry.py](./researchos/agents/registry.py) 的正式 task 只有 `HELLO`、`T1`、`T7.5`、`T2`。T3-T9 还在开发中。
...
- 当前已实现的 agent：`HelloAgent`、`PIAgent`（T1/T7.5）、`ScoutAgent`（T2），T3-T9 正式 agent 还在开发中。
