...
| [config/agent_params.yaml](./config/agent_params.yaml) | Agent 参数配置（model_tier、budget、timeout） | 参考用 |
| [config/mcp.example.yaml](./config/mcp.example.yaml) | MCP server 配置模板 | 模板文件 |

### Agent 参数配置

[config/agent_params.yaml](./config/agent_params.yaml) 集中管理所有 Agent 的参数：

- **模型选择**：model_tier (heavy/medium/light)
- **预算限制**：max_steps、timeout、budget_hint
- **输出期望**：expected_outputs、expected_sections
- **Docker/GPU 要求**：docker_required、gpu_required
- **重试策略**：retry_policy

详细配置见 [config/agent_params.yaml](./config/agent_params.yaml)。
...
| Agent 参数（model_tier、budget、timeout） | [config/agent_params.yaml](./config/agent_params.yaml) |
