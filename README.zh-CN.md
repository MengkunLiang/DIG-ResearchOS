...
`resume --from-task` 不会合并历史记录，也不要求你修改 `state.yaml`。`T3.6` 是可选综述决策，`T8` 是写作入口。普通 `resume` 回到 T2/T3 时会先展示“继续当前范围或修改”的轻量 Gate；确认继续不发起新检索。显式重开 T2 会打开完整参数选择，避免新的检索边界静默沿用旧范围。显式重开 T3 会先展示检索覆盖决策；只要阅读队列还在，缺少旧 `search_log.md` 等摘要不会跳过该选择。
...
同一 workspace 的明确重入使用 `resume --from-task <task>`。该命令会先校验目标 task 的前置 artifact，再清除旧 pending gate，并在 `state.yaml` 中记录重入原因；不需要、也不允许用户手工修改状态文件。T2/T3 是面向研究者的例外：显式重入或 `--from` 迁移时会优先打开参数或覆盖决策，覆盖 Gate 允许用保存的阅读队列检查并修复历史摘要缺失。T3.6 已完成但需要直接进入 Idea 时使用 `--from-task T4`。若 T4 前置材料不完整，命令会在启动模型前退出并说明缺项，不会用“跳过”掩盖缺失证据。
