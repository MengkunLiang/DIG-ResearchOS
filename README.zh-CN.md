...
- [config/state_machine.yaml](./config/state_machine.yaml) 已定义完整的 T1-T9 工作流。

### 鲁棒性增强功能

根据 `ResearchOS_Agent_Dev_Spec_Addendum_Robustness.md`，已实现以下4个必做鲁棒性增强项：

1. **T4 Hypothesis Pre-mortem（§4.1）**：在Gate1和Gate2之间添加反常识验证
   - 对选定的研究方向执行三维检查：物理/数学约束、已知反例、资源可行性
   - 如果发现High风险且无缓解方案，提示用户重新选择方向
   - 实现位置：`researchos/prompts/ideation.j2`（阶段A.5）

2. **Runtime Budget Drift Warning（§7.1）**：预算漂移预警
   - 在每个task完成后检查累计花费
   - 超过预算70%时记录警告日志
   - 超过预算90%时记录严重警告并写入警告文件
   - 实现位置：`researchos/orchestration/state_machine.py`（`_check_budget_drift`方法）

3. **T1 Ethical Screening（§8.1）**：敏感方向拦截
   - 在T1的`validate_outputs`中检查敏感词
   - 检测武器、监控、操纵、隐私侵犯、歧视等敏感领域
   - 如果检测到敏感词，返回警告并要求用户确认
   - 实现位置：`researchos/agents/pi.py`（`_check_ethical_concerns`方法）

4. **T1 External Resources Management（§10.1-10.2）**：外部资源管理
   - 在T1的三轮对话中询问用户已有的外部资源
   - 支持7种资源类型：dataset、baseline_repo、pretrained_model、docker_image、tool、script、other
   - 生成`user_seeds/seed_external_resources.jsonl`文件

5. **迭代死锁检测（Phase 2.3）**：防止无限循环
   - 检测任务在相同参数下的重复执行
   - 当同一参数组合尝试3次以上时，自动触发错误并快速失败
   - 记录每次迭代的参数哈希、时间戳和参数内容到 `state.yaml`
   - 实现位置：`researchos/orchestration/state_machine.py`（`_check_iteration_deadlock`方法）
   - 验证资源格式和source前缀
   - 实现位置：`researchos/prompts/pi.j2`（第2.5轮对话）和`researchos/agents/pi.py`（`_validate_external_resources`方法）
...
ResearchOS 已实现以下鲁棒性增强功能（基于 `ResearchOS_Agent_Dev_Spec_Addendum_Robustness.md`）：

### 1. T1 Ethical Screening（§8.1）

在 T1 项目初始化阶段，自动检测研究方向是否涉及敏感领域：

- 检测范围：武器、监控、操纵、隐私侵犯、歧视等
- 实现位置：`researchos/agents/pi.py` 的 `_check_ethical_concerns` 方法
- 触发时机：T1 输出校验阶段
- 如果检测到敏感词，会返回警告信息，要求用户确认研究目的符合伦理规范

### 2. T1 外部资源管理（§10.1-10.2）

支持用户在 T1 阶段提供已有的外部资源，避免重复工作：
...
- 资源格式：每行一条 JSON，包含 type、name、source、purpose 等字段
- source 支持：huggingface、github、docker、pip、url、local 等多种来源
- 实现位置：
  - `researchos/prompts/pi.j2`：在第2.5轮对话中询问外部资源
  - `researchos/agents/pi.py`：`_validate_external_resources` 方法验证格式

### 3. T4 Hypothesis Pre-mortem（§4.1）

在 T4 假设生成阶段，对选定的研究方向执行反常识验证：

- 检查维度：
  1. 物理/数学约束检查：是否违反已知定律或计算复杂度下界
  2. 已知反例检查：文献中是否已有反例证明不可行
  3. 资源可行性检查：在给定预算和时间内是否可验证
- 实现位置：`researchos/prompts/ideation.j2` 的阶段A.5
- 触发时机：Gate1 用户选定方向后、展开假设之前
- 输出文件：`ideation/_premortem.md`
- 如果发现高风险且无缓解方案，会提示用户重新选择方向

### 4. Runtime Budget Drift Warning（§7.1）

在每个任务完成后，检查累计花费是否超出预算：

- 警告阈值：
  - 70%：记录警告日志
  - 90%：记录严重警告日志，并写入 `<workspace>/.researchos/budget_warning.txt`
- 实现位置：`researchos/orchestration/state_machine.py` 的 `_check_budget_drift` 方法
- 触发时机：每次 `advance` 方法更新累计预算后
- 预算来源：`project.yaml` 的 `constraints.max_budget_usd` 字段
...
测试覆盖：
- Ethical screening：检测敏感词、正常研究通过
- 外部资源管理：合法格式、非法类型、非法source、缺少字段、空文件、非法JSON
- Budget drift warning：70%警告、90%严重警告、低于阈值不警告
