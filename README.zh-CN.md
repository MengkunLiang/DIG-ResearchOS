...
T1 研究范围 -> T2 文献 -> T3 阅读 -> T3.5 综合
  -> 可选 T3.6 综述 -> T4 Idea -> T4.5 研究方案审计与正式化
...
T3.6 完成后，结果页会优先展示 `drafts/survey/survey.pdf`，随后列出可编辑 TeX、覆盖审计和可选的 T4 交接材料。交接材料保留综述的 taxonomy 与有证据边界的挑战，但不会迫使后续 Idea 机械重复或只沿用综述结论。写作某一节时，系统会先复用现有语料；只有发现会改变该节分类、发展脉络或代表性覆盖的具体缺口时，才允许自主补检。每节最多两轮、每轮最多 10 篇、两轮累计最多 20 篇经核验候选；第二轮必须针对读取第一轮笔记后仍存在的不同缺口，重复查询优先复用缓存。
