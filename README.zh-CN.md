...
- T3.6 是可选综述论文支线：T3.5 后通过状态机级 immediate gate 问“是否撰写综述论文”，选择 yes 后按 taxonomy 规划、人工确认、逐 section 写作、拼装审阅、LaTeX 编译和导出 `survey_insights.json` 的方式执行；它不是把 `synthesis.md` 转成 TeX。综述质量标准来自通用综述写作原则，而不是当前题目模板：PLAN 必须写出 `writing_language`、`central_question`、`scope_boundaries`、`review_contribution`、`quality_plan`，并要求每个核心 section 有 `reader_question/section_argument`。默认写成紧凑专业综述，taxonomy 类放在 `Taxonomy` 和 `Comparative Analysis` 内部，而不是为每个类拆出独立 theme 大章；`survey_plan.sectioning_policy` 会在 PLAN 阶段被校验，缺失或 compact 模式下仍输出 `theme_*` 会被拒绝。`T3.6-SEC-THEME-*` 只是兼容/显式长综述槽位，默认 skipped；skipped 只表示不写独立大章，不表示删除内容，`compact_theme_content_absorbed` 审计会要求每个 taxonomy class 已被 Taxonomy 和 Comparative Analysis 吸收。`build_survey_state` 会把每节不同的 `Section Writing Contract` 写入 `section_outlines/*.md`，规定本节 purpose、required_content、internal_shape、evidence_rules 和 avoid，防止 Abstract/Introduction/Background/Taxonomy/Comparison/Challenges/Future/Conclusion 用同一套短模板。SECTION/ASSEMBLE 会拦截中英混写、篇幅过短、缺少本节标题、文献串烧、空泛 future/gap、placeholder、内部 `C1/CID/ResearchOS alignment` 标号和未知 BibTeX key；REVIEW 还必须检查 review contribution 与语言/深度，不能把语言混杂降级为低风险。`decision.json` 或 `corpus_decision.json` 缺失/损坏时会回到对应 gate；上游 synthesis/corpus 变更后旧 gate 决策也会失效。每个 section、`survey_state.json`、`survey_audit.json`、`survey_review_actions.json` 和 `survey_compile_report.json` 都有 input fingerprint 或 TeX/PDF/log/dependency hash 绑定；Abstract 源文件只能写纯正文，禁止正式引用、`\section{Abstract}` 和 `\begin{abstract}`，旧 PDF/log 不能在 TeX 或 references 变化后放行。`survey_insights.json` 会合并 `survey_plan` 与 `survey_state.shared_facts` 中的 `resource_upgrade_needs`，把 taxonomy/challenge/future hints 传给 T4。T4 可以生成 `survey_driven` 候选，但 weak-only 候选必须标 `not_supported_by_current_evidence`，不能被选中或绑定最终 hypothesis。
