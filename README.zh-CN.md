...
- T2 前会先进入 `T2-PARAM-GATE`，用人类可读语义确认“保留候选数、精读目标、摘要轻读目标”，并写入 `literature/literature_params.json`。这不是网络请求 cap：对保留候选会尽可能补 DOI、摘要、OpenAlex/Crossref/Semantic Scholar/arXiv 和 PDF hints；内部 safety cap 只防异常失控。T2 正常路径由检索工具返回值触发 runtime 自动保存 raw。Scout 负责设计 query 和 LLM `semantic_screen`，`expand_queries` 只做机械合并/去重，不内置领域知识。runtime 收尾会先把全量 `papers_raw.jsonl` 去重成审计池，对排序后的前置候选做 bounded light backfill，再按 `literature/literature_params.json` 或默认配置中的 `active_pool_max` 选出保留候选写入 `papers_dedup.jsonl` / `papers_verified.jsonl`，超额候选写入 `papers_backlog.jsonl`。`must_explore` 与 `should_explore` bridge 有不同保留上限；同一 bridge 即使命中 semantic screen、citation snowball 或 metadata fill，也不能绕过 cap，`no_cross/skip` 不强制进入 T3。每篇保留候选都必须有 deep_read、shallow_read 或 backlog/deferred 去向，不能因为缺少 `semantic_screen` 被静默丢弃。T2 收尾会写 `literature/t2_finalize_manifest.json`，把 `project.yaml`、seed 文件/PDF、`bridge_domain_plan.json`、PDF 目录和配置 hash 绑定；这些输入或 coverage 参数变更后旧 T2 产物不能被 resume 误复用。
