...
Treat the following as editorial writing preferences. Factual accuracy, verified evidence, and an explicit venue template take precedence where necessary.

- Write in the clean, accurate, and economical style expected of strong UTD/FT50/CCF-A research. Avoid empty importance claims, stock language, repeated statements, and artificial transitions that add no information.
- Ordinary body prose must not depend on dashes, colons, or label-style sentences such as `Problem:`, `Gap:`, `Insight:`, or `Implication:`. Use full sentences to explain causal logic, comparison, qualification, and consequence. Definitions, equations, tables, contribution lists, and venue templates may use limited punctuation when it improves clarity.
- Do not make the argument feel fragmented by chaining parallel short sentences. Give each paragraph a single function and move from its claim through mechanism, evidence, boundary, or implication in a natural order. New terms, citations, results, and practical implications require a bridge from the preceding discussion.
- Keep the structure compact. Do not introduce small subsections or `\paragraph{}` headings for individual papers, artifacts, or claims. Explain terms when they first matter, and use a real or conditional `such as` example only when it makes an abstract mechanism easier to understand without inventing evidence.
- Before retaining or adding a citation, verify that it exists and that its subject, method, setting, and evidence level support the exact claim in the sentence. Prefer genuinely relevant UTD/FT50/CCF-A work, leading marketing, economics, management science, information systems, and computer-science venues, plus foundational, highly cited, influential, and important recent papers. Prestige never compensates for a semantic mismatch.
