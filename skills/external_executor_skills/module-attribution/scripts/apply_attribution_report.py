#!/usr/bin/env python3
"""Atomically apply a validated phase report to its owned result-pack section for the module attribution Skill.

The ``apply attribution report`` entrypoint is deterministic: it reports contract failures explicitly and never fills missing research facts, authorization, or execution evidence by inference.
"""
...
def main() -> int:
    ...
    snapshot = load_json(ws / "external_executor/report/phase_E/module_attribution_snapshot.json")
    facts = load_json(ws / "external_executor/report/phase_E/module_attribution_facts.json")
