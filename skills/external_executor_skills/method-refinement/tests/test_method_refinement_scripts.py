...
class MethodRefinementScriptTests(unittest.TestCase):
    ...
    def test_initial_refinement_passes_and_applies_narrowly(self) -> None:
        ...
        review = json.loads((ws / "external_executor/method_refinement_review.json").read_text())
        self.assertEqual(review["review_status"], "pass", review)
        self.assertEqual(review["approved_for"], "implementation")
        report = json.loads((ws / "external_executor/method_refinement_report.json").read_text())
    ...
    def test_major_core_mechanism_drift_is_blocked(self) -> None:
        ...
        scope = json.loads((ws / "external_executor/method_scope_assessment.json").read_text())
        review = json.loads((ws / "external_executor/method_refinement_review.json").read_text())
        ...
        request = json.loads((ws / "external_executor/scope_change_request.json").read_text())
    ...
    def test_minor_engineering_refinement_creates_version_two(self) -> None:
        ws = self.clone_workspace()
        first = json.loads((ws / "external_executor/method_spec_fingerprint.json").read_text())
        ...
        delta = json.loads((ws / "external_executor/method_delta.json").read_text())
        ...
        self.assertEqual(review["review_status"], "pass", review)
        second = json.loads((ws / "external_executor/method_spec_fingerprint.json").read_text())
