#!/usr/bin/env python3
"""Validate schema, lineage, and decision-boundary requirements for the method refinement Skill.

The ``validate method implementation spec`` entrypoint is deterministic: it reports contract failures explicitly and never fills missing research facts, authorization, or execution evidence by inference.
"""
...
def main() -> int:
    ...
    parser.add_argument("--output", default="external_executor/report/phase_D/method_implementation_spec_validation.json")
    args = parser.parse_args()
    ws = resolve_workspace(args.workspace)
    ext = ws / "external_executor"
    spec = load_json(resolve_in_workspace(ws, args.spec))
    intent = load_json(ext / "report" / "phase_D" / "method_intent_contract.json")
