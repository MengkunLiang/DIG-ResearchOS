...
class ResourceSkillScriptTests(unittest.TestCase):
    def make_workspace(self) -> Path:
        ...
        (ext / "expr").mkdir()
        (ext / "expr" / "baseline_a").mkdir()
        (ext / "expr" / "baseline_a" / "README.md").write_text("baseline\n", encoding="utf-8")
        (ext / "expr" / "baseline_a" / "LICENSE").write_text("MIT\n", encoding="utf-8")
        return root
    ...
    def test_local_inventory(self) -> None:
        ...
        inventory = json.loads((ws / "external_executor/resource_local_inventory.json").read_text())
    ...
    def test_initialize_report_preserves_sections(self) -> None:
        ...
        path = ws / "external_executor/resource_preparation_report.json"
    ...
    def test_report_validation_and_apply(self) -> None:
        ...
        report_path = ws / "external_executor/resource_preparation_report.json"
    ...
    def test_static_review_blocks_destructive_pattern(self) -> None:
        ...
        proc = run("static_review_repository.py", "--workspace", str(root), "--path", str(root), "--output", str(output), check=False)
