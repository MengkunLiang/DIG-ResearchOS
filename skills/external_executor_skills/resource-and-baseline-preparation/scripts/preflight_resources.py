#!/usr/bin/env python3
"""Validate prerequisites before the executor phase can mutate artifacts for the resource and baseline preparation Skill.

The ``preflight resources`` entrypoint is deterministic: it reports contract failures explicitly and never fills missing research facts, authorization, or execution evidence by inference.
"""
...
def default_resource_acquisition_policy() -> dict:
    return {
        ...
        "material_absence_policy": (
            "Check resources/ for local project resource materials first; continue by authorized acquisition "
            "or baseline reimplementation into resources/ when local materials are absent."
        ),
    }
