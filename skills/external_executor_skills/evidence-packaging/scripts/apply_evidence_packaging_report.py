#!/usr/bin/env python3
"""Atomically apply a validated phase report to its owned result-pack section for the evidence packaging Skill.

The ``apply evidence packaging report`` entrypoint is deterministic: it reports contract failures explicitly and never fills missing research facts, authorization, or execution evidence by inference.
"""
...
def main() -> int:
    ...
    result["evidence_packaging"] = {
        ...
        "result_tables": report["result_tables"],
        "result_figures": report["result_figures"],
