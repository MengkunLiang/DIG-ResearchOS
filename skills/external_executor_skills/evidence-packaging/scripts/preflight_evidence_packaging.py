...
def main() -> int:
    ...
    parser.add_argument("--output", default="external_executor/evidence_packaging_preflight.json")
    ...
    required_files = {
        ...
        "run_manifest": ext / "run_manifest.json",
    }
    ...
    executor_state = str(status.get("status") or "unknown").lower()
    ...
    if not any(nonempty(result.get(key)) for key in ("implementation_spec", "method_specification", "realized_method", "implementation_records")):
        warnings.append("implementation_definition_missing:realized_method_may_be_unavailable")

    write_targets = [
        output,
        ext / "final_evidence_snapshot.json",
        ext / "evidence_package" / "realized_method_package.json",
        ext / "evidence_package" / "framework_figure_spec.json",
        ext / "evidence_package" / "framework_figure.svg",
        ext / "evidence_package" / "framework_figure.mmd",
        ext / "evidence_package" / "figure_table_inventory.json",
        ext / "evidence_package" / "evidence_mapping.json",
        ext / "evidence_packaging_report.json",
    ]
