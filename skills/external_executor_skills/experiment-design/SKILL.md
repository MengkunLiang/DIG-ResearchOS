...
- `external_executor/protocol_change_impact.json` when comparing versions;
...
python <skill-dir>/scripts/preflight_experiment_design.py --workspace <workspace> \
  --output external_executor/experiment_design_preflight.json
```
...
python <skill-dir>/scripts/build_claim_evidence_matrix.py --workspace <workspace> \
  --output external_executor/claim_evidence_matrix.json
```
...
python <skill-dir>/scripts/build_protocol_snapshot.py --workspace <workspace> \
  --output external_executor/protocol_snapshot.json
```
...
python <skill-dir>/scripts/fingerprint_protocol.py --workspace <workspace> \
  --protocol external_executor/protocol_snapshot.json \
  --output external_executor/protocol_fingerprint.json --write-back
```
...
python <skill-dir>/scripts/compare_protocol_versions.py --workspace <workspace> \
  --old <old-protocol.json> --new external_executor/protocol_snapshot.json \
  --plan external_executor/experiment_plan.json \
  --output external_executor/protocol_change_impact.json
```
...
python <skill-dir>/scripts/build_experiment_plan.py --workspace <workspace> \
  --claims external_executor/claim_evidence_matrix.json \
  --protocol external_executor/protocol_snapshot.json \
  --fingerprint external_executor/protocol_fingerprint.json \
...
An ablation must map to a named mechanism and a controlled intervention. Removal alone is not sufficient when replacement, neutralization, capacity matching, or a controlled diagnostic better tests the mechanism.
...
python <skill-dir>/scripts/validate_plan_dag.py --workspace <workspace> \
  --plan external_executor/experiment_plan.json \
  --output external_executor/experiment_plan_dag_validation.json
```
...
python <skill-dir>/scripts/validate_experiment_plan.py --workspace <workspace> \
  --plan external_executor/experiment_plan.json \
  --output external_executor/experiment_plan_validation.json
```
...
python <skill-dir>/scripts/compute_design_gate.py --workspace <workspace> \
  --plan external_executor/experiment_plan.json \
  --plan-validation external_executor/experiment_plan_validation.json \
  --dag-validation external_executor/experiment_plan_dag_validation.json \
  --output external_executor/experiment_design_gate.json --write-back
```
...
python <skill-dir>/scripts/assemble_experiment_design_report.py --workspace <workspace> \
  --output external_executor/experiment_design_report.json

python <skill-dir>/scripts/validate_experiment_design_report.py --workspace <workspace> \
  --report external_executor/experiment_design_report.json
