#!/usr/bin/env python3
"""Assemble a durable phase report from validated artifacts for the experiment design Skill.

The ``assemble experiment design report`` entrypoint is deterministic: it reports contract failures explicitly and never fills missing research facts, authorization, or execution evidence by inference.
"""
...
def main() -> int:
    ...
    parser.add_argument("--output", default="external_executor/report/phase_C/experiment_design_report.json")
    args = parser.parse_args()
    ws = resolve_workspace(args.workspace)

    paths = {
        "preflight": "external_executor/report/phase_C/experiment_design_preflight.json",
        "claim_evidence_matrix": "external_executor/report/phase_C/claim_evidence_matrix.json",
        "protocol_snapshot": "external_executor/report/phase_C/protocol_snapshot.json",
        "protocol_fingerprint": "external_executor/report/phase_C/protocol_fingerprint.json",
        "experiment_plan": "external_executor/experiment_plan.json",
        "plan_validation": "external_executor/report/phase_C/experiment_plan_validation.json",
        "dag_validation": "external_executor/report/phase_C/experiment_plan_dag_validation.json",
        "design_gate": "external_executor/report/phase_C/experiment_design_gate.json",
    }
