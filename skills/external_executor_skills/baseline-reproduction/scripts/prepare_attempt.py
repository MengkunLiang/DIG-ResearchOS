#!/usr/bin/env python3
"""Prepare an isolated, policy-bound execution workspace for the baseline reproduction Skill.

The ``prepare attempt`` entrypoint is deterministic: it reports contract failures explicitly and never fills missing research facts, authorization, or execution evidence by inference.
"""
...
def main() -> int:
    ...
    if dest.exists() or result_dir.exists() or evidence_dir.exists():
        if not args.force:
            raise SystemExit(f"Attempt exists: {dest} or {result_dir} or {evidence_dir}")
        if dest.exists():
            shutil.rmtree(dest)
    ...
    dump_json_atomic(evidence_dir / "attempt_provenance.json", provenance)
