...
description: Compare two or more existing ResearchOS note cards or source records on a user-defined research question, method, evidence, limitation, and contribution axis. Use when a researcher needs a source-grounded comparison rather than a generic related-work summary.
tools:
  - read_file
  - list_files
  - grep_search
...
interaction:
  mode: guided
  language: zh-CN
  summary: 比较两篇或多篇已有笔记卡/论文记录，按问题、机制、方法、证据、局限和可组合性给出可追溯结论。
  ...
  required_inputs:
    - id: sources
      label: 待比较的论文路径清单
      description: 一行一个 workspace 相对路径，指向 paper note、skill PDF note card 或 identifier record；至少列出两项。
      paths:
...
Read the source list first and verify every listed workspace-relative file exists before
making a comparison. If a source is missing, malformed, or not a note/record, write a
focused follow-up request rather than replacing it with model knowledge. Extract only
what the actual sources support and retain each source path plus section/field anchors.
