...
- `workspaces/<name>/_runtime/logs/researchos.log`：统一人类时间线，由 runtime 写入，不受 quiet 影响
- `workspaces/<name>/_runtime/logs/researchos-debug.log`：底层 Python/structlog 调试日志
- `workspaces/<name>/_runtime/traces/*.jsonl`：机器级完整 trace
...
- T9/LaTeX 超时：检查 `runtime.timeouts.latex_compile`；普通 `max_tool_call` 不应承担长 TeX 编译上限。Docker 仅是 legacy/显式自定义执行路径，不是默认 T9 fallback。
