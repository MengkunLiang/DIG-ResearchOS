...
def test_t45_review_can_validate_before_runtime_publishes_post_novelty_receipt(tmp_path: Path) -> None:
    """The final receipt is written after this pre-publication check.

    Requiring it here creates a circular validator failure and prevents every
    otherwise valid T4.5 review from reaching ``write_post_novelty...``.
    """

    populate_valid_t45_workspace(tmp_path)
    (tmp_path / "ideation/post_novelty_formalization.json").unlink()
...
def test_t45_rejects_a_derived_validation_map_from_another_claim_registry(tmp_path: Path) -> None:
    populate_valid_t45_workspace(tmp_path)
    path = tmp_path / "ideation/validation_map.yaml"
    payload = yaml.safe_load(path.read_text(encoding="utf-8"))
    payload["claims"][0]["claim_id"] = "STALE-OTHER-TRACK"
    path.write_text(yaml.safe_dump(payload, allow_unicode=True, sort_keys=False), encoding="utf-8")
    ok, error = validate_t45_research_proposal(tmp_path, tmp_path / "ideation/novelty_audit.md")
    assert ok is False
    assert "validation_map claim IDs" in (error or "")
