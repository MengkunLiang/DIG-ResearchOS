...
from researchos.runtime.workspace import create_directory_guides, initialize_workspace
...
def test_directory_guides_explain_current_skill_suite_and_t8_boundary(tmp_path: Path) -> None:
    ...
    assert "2026-07-skill-suite-t5-t8-v3" in executor_guide
...
def test_directory_guide_refreshes_legacy_generated_paths_but_preserves_custom_guides(tmp_path: Path) -> None:
    initialize_workspace(tmp_path, create_project_file=False)
    legacy_guide = tmp_path / "literature/paper_notes/_DIR_GUIDE.md"
    custom_guide = tmp_path / "external_executor/expr/_DIR_GUIDE.md"
    _write(
        legacy_guide,
        "# Workspace Directory Guide\n\nGenerated by ResearchOS workspace initialization.\n",
    )
    _write(custom_guide, "# My Local Deployment Notes\n\nKeep this text unchanged.\n")

    create_directory_guides(tmp_path)

    assert "2026-07-skill-suite-t5-t8-v3" in legacy_guide.read_text(encoding="utf-8")
    assert custom_guide.read_text(encoding="utf-8") == "# My Local Deployment Notes\n\nKeep this text unchanged.\n"
