...
The same `config/model_settings.yaml` also contains `context_window_fallback: 262144`, `truncation.max_input_tokens: 160000`, and optional `rate_limit`, so connection and context settings live together. The fallback is not a raw prompt-input limit: it is the total token capacity ResearchOS assumes only when a provider cannot report the model's real context window, shared by prompts, research materials, history, Tool I/O, and response space. A provider-reported real capacity always takes priority. `max_input_tokens` retains a smaller, safe amount of conversation/Tool history per request without reducing PDF reading or stored evidence. Local `rate_limit` is off by default because provider account quotas, not context capacity, determine TPM/RPM. See [Configuration](docs/en/config.md) for the exact fields and fallback semantics.
