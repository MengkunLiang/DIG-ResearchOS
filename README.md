...
Each archived track must also pass a candidate-identity check. The `candidate_id` and `selection_fingerprint` in `selected_candidate.json`, the isolation receipt, the research dossier, the Proposal manifest, and the formalization receipt must agree. File presence and valid YAML/JSON syntax alone are not sufficient evidence of a trustworthy package. An older Gate may have persisted a 17-file table or omitted the shared `orientation_config.yaml`; on resume, ResearchOS uses that track's own receipt to restore the missing shared file and deterministically recompile derived dossier/maps, never borrowing research content from another track. If a dossier belongs to another Candidate, a fingerprint is stale, or a historical receipt cannot prove the source, the table marks the row as provenance-inconsistent, withholds its D1/D2 alias, and prevents T5 from consuming it. Re-run T4.5 for that Candidate instead of copying another track's files by hand.
