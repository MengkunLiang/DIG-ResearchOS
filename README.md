...
After T3.6 succeeds, the completion page leads with `drafts/survey/survey.pdf`, then shows the editable TeX, coverage audit, and the optional T4 handoff. The handoff preserves the Survey's taxonomy and evidence-bounded challenges without forcing later ideation to repeat or mechanically follow the Survey.
...
At the T4 Gate, Copilot can advance one Candidate or choose 2–3 Candidates to **write separate Proposals**. The latter never splices Ideas together: each Candidate completes its own T4.5 path and is archived under `ideation/proposal_portfolio/tracks/`. With one Proposal, the workflow goes straight to T5. With multiple accepted Proposals, a pre-T5 table assigns stable aliases such as `D1` and `D2`, and shows each Candidate's core problem, core mechanism, parent/child or shared-parent relationship, full Candidate ID, Proposal path, and structured-artifact completeness. A parent direction and its evolved child may still be formalized separately, but the child must make its added mechanism and discriminating test explicit; they are never treated as unrelated tracks or allowed to share one active package. Enter `D1`, `D2`, a full Candidate ID, or natural wording such as “the first/second Proposal” or “第一条/第二条” to choose one; bare `1` and `2` remain menu choices, while a number entered in the follow-up Candidate field resolves by table order. T5 and later writing then read only that Proposal, while the others remain preserved for later review or a separate experiment.

During parallel formalization, the files under the active `ideation/` projection (including the Proposal, hypotheses, blueprint, claims, experiment plan, validation/kill criteria, research dossier, and novelty fingerprint) represent only the track currently being processed. Before the next track starts, every candidate-bound artifact is copied into `ideation/proposal_portfolio/tracks/<Candidate>/artifacts/` and the active projection is cleared. Seeing only the last Proposal at the canonical root is therefore expected and does not mean the earlier Proposal was lost. The durable source of truth for both outputs is `ideation/proposal_portfolio/manifest.json` plus its per-Candidate `tracks/` directories. Selecting a track for T5 restores that track's complete candidate-bound package; if a required archive file is missing, the workflow returns to the same selection Gate instead of borrowing files from another track.

Each archived track must also pass a candidate-identity check. The `candidate_id` and `selection_fingerprint` in `selected_candidate.json`, the isolation receipt, the research dossier, the Proposal manifest, and the formalization receipt must agree; the manifest also records a Candidate anchor, Candidate fingerprint, and active-track context. File presence and valid YAML/JSON syntax alone are not sufficient evidence of a trustworthy package. An older Gate may have persisted a 17-file table or omitted the shared `orientation_config.yaml`; on resume, ResearchOS uses that track's own receipt to restore the missing shared file and deterministically recompile derived dossier/maps, never borrowing research content from another track. If a dossier belongs to another Candidate, a fingerprint is stale, the active projection points at the wrong track, or a historical receipt cannot prove the source, Formalizer stops before model invocation, the table marks the row as provenance-inconsistent, withholds its D1/D2 alias, and prevents T5 from consuming it. Exact duplicate or high-similarity research packages across tracks are also rejected. Re-run T4.5 for that Candidate instead of copying another track's files by hand.

The formalization receipt also records SHA-256 digests for candidate-bound files and the handoff checks that validation maps, contribution maps, the research dossier, experiment plan, and claim registry contain the same claim IDs. This catches cross-Proposal mixing even when each individual YAML or JSON file remains schema-valid.
