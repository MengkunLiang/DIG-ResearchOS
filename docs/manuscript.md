...
T8 要把前面阶段的 `synthesis.md`、paper notes、`comparison_table.csv`、`hypotheses.md`、`idea_scorecard.yaml`、`novelty_audit.md`、`results_summary.json`、`ablations.csv`、run logs、代码和图表转化为可审计 manuscript。单次 prompt 写作会浪费这些材料，也会造成上下文爆炸、数字漂移、术语不一致和后半段质量衰减。
...
旧入口 `T8` 和旧报告中的 `next_task: T8-WRITE` 会映射到 `T8-RESOURCE`。旧 `T8-SECTIONS` 只作为兼容入口，不再是主链正文写作节点。
...
`WriterAgent(resource_index)` 调用 `build_manuscript_resource_index` 扫描项目、文献、假设、新颖性审计、实验、图表、表格和代码资源，写 `drafts/manuscript_resource_index.json`。随后调用 `plan_manuscript_sections` 写 `drafts/section_plan.json`，再调用 `plan_manuscript_evidence` 写 `drafts/evidence_plan.json` 和 `drafts/figure_table_plan.json`。

Validator 会解析 JSON，而不是只看字符串长度；必须有 `artifacts`、核心 sections、`claim_slots`、`experiments_main_result`、`fig:main_results`、`tab:main_results`。
...
- `drafts/section_outlines/limitations.md`
- `drafts/section_outlines/conclusion.md`

`paper_state.json` 包含 `section_order`、每章状态、每章目标文件、`shared_facts.bib_keys`、`shared_facts.result_metrics`、claim slots 和 planned visuals。它是防止数字漂移和术语漂移的共享状态。
...
只写 `drafts/sections/limitations.tex`。它读取 risks、novelty audit、results 和 `paper_state.json`，说明 direct-full、跳过 T5/T6、baseline 覆盖、数据规模、外部有效性和复现性风险。

### T8-SEC-CONCLUSION

只写 `drafts/sections/conclusion.tex`。它读取 introduction、experiments、limitations，收束贡献与未来工作，不引入新 claim。

### T8-SEC-ABSTRACT

只写 `drafts/sections/abstract.tex`。它最后运行，读取主要章节，压缩 problem、method、evidence、result、contribution。Abstract 不能出现正文没有的数字、引用或 claim。

### T8-DRAFT

`WriterAgent(draft)` 调用 `assemble_manuscript` 从 `drafts/sections/*.tex` 拼装 `drafts/paper.tex`，加入 LaTeX wrapper、title、abstract、正文和 `\bibliography{related_work}`。随后 Writer 做全局 spot-check：术语、变量名、baseline 名称、章节过渡、intro/conclusion 呼应、method/experiment setup 一致性。若需要改正文，先改对应 section 文件，再重新拼装；不要直接一次性重写整篇 `paper.tex`。最后调用 `audit_manuscript_claims` 写 `drafts/manuscript_audit.md`。

### T8-SELF-CHECK

写 `drafts/self_check.md`，必须覆盖 argument chain、number audit、citation audit、figure/table audit、reproducibility audit、direct-full/T5/T6 boundary 和 High/Medium/Low TODO。
...
该文件可命名为 `drafts/claim_ledger.json` 或 `drafts/claim_audit.jsonl`。它应该由 tool 做抽取和初筛，由 LLM/Reviewer 做语义判断。
...
建议后续加入 `drafts/figure_registry.json`：
...
- `T8-SECTION-PLAN` 必须生成 `paper_state.json` 和 8 个 section outline。
...
## 9. Direct-Full 与跳过 T5/T6

当前主链支持暂时跳过 T5/T6，走 `T4.5 -> T7 -> T7.5 -> T8-RESOURCE`。写作时必须说明：

- Introduction 可以说本文基于 T4.5 novelty pre-audit 和 T7 full experiments 推进。
- Experiments 不应声称做过 pilot validation，除非 `pilot/` artifact 存在。
- Limitations 必须说明没有 T6 novelty-final 时，最终 novelty claim 仍依赖 T4.5 预审和实验后审查。
...
- `claim_ledger.json`、`figure_registry.json`、结构化 audit JSONL。
- 图表生成 tool：从 results/ablations 生成 plot script、image 和 LaTeX snippet。
...
- 不写学术结论。
- 只做资源索引、文件存在性、BibTeX key、数字候选、图表候选和结构化计划。
...
- `ideation/idea_scorecard.yaml`
- `drafts/evidence_plan.json`
- 可用的 `experiments/code/`、`pilot/pilot_code/` 或 run configs
...
- compute/seed limits
- direct-full path constraints
- skipped T5/T6 的影响，如果确实跳过
...
- `limitations`：risks、novelty audit、direct-full/T5/T6 边界、limitations section。
...
当前 T8 已有 `figure_table_plan.json`，后续应把图表生成进一步 tool 化。建议新增工具：
...
- `audit_manuscript_claims` 目前偏机械 hint，尚未形成强制 `claim_ledger.json`。
- 图表生成还处于 plan/registry 设计阶段，未完全接入自动 plot/table tool。
- 顶会/顶刊风格判断仍依赖 LLM 和 Skill，不应硬编码到工具。

下一步优先级：

1. 增加 `claim_ledger.json`，让每条核心 claim 显式绑定 source artifact。
2. 增加 `figure_registry.json` 和 results/ablation plot tools。
3. 按 13.1.1 把 Reviewer 拆成可选的 per-section review execution mode。
4. 增加 T8 小 workspace E2E，覆盖 RESOURCE -> one SEC -> DRAFT -> REVIEW -> REVISE。
5. 增加 T9 TeX fixture，测试真实 `latex_compile` 成功和失败修复路径。
