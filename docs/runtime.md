...
用途：以可恢复会话执行一个独立 Skill。

对于声明 `interaction.mode: guided` 的公共 Skill，执行顺序是：
...
`_intake.md` 会标记 `standalone` 或 `project` workspace。前者要求用户把资料放到声明的
`user_inputs/<skill>/...` 路径；后者只表示 runtime 找到了候选项目产物，不能证明其语义上足以支持
请求。guided Skill 会在发现缺少 source、result、citation、venue 决策或约束时，先写
`user_inputs/<skill>/_followup_request.md`（缺口、影响、建议回答/文件路径），再调用 `ask_human`。
它不能凭猜测产出最终交付物。
...
用途：`list-skills` 按研究流程展示可发现 Skill 的终端卡片；`browse-skills` 允许先输入
序号/名称查看详情，再显式输入 `run <序号或名称>` 启动引导会话；`describe-skill NAME`
展示完整上传/输出/恢复契约；`skill-status [NAME]` 读取 workspace 中的可恢复会话和最近可观察进度。
...
```bash
cd ResearchOS
PYTHONPATH=. python -m researchos.cli describe-skill paper-outline \
  --workspace ./workspace/local-test2
```
...
```bash
PYTHONPATH=. python -m researchos.cli run-skill paper-outline \
  "为英文实证论文建立大纲" \
  --workspace ./workspace/local-test2 \
...
interaction:
  ...
  request_prompt: 请说明论文类型、语言和范围。
  example_request: 为英文 systems 论文构建大纲。
  required_inputs:
    - id: research_brief
      label: 研究简介
      description: 问题、方法、证据和限制。
      paths: [user_inputs/my-skill/brief.md, drafts/outline.md]
      extensions: [.md]
      min_bytes: 80
  optional_inputs: []
  outputs:
    - id: result
      label: 结果报告
      path: drafts/my_skill_result.md
      description: 人类可读的结果及其证据边界。
```

一个输入项的 `paths` 中任意一个通过检查即可满足它。路径必须是 workspace 相对路径，
不能使用绝对路径或 `..`。`researchos/skills/contracts.py` 在无 LLM 的情况下检查文件存在、
普通文件、扩展名和最小字节数。`interaction.outputs` 与 `outputs_expected` 同时存在时必须
指向同一组路径。

### 9.2 发现与严格工具映射

[researchos/skills/loader.py](../researchos/skills/loader.py) 的 `discover_skills`、
`discover_skills_from_roots`、`resolve_skill` 和 `register_skill_tools` 只扫描 skill root
的直接子目录；每个目录必须含 `SKILL.md`，名称不能重复，guided contract 在发现时校验。

`SkillAgent` 仍支持兼容别名：`Read -> read_file`、`Write/Edit -> write_file`、
`Bash(*) -> bash_run`、`Glob(*) -> glob_files`、`Grep(*) -> grep_search` 和
`WebFetch -> web_fetch`。新公共 Skill 使用 runtime 原生工具名和 `strict_tools: true`：任何
未注册工具都会在运行前失败，不能静默丢弃后降级。

根目录 `skills/external_executor_skills/` 是嵌套的外部执行器协议树；它不会被默认的
standalone `list-skills` 扫描或在本节的公共 Skill 契约中改写。

### 9.3 引导式运行、会话和恢复

`run-skill` 的 guided 执行顺序是：
...
researchos describe-skill paper-polish --workspace ./workspace/project-a

researchos run-skill paper-polish "按英文期刊风格润色，不改变数字" \
  --workspace ./workspace/project-a --session-id paper-v2-polish

researchos run-skill paper-polish \
  --workspace ./workspace/project-a --session-id paper-v2-polish --resume
...
| `paper-outline` | 先建立论证、证据、章节和 venue-aware storyline | `drafts/outline*`、`drafts/writing_storyline.md` |
| `paper-write` | 写入证据约束的论文初稿并运行审计 | `drafts/paper.tex`、storyline 和审计 |
| `paper-polish` | 保留源稿的独立润色副本和变更报告 | `drafts/polished_*` |
...
`method-builder` 仍作为外部执行器兼容指导存在，不属于本轮公共 Skill 重写范围。

### 9.4.1 Venue-Aware 论文写作档案

`config/system_config/venue_writing_profiles.yaml` 是 T8 与论文 Skills 的内部写作档案。它把
target venue / template / language 解析为 `informs_story`、`neurips_concise`、`icml_concise`、
`iclr_concise`、`kdd_technical` 或通用档案。outline phase 还必须写
`drafts/writing_storyline.md`，保存 rationale 或技术根因、core insight、design、claim-to-evidence
mapping、alternative explanation、limitations 和 reviewer questions。

`audit_writing_craft` 输出 profile、section word counts、内部目标与 storyline coverage。这些均为
`WARN` 级写作诊断，不是官方页数、匿名或投稿规则。UTD/IS/INFORMS 强调
theory/rationale -> mechanism -> design -> evidence -> bounded implication；CCF-A 强调
technical bottleneck -> insight -> method -> result/ablation/analysis/failure evidence。投稿前仍须以当前
官方 CFP/template/checklist 为准。

### 9.5 Skill 自带工具

若某个 Skill 目录含 `tools/*.py` 并导出 `TOOL` 实例，`register_skill_tools()` 会把它注册
到 `ToolRegistry`。公共 Skill 目前复用经过测试的 builtin tools。新增私有工具必须为发现、
strict tool 映射、输入/输出契约和真实执行补测试。
...
### 11.5 LaTeX backend preflight

`T3.6-COMPILE` 与 `T9` 在启动 Agent/LLM 前执行 `latex_backend_preflight`。它先选当前
环境的 `latexmk`、再选 `tectonic`，宿主机仍缺失时才验证配置的 Docker TeX 镜像。preflight
不只检查 `docker` 命令：它会检查 daemon、允许的镜像以及镜像内 `latexmk/pdflatex/xelatex/bibtex`。
