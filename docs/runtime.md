...
- 如果 stdin 不可用或回答为空，当前 task 会进入可恢复暂停，不会把空输入当成“确认”
...
- PDF / section 处理：`extract_paper_sections`、`extract_pdf_text`、`fetch_paper_pdf`；T3 的 `[FULL-TEXT]` note 必须由 `extract_pdf_text` 覆盖完整页码并记录 `Reading Coverage`
...
T3 的恢复器会生成 `deep_read_queue_pending.jsonl` 和 `deep_read_queue_pending_meta.json`。这个 meta 是快照，不是实时计数器；runner 现在会在任意退出路径 best-effort 刷新它，包括 `finished`、`max_steps`、`budget` 和校验修复暂停。它会同时记录合格 note 文件数、内部匹配 key 数、无效 note 文件数和 `refresh_reason`，用于判断是否是旧快照或历史 stub 干扰。
...
- T3 看 note / table / bib
- T7 看 results_summary / ablations / seed ensemble / 真实 Docker digest
- T9 看 `main.pdf` + `编译状态: 成功`，并检查 PDF 头、mtime 和编译日志
