...
- 旧镜像缺少 `texlive-lang-chinese/ctexhook.sty` 时，中文 survey 会在 `xeCJK` 处失败；当前 Dockerfile 已补 `texlive-lang-chinese`、`fonts-noto-cjk` 和 `fontconfig`，需要重建 `researchos/system:latest` 后中文 XeLaTeX 才能通过。
