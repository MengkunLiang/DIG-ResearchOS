...
| T4 note-card 使用 | `ideation.py` 从 `synthesis_workbench.json` 的 `all_note_cards` 或旧结构 `notes/abstract_notes` 提取可检索 note-card section，但只优先暴露 `score>=0.55` 且非 `do_not_cite` / 非 unrelated 的卡片；缺少压缩摘要时，`ideation.j2` 会显式要求读取 `synthesis_workbench.json`、`paper_notes/` 和 `paper_notes_abstract/`，不会静默退化成只读 `synthesis.md` | 已落地 |
