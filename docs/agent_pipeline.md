...
最终得到 `relevance_score`、`priority_score_hint`、`relevance_score_semantics=metadata_priority_hint_requires_llm_review`、`relevance_score_components`。T2 恢复路径不会再用 `relevance_score >= 0.5` 硬过滤论文；如果候选池超过 `active_pool_max`，会按 active pool 分层保留本轮候选，其余写入 `papers_backlog.jsonl`。
...
当前核心参数来自 `config/agent_params.yaml -> agents.reader.modes.read.behavior`：

| 参数 | 默认值 | 作用 |
| --- | --- | --- |
| `deep_read_min` | `35` | T3 validator 最少完成的结构合格 deep-read note 数 |
| `deep_read_target` | `35` | T3 默认应读目标 |
| `deep_read_max` | `45` | active deep-read target 上限，保护位也在其中计数 |
...
5. **配额和桶**：新队列桶是 `seed`、`mainline_deep`、`bridge_deep`、`mainline_screened`、`bridge_screened`。active 深读目标、硬上限、主线筛读 backlog、bridge pool、screened backlog 和 must_explore bridge 保底都由 `agents.reader.modes.read.behavior` 配置；默认值见上表。候选仍必须先通过 LLM semantic screen，不能只靠 bridge_id 强制阅读。
