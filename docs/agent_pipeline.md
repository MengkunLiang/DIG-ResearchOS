...
- finalize 会先做 bounded OpenAlex DOI/OA 详情补全：最多 120 篇，只补 OpenAlex ID、abstract、references/related works、OA/PDF locations，不做相关性判断；再做 Crossref DOI 详情补全；最后从已有 references 做 bounded Crossref one-hop snowball，默认最多 40 个候选。
- `citation_edges.json` 会同时包含 record 自带引用边和 `crossref_snowball_backfill` 的 `source paper -> snowball candidate` 边。这样 `domain_map`、`identify_citation_hubs`、`deep_read_queue` 和 T3/T3.5/T4 都能看到 citation 补充，而不是只在 raw 里堆元数据。
- `search_log.md` 的 `## 检索式` 表会显示 Query、Bucket、Bridge、Tool/Source、Results、Persisted；`## Bridge Domain Query 覆盖` 和 `## Bridge Domain Plan 覆盖` 会明确列出 b1/b2/... 的实际召回和是否 missing/covered。即使 trace 缺失，finalize 也会从 `papers_raw.jsonl` 的 provenance 重建这些表。
