...
当前主链已经废弃“ResearchOS 自己在 T5-T7 内部实现并长时间跑实验”的默认语义。ResearchOS 负责研究协议、执行器选择、handoff prompt、文件契约、结果摄取、诚信审计、result-to-claim 和写作证据闭环；Codex CLI、Claude Code 窗口、人工外部执行器或 mock dry-run 负责在隔离路径中实现和运行实验。旧 `T5` Pilot、`T6` Pilot 后 novelty 复核和旧 `T7` 内部完整实验仍保留为显式 `run-task` 兼容节点，但不会作为默认主链入口。
...
- 会完整体现 `T5-HANDOFF -> T5-DRY-RUN -> T7-INGEST -> T7-AUDIT -> T7-CLAIMS -> T7.5 -> ask_human -> T8` 这样的链条
...
| `T5-HANDOFF` | `ExperimenterAgent` | `handoff` | 编译外部实验执行协议、allowed paths、Codex/Claude/manual prompt；不运行真实实验 | `external_executor/handoff_pack.json`, `executor_prompt.md`, `codex_prompt.md`, `claude_code_prompt.md`, `expected_outputs_schema.json`, `allowed_paths.txt` |
| `T5-DRY-RUN` | `ExperimenterAgent` | `dry_run` | 用 mock executor 跑通 result_pack/status/manifest/raw/config/log 文件协议 | `external_executor/result_pack.json`, `executor_status.json`, `run_manifest.json`, `heartbeat.json`, `raw_results/`, `configs/`, `logs/` |
...
| `T7-AUDIT` | `ExperimenterAgent` | `integrity_audit` | 审计 provenance、hash、metric source、mock_only、run_manifest；不相信执行器总结 | `experiments/integrity_audit.json` |
| `T7-CLAIMS` | `ExperimenterAgent` | `result_to_claim` | 把审计后的实验数字转成保守 claim mapping，并生成 T8 evidence pack | `experiments/experimental_claims.json`, `drafts/result_to_claim.json`, `drafts/experiment_evidence_pack.json`, `experiments/iteration_log.md` |
| `T5` | `ExperimenterAgent` | `pilot` | legacy 兼容节点：显式 run-task 才使用的小规模内部实验 | `pilot_plan.yaml`, `pilot_code/`, `pilot_results.json`, `motivation_validation.md` |
| `T6` | `NoveltyAgent` | - | legacy 兼容节点：基于 Pilot 做增量 novelty 复核 | `novelty_report.md`, `collision_cases.md`, `must_add_baselines.md` |
| `T7` | `ExperimenterAgent` | `full` | legacy 兼容节点：显式 run-task 才使用的内部完整实验 | `results_summary.json`, `runs/`, `configs/`, `iteration_log.md`, `ablations.csv` |
...
`T5-HANDOFF` 是新版实验链入口。它不写实验代码、不运行 Docker、不碰 GPU，也不把 LLM 生成的自然语言当实验结果。它只把 `project.yaml`、`hypotheses.md`、`exp_plan.yaml`、`risks.md`、`novelty_audit.md`、`idea_scorecard.yaml`、`synthesis.md` 和 `comparison_table.csv` 编译成外部执行器可执行的 artifact 协议。
...
| `external_executor/executor_selection.json` | 当前选择的执行器及 fallback 列表；默认测试选择 `mock_dry_run` |
...
`ExperimenterAgent(handoff)` 的初始消息要求只调用 `build_experiment_handoff_pack`。该工具读取 `ideation/exp_plan.yaml` 抽取 metric 和实验条目，读取 `project.yaml` 抽取 seed ensemble 和 venue hint，读取 `ideation/hypotheses.md` 做短 preview，并为每个上游 artifact 记录存在性、bytes 和 sha256。然后它写出 handoff pack、schema、allowed paths 和多执行器 prompt。validator 会检查这些文件全部存在、`semantics` 正确、executor output schema 包含必需字段、allowed paths 非空，并确认没有把 mock 或自然语言总结写成真实结果。

### 恢复逻辑

如果 handoff 文件已经存在且结构合格，`resume` 不需要让 LLM 重新生成实验协议；外部执行器选择、input manifest 和 prompt 会作为已有 artifact 进入恢复上下文。用户要切换真实执行器时，应重新运行 `T5-HANDOFF` 并显式传入 executor，而不是在后续 ingest 阶段改写结果来源。

## 6.10 T5-DRY-RUN：ExperimenterAgent（dry_run）
...
## 6.11 T7-INGEST / T7-AUDIT / T7-CLAIMS：外部实验结果闭环

新版 T7 不再是单个“完整实验”节点，而是三个互相分离的证据节点。
...
`T7-AUDIT` 读取 `experiments/results_summary.json` 和 `experiments/evidence_index.json`，审计每个 metric 是否有 source artifact、source artifact 是否在 index 中、artifact 是否仍存在、sha256 是否匹配、run manifest 是否存在且语义正确。它会把 `mock_only` 标为 WARN/状态，不把 dry-run 当成论文证据。
...
该文件的 `status` 可以是 `pass`、`mock_only` 或 `fail`。`fail` 表示 provenance 或结构有硬错误；`mock_only` 表示协议可通但不能作为论文 claim evidence。
...
`result_to_claim.json` 的核心字段是 `claim_mappings[]`，每条 mapping 包含 `support_status`、`metric_refs`、`evidence_refs`、`allowed_wording`、`forbidden_wording` 和 `limitations`。如果来源是 mock dry-run，`support_status=unsupported_mock_only`，Writer 只能把它写成协议测试或限制，不能写成实证结果。

### Legacy T5/T6/T7 兼容节点

旧 `T5`、`T6`、`T7` 保留是为了已有 workspace 和显式调试：

- `researchos run-task T5 --workspace ...`：运行 legacy pilot 内部实验。
- `researchos run-task T6 --workspace ...`：运行 legacy pilot 后 novelty 复核。
- `researchos run-task T7 --workspace ...`：运行 legacy 内部完整实验。

完整 `run/resume` 默认不会从 T4.5 进入这些 legacy 节点。T4.5 pass 路由到 `T5-HANDOFF`；T7.5 的旧推荐 `next_task: T7` 也会被状态机安全映射到 `T5-HANDOFF`。这样可以保留旧功能，又不会让主链误回“ResearchOS 自己长时间跑实验”的旧设计。
...
```text
T5-HANDOFF -> T5-DRY-RUN -> T7-INGEST -> T7-AUDIT -> T7-CLAIMS
 -> T7.5
 -> ask_human / gate
 -> 按 PI 推荐或人工指定进入下一步
```
