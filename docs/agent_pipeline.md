...
- `T3.6-VISUALS` 位于 survey state 和第一节之间。它只从 `literature/comparison_table.csv` 生成可复现的年份/方法分布图；每张图默认需要至少 8 条对应有效数据，数据字段、覆盖或样本量不足时写 `drafts/survey/figures/survey_visual_manifest.json` 的 `skipped` 状态，后续章节不得为了版式补造图像。
- T4 Gate1 前半段的必需落盘顺序是 Pass1 候选 → Pass2 接地复核 → 结构化候选方向 → 谱系分布 → 完整候选卡片 → 选择简报。`ideation/t4_progress.md` 从真实文件存在性计算 `[running]`、`[done]`、`[queued]` 或 `[paused]`；CLI 同时显示模型请求已提交、provider 等待、工具起止和每个 durable artifact 写入。它不会显示或伪装模型内部思维。`idea_scorecard.yaml`、假设和实验计划明确属于人工选择后的后半段，不会在前半段显示成误导性的 pending。
...
| `T4` | `IdeationAgent` | - | 先生成 compact context pack 与 `t4_progress.md`，再生成 Pass1/Pass2/Gate1 候选池并停到 T4-GATE1；用户选择后再生成最终假设、实验计划、决策链和风险评估；可消费 `survey_insights.json` | `t4_context_pack.json`, `t4_context_pack.md`, `t4_progress.md`, `_pass1_forward_candidates.json`, `_pass2_grounding_review.json`, `_candidate_directions.json`, `_gate1_selection_brief.md`, `hypotheses.md`, `exp_plan.yaml`, `idea_scorecard.yaml`, `rejected_ideas.md`, `gate_decisions.json`, `idea_rationales.json`, `risks.md`, `_family_distribution.md` |
...
| `survey_visual_manifest` | `drafts/survey/figures/survey_visual_manifest.json` | 由 comparison table 支撑的图路径、字体/DPI、可插入 section 或 skipped 原因 |
...
Agent 调用 `build_survey_figures`，只从 `literature/comparison_table.csv` 生成已定义的学术数据图：publication-year evidence landscape 和 method-family taxonomy distribution。工具使用 150 DPI、可用的 serif 字体、英文标签和克制配色，并在 manifest 中记录实际字体，不会假称当前系统安装了 Times New Roman。字段或行数不足时会生成 `status=skipped` 的 manifest，这是合法结果，后续 writer 不能为了让 survey “有图”而创建假图。只有 manifest 中 `status=generated` 且路径存在的图才能由 Taxonomy/Comparison 在正文解释后插入。
...
公共 standalone Skill 使用 `interaction` input/output contract：CLI 在调用 LLM 前验证
必需输入、写 `_runtime/skill_sessions/<session-id>.json`，输入不足则停在 `WAITING_INPUT`。
