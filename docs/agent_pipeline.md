...
| `T4` | `IdeationAgent` | - | 生成假设、实验计划、idea依据、风险评估 | `hypotheses.md`, `exp_plan.yaml`, `idea_rationales.json`, `risks.md` |
...
- 在 `ideation/idea_rationales.json` 里写机器可读记录，包含 `source_questions`、`literature_observations`、`missing_area_links`、`comparison_table_signals`、`seed_idea_links`、`lens_insights`、`reasoning` 和 `confidence`

validator 会要求 `idea_rationales.json` 符合 `idea_rationales.schema.json`，并覆盖所有最终假设 anchor。这样 T4.5 新颖性审计或 T5 实验计划都能回看“为什么会有这个 idea”。
