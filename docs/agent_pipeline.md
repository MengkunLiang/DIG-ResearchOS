...
- `audit_writing_craft`：检查 alignment/craft 机械问题，如独立 Limitations、abstract citation、CID 覆盖、实验 table/metric 锚点、数字可追溯、AI 套话等；当 `venue_style=both` 时也会审计两套风格变体。
...
本章先定义 artifact 和 design rationale，再讲整体架构、组件、notation、算法和实现。每个设计选择都要解释 why，不用实验结果证明方法有效；对应贡献段落要标 `% [Cn]`。写完必须调用 `update_manuscript_section_state(section_id="methodology", status="written")`。
...
本章解释实验和消融如何支持、削弱或仅部分支持 design rationale；至少提出并排除一个 alternative explanation，呈现 failure case 和 sensitivity。不能把未做的 T5/T6 或额外实验写成已完成。每条核心 cid 的解释段落也要标 `% [Cn]`。
...
Introduction 采用 5-move：Problem、Gap、Approach、numbered Contributions、venue-specific closing。gap/motivation 通常不超过 3 个，contribution 3-4 条，每条要和 cid 一一对应并标 `% [Ci]`。`ccf_a` 风格需要量化 results headline；`is` 风格需要理论或 reference anchor。Intro 不能超过已有 evidence。
...
Abstract 用 5 句骨架压缩全文：Problem、Gap、Approach、Key result、Contribution type。它不能出现 `\cite`，不能引入正文没有的数字、claim 或术语。`ccf_a` 风格通常 150-250 词，`is` 风格通常 200-300 词。
...
- `audit_writing_craft(paper_path="drafts/paper.tex", sections_dir="drafts/sections", paper_state_path="drafts/paper_state.json", alignment_matrix_path="drafts/alignment_matrix.json", venue_style=<venue_style>, output_path="drafts/craft_audit.md")`：检查独立 Limitations、abstract citation、CID anchor 覆盖、每个 cid 的 experiment table/metric/ablation 锚点、related-work orphan/laundry-list、AI 套话、贡献条数和数字可追溯，并同时写 `drafts/craft_audit.json`。`number_traceability`、abstract citation、独立 Limitations、缺 cid experiment artifact 等机械可查问题是 FAIL；Writer validator 会拒绝带未通过 FAIL 的 draft/revise。
