...
| `T2` | `ScoutAgent` | - | 检索、active pool/backlog 分层、验证、构建引用图领域地图和精读队列 | `papers_raw`, `papers_dedup`, `papers_verified`, `papers_backlog`, `citation_edges`, `domain_map`, `deep_read_queue` |
...
4. **读取/处置**：finish 后的 deterministic 收尾会把 `semantic_screen` 作为唯一语义判定来源。runtime 先把全量 raw 去重为候选池，再分成 `papers_dedup.jsonl`/`papers_verified.jsonl` active pool（10-120 篇）和 `papers_backlog.jsonl` backlog。`build_domain_map` 只允许 active pool 中 LLM-screened 论文进入 `core/theory_bridge/adjacent`，缺 screen 的非 seed 论文只能进入 boundary/backlog；`build_deep_read_queue` 只对 active verified 池做 100% deep/shallow 处置，不能只凭分数、bucket 或 degree 宣称它属于 core/target。

raw 数量只是完成 T2 的必要条件，不是充分条件；Scout 必须先判断 query/source/bucket 覆盖是否足够，完成摘要回填和必要 semantic screening，随后调用 `finish_task`。runtime 才会确定性调用去重、active pool/backlog 分层、metadata priority hint、enrich、metadata verification、引用边/domain map、access audit 和 deep-read queue 构建逻辑，依次产出 `papers_dedup.jsonl`、`papers_verified.jsonl`、`papers_backlog.jsonl`、`verification_failures.jsonl`、`citation_edges.json`、`domain_map.json`、`deep_read_queue.jsonl`、`access_audit.md`、`search_log.md` 和 `missing_areas.md`。最后 `ScoutAgent.validate_outputs()` 再检查数量、schema、`dedup <= raw`、queue 是否来自 active verified 池、active verified 论文是否 100% 进入 deep/shallow 阅读处置链、seed paper 是否进入队列，以及若 active verified 池里已有 `semantic_screen` 允许的跨域/理论桥接候选，queue 至少保留一个并放入非 `triaged_out` 的 `seed/mainline_deep/bridge_deep` 阅读区，避免该类素材只留在 screened backlog 后被 T3 跳过。
...
6. runtime 在 `finish_task` 后从 raw 生成 active `papers_dedup`、active `papers_verified`、`papers_backlog`、`citation_edges`、`domain_map`、`deep_read_queue`、`access_audit`、`search_log`、`missing_areas`
...
`papers_raw.jsonl` 是全量检索审计池，可以超过 120；`papers_dedup.jsonl` 不是全量 raw 的简单去重结果，而是本轮 active candidate pool，必须控制在 10-120。超出 active pool 的候选写入 `literature/papers_backlog.jsonl`，带 `t2_pool_role=backlog`、`triaged_out=true` 和 `triaged_reason=t2_active_pool_cap_exceeded`，用于后续 abstract sweep、人工回捞或排障，不会被静默丢弃。

active pool 默认选择顺序：

- 用户 seed
- `semantic_screen.can_enter_deep_read=true` 的高置信候选
- confirmed bridge 的召回候选，每个 bridge 默认最多 15 篇
- citation snowball 候选，默认最多 12 篇
- 其余按 `metadata/search priority hint` 补足到 120

`search_log.md` 会写 `T2 active candidate pool: input=..., active=..., backlog=..., selection_reasons=...`。如果看到 raw 很大但 `papers_dedup=120`，这是正常分层；如果 `papers_dedup > 120`，才是 finalize/validator 错误。爆量排障顺序是先看 `## Bucket 覆盖` 和 `## Source/Tool 覆盖`：若 core/theory_bridge/adjacent 的 Query Calls 很高，通常是 query 或 bridge 扩展重复；若 `OpenAlex/Crossref citation snowball` 的 `raw_persisted` 很高，才说明 citation 扩张过多。
...
最终得到 `relevance_score`、`priority_score_hint`、`relevance_score_semantics=metadata_priority_hint_requires_llm_review`、`relevance_score_components`。T2 恢复路径不会再用 `relevance_score >= 0.5` 硬过滤论文；如果候选池超过 120 篇，会按 active pool 分层保留本轮候选，其余写入 `papers_backlog.jsonl`。
...
该工具不调 LLM，也不判断论文是否学术相关。hub 的准入语义是“小额保护 + Reader 复核”：seed hub 直接进入；已有 `semantic_screen.can_enter_deep_read=true` 且 `relation_to_project` 不是 `shared_keyword_only/unrelated` 的 hub 可以进入；缺少 semantic screen 的结构 hub 也可以占用最多 `citation_hub_slots=3` 个保护槽，并写入 `citation_hub_needs_reader_screening=true`，由 T3 Reader 决定是否真正有机制/方法/评价价值。已有 LLM screen 明确排除的 hub 不会被强制 deep-read。`citation_hub_slots` 是保底保护槽，不是所有 hub 的硬上限；少数 hub 仍可能因为普通 mainline ranking 自然进入 active target，但不会绕过 Reader 审查。进入保护配额的记录会写入 `is_citation_hub`、`hub_type`、`hub_score`、`citation_hub_protected_slot=true`。排序语义是：`seed > seed_neighbor hub > citation protected hub > semantic-screened must_explore bridge > should_explore bridge > primary high priority > screened backlog`。`ReaderAgent.validate_outputs()` 会把这些 active 内 citation hub 当作 protected record 检查，防止引用图识别出的关键节点从未被 T3 阅读。
...
5. **配额和桶**：新队列桶是 `seed`、`mainline_deep`、`bridge_deep`、`mainline_screened`、`bridge_screened`。默认 active 深读目标为 35，硬上限为 45；`mainline_screened_cap=90` 保留主线筛读 backlog；每个 bridge 最多保留 `bridge_pool_cap=15` 个候选，screened backlog cap 为 `bridge_screened_cap=7`；`must_explore` bridge 有 `bridge_deep_floor=3` 的 active 保底，但仍必须先通过 LLM semantic screen，不能只靠 bridge_id 强制阅读。
...
- 关键字段存在
- `dedup <= raw`
- active `papers_verified.jsonl` 存在且数量合理
- `verification_failures.jsonl` schema 正确
- `deep_read_queue.jsonl` 必须来自 verified 池
- active `papers_verified.jsonl` 中每篇论文都必须在 `deep_read_queue.jsonl` 中有 deep_read 或 shallow_read/backlog 处置，不能因为缺 `semantic_screen` 被静默丢弃；active pool 之外的候选应在 `papers_backlog.jsonl` 可见
...
- **阈值**：`t2_finish_finalize_min_raw`，默认 30，下限 10
...
- **阈值**：同 `t2_finish_finalize_min_raw`
...
| `papers_dedup` | `literature/papers_dedup.jsonl` | 是 | active candidate pool，旧 workspace 回退时使用 |
| `papers_verified` | `literature/papers_verified.jsonl` | 否但强烈推荐 | active verified 池 |
| `papers_backlog` | `literature/papers_backlog.jsonl` | 否 | active pool 外的轻量补读候选 |
...
Deep read 完成后，orchestrator 自动运行 abstract sweep，从 active verified/dedup 和 `papers_backlog` 中补读尚未被 `paper_notes/` 或 `paper_notes_abstract/` 覆盖的候选。它只基于 title/abstract/metadata 做轻量补读，不是全文证据，也不会新增复杂 report；继续复用 `literature/paper_notes_abstract/`、`literature/access_audit.md` 和 `_runtime/logs/researchos.log`。
...
- 保留缺摘要但有 title 的 metadata-only 轻量记录，并标为弱证据
...
abstract sweep 的目标是让未进入 deep-read target 但有摘要的论文也形成可读的低成本证据提示。它仍然必须标注 abstract-only，不能被 T8 当作 FULL-TEXT 证据；如果轻量阅读发现某篇论文对主线非常关键，应在后续 resume/人工补读中升级为 deep read，而不是在 abstract note 里伪装全文阅读。
