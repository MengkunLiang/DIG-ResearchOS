...
- `T4-GATE1` 给用户看的主文件是 `ideation/_gate1_candidate_cards.md`：每个方向使用短 `display_title` 作为标题，正文展示完整方向描述、技术机制、明确创新（类型/相对最近工作的变化/非增量理由）、Gate1 候选 `D#-H#`、可组合关系、现实/管理/商业含义、逐维评分依据、核心论文依赖、风险和推荐动作。`D#-H#` 只是选择前的候选命题，不能和后半段的最终 `hypotheses.md` 混同；没有证据支撑 H2/H3 时必须显示缺口而不是补造。卡片带有 `ResearchOS Gate1 candidate-card schema: v2` 标记；恢复时，旧版无标记的长标题派生卡会由 `_candidate_directions.json` 安全重建，候选 JSON、选择记录和最终假设不会被改写。`_candidate_directions.json`、Pass1/Pass2 JSON 只作为机器可读附录。用户选择后会先写 `_gate1_user_selection.json` 和 `selected_idea_brief.md`，T4 后半段再把最终假设、实验计划、scorecard、reject/defer 记录和最终 `selected_idea_brief.md` 补全。
- `T3.6-VISUALS` 位于 survey state 和第一节之间。它是受限的确定性阶段：只读取 `drafts/survey/survey_plan.json` 的显式 taxonomy tree，最多生成 `drafts/survey/figures/fig_taxonomy_overview.pdf`。该图只能表达 class label 和直接 paper-ID 链接数，不能表达性能、baseline、相对提升、T2 筛选分数、推断性安全/风险或“研究前沿”。每个直接 paper ID 必须能在 `literature/paper_notes/` 或 `literature/paper_notes_bridge/` 解析到本地结构化 note card；否则删除旧 canonical PDF 并写 `survey_visual_manifest.json` 的 `skipped` 状态。解析只证明来源链接可追溯，不是证据强度判断；后续章节不得为了版式补造图像。
- T4 Gate1 前半段的必需落盘顺序是 Pass1 候选 → Pass2 接地复核 → 结构化候选方向 → 谱系分布 → 完整候选卡片 → 选择简报。`ideation/t4_progress.md` 从真实文件存在性和 `ideation/t4_execution_events.jsonl` 计算 `[running]`、`[done]`、`[queued]` 或 `[paused]`；CLI 同时显示模型请求已提交、provider 等待、Pass1 主线与四个补充通道、逐候选 Pass2、已落盘评分、工具起止和每个 durable artifact 写入。若模型没有写进度事件，runtime 会从刚落盘的候选 JSON 反向补发事实摘要。它不会显示或伪装模型内部思维。`idea_scorecard.yaml`、最终假设和实验计划明确属于人工选择后的后半段，不会在前半段显示成误导性的 pending。
...
## 2.1 所有阶段共享的 CLI 可观测性

完整 pipeline 的 `run` / `resume` 和单阶段 `run-task` 不再只显示 “tool called” 或
“task finished”。每个正式 task 与 Human Gate 都通过统一的 **Stage Start -> Stage
Progress -> Stage Summary** 协议展示：

1. **开始**：阶段目标、要解决的研究问题、计划计算、输入 Artifact 的含义/状态/规模，以及预计输出及其下游消费者。
2. **过程**：只展示已经计算且对研究决策有价值的受限事实，例如 Top-N、分布、失败、backlog、风险、unsupported 和候选/claim 的变化；不会展示完整 prompt、LLM 私有思维或原始 Tool payload。
3. **结束**：主要结论、关键统计、当前决策、限制、实际读取文件和 Artifact Manifest。Artifact 状态包括 `created`、`updated`、`reused`、`missing`、`invalid`；如果外层状态机 validator 推翻了刚完成的 Agent 结果，会额外显示 `invalidated`，防止用户误判该阶段已成功。

各阶段的核心可见内容如下。完整数据始终保留在表中列出的 Artifact，不在终端无边界展开。

| Stage | CLI 重点展示 | 关键 Artifact |
| --- | --- | --- |
| T1 | 研究范围、seed/bridge 边界与输入约束 | `project.yaml`, `literature/bridge_domain_plan.json` |
| T2 | Query/source/bucket 覆盖、去重/核验、候选分数提示、引用结构、queue/backlog | `papers_*.jsonl`, `search_log.md`, `domain_map.json`, `deep_read_queue.jsonl` |
| T3 | 每篇卡的 access/evidence、页码、抽取、截断、机制证据类型与未支持字段 | `literature/paper_notes/`, `notes_manifest.json`, `comparison_table.csv` |
| T3.5 / T3.6 | contribution/mechanism/tension/transfer 综合；survey 语料/taxonomy/可视化/编译充分性 | `synthesis_workbench.json`, `drafts/survey/*` |
| T4 / Gate1 | 主线、bridge、四个 coverage supplement 的区分；Pass1->Pass2；候选创新、H1/H2/H3、评分、风险与可组合关系 | `ideation/_pass*.json`, `_candidate_directions.json`, `_gate1_candidate_cards.md` |
| T4.5 / T5 | collision/CDR/baseline 的约束；按需论文卡核验、reboost、project Skill、executor handoff | `novelty_audit.md`, `paper_notes*/`, `paper_card_evidence_index.json`, `handoff_pack.json`, `skill_specialization_report.json` |
| T7 / T7.5 | run 成功/失败/OOM/NaN、provenance、baseline coverage、claim support 和人工放行依据 | `experiments/*`, `integrity_audit.json`, `result_to_claim.json` |
| T8 | Writer 实际资源、section evidence/claim 对齐、审稿发现与 patch | `manuscript_resource_index.json`, `alignment_matrix.json`, `paper_state.json` |
| T9 | 真实编译后端、尝试、warning/error、PDF hash、source/bundle fingerprint 与 claim audit | `submission/compile_report.json`, `submission/bundle/*` |

特别规则：检索覆盖缺口不等于研究缺口；citation graph/score 是阅读优先级提示而非论文质量；工具 cluster 不是机制结论；`prior_art: none` 是高不确定性而不是新颖性证明；T4 四个 supplement 是 coverage check，不能替代主线发散，也不能因没有证据而硬编 candidate。`unsupported` 必须保持可见。

`--verbosity concise|normal|detailed` 控制展示密度，`--no-color` 支持 CI/SSH，`--json-events` 把同一结构化事件镜像到 stdout。所有运行无条件写入 `<workspace>/_runtime/events/<run_id>.jsonl`；恢复只展示当前待完成工作，不重放历史工具过程。完整说明见 [logging.md](./logging.md)。
...
| `T4` | `IdeationAgent` | - | 先生成 compact context pack、`t4_progress.md` 与候选级公开执行事件，再生成 Pass1/Pass2/Gate1 候选池并停到 T4-GATE1；用户选择后再生成最终假设、实验计划、决策链和风险评估；可消费 `survey_insights.json` | `t4_context_pack.json`, `t4_context_pack.md`, `t4_progress.md`, `t4_execution_events.jsonl`, `_pass1_forward_candidates.json`, `_pass2_grounding_review.json`, `_candidate_directions.json`, `_gate1_selection_brief.md`, `hypotheses.md`, `exp_plan.yaml`, `idea_scorecard.yaml`, `rejected_ideas.md`, `gate_decisions.json`, `idea_rationales.json`, `risks.md`, `_family_distribution.md` |
...
| `T4.5` | `NoveltyAuditorAgent` | - | 对假设做新颖性预审（含 mechanism/design-rationale tuple 碰撞检测）；需要时定向读取论文卡的精确 section，abstract-only 仅作补检线索；非通过 verdict 进入人工决策 gate | `novelty_audit.md`, `_mechanism_tuples/`, `_design_rationale_tuples/`; 有撞车时另写 `collision_cases.md` |
| `T5-HANDOFF` | `ExperimenterAgent` | `handoff` | 编译外部实验执行协议、allowed paths、Codex/Claude/manual prompt、AGENTS/CLAUDE 指南、required baselines 和受约束论文卡索引；不运行真实实验 | `external_executor/handoff_pack.json`, `paper_card_evidence_index.json`, `input_manifest.json`, `AGENTS.md`, `CLAUDE.md`, `README.md`, `executor_prompt.md`, `codex_prompt.md`, `claude_code_prompt.md`, `expected_outputs_schema.json`, `allowed_paths.txt`, `job_state.json` |
...
| T4 note-card 使用 | runtime 在 T4 prompt 渲染前生成 `ideation/t4_context_pack.json`、`ideation/t4_context_pack.md` 和 `ideation/t4_progress.md`；`ideation.py` 从 `synthesis_workbench.json` 的 `all_note_cards` 或旧结构 `notes/abstract_notes` 提取可检索 note-card section，只保留 `score>=0.55` 且非 `do_not_cite` / 非 unrelated 的卡片；prompt 要求先读 compact pack，只在核验具体 claim 时定向打开单篇 note 或 `comparison_table.csv` | 已落地 |
| T4.5 collision + ambition | `novelty_auditor.j2` 要求同时写 Collision Axis 和 Ambition Axis；当最近工作、机制、设计理由或基线依据需要确认时按需读取 `paper_notes*` 的精确 section，abstract-only 不能单独确认碰撞结论；`mechanism_tools.py` 增加 `extract_design_rationale_tuple` / `compare_design_rationale_tuples`；`novelty_auditor.py` 校验 `_design_rationale_tuples/` 与 routine reframe 要求 | 已落地 |
...
`build_synthesis_workbench` 会在 `literature/synthesis_workbench.json` 中保留 `notes`（deep/bridge notes）、`abstract_notes`（粗读 notes）和 `all_note_cards`（合并卡片）。T4 启动时 runtime 会按质量和方法/bridge/缺口/来源 lane 多样性从卡片中选择最多 12 张（硬上限 18）压缩为 `ideation/t4_context_pack.json` 和 `ideation/t4_context_pack.md`，并写 `ideation/t4_progress.md` 给 CLI 展示当前 T4 已准备哪些证据。`build_manuscript_resource_index` 会在 `drafts/manuscript_resource_index.json.paper_note_cards` 中写入同样的 section 摘录，并在每个 `drafts/section_outlines/*.md` 的 Note Card Retrieval Plan 中列出当前 section 最相关的 cues。`IdeationAgent` 优先消费 compact context pack，避免一开始分页扫全部 paper notes 或 `comparison_table.csv`。

后续阶段读取 note cards 的协议：

1. **先读结构化索引，再回查原始 note**。T3.5/T3.6 优先读 `literature/synthesis_workbench.json.all_note_cards`；T4 优先读 `ideation/t4_context_pack.md` 或 `ideation/t4_context_pack.json`；T8 优先读 `drafts/manuscript_resource_index.json.paper_note_cards` 和当前 `drafts/section_outlines/<section>.md` 的 Note Card Retrieval Plan。索引只提供摘录和路由；真正写 claim 或引用前必须回查 `path`/`source_file` 指向的原始 `.md` section。
...
4. **T3.6 survey 使用方式**。Survey plan 和 section writer 应把 `all_note_cards` 作为 taxonomy class、comparison stream、challenge 和 future agenda 的证据路由表；taxonomy/comparison 不应只复述 synthesis.md，而应回查代表卡片的 A/B、§13、§14、§18、§19。若某个 survey section 证据不足，应先用 `grep_search` 在 citation pool 或 Note Card Retrieval Plan 指向的 `source_file` 中定位对应 section，再定向 `read_file`；仍找不到直接证据时写成 coverage boundary 或弱化表述。
5. **T4 idea 使用方式**。IdeationAgent 会收到 compact context pack，而不是直接面对几十到上百篇 note。候选 idea 的 `source.idea_origin` 可以来自 `evidence_driven`、`mechanism_challenge`、`cross_domain_analogy` 或 `bridge_synthesis`，但如果依赖 abstract-only card，必须在 provenance 中标明 low-evidence/upgrade-needed，不能把它写成已验证机制。T4 只在需要核验具体机制、引用或边界条件时读取单篇 note，并应更新 `ideation/t4_progress.md` 说明当前 Pass 和已写入文件。若 synthesis 中某个论断需要回查原始 note，T4 可先用 `grep_search` 在 context pack 的 `source_file` 中定位 `## A/B`、`## 13`、`## 14`、`## 18` 或 `## Raw Abstract`，再定向 `read_file`，避免为了一个 section 分页读整批 notes。
6. **T8 写作使用方式**。Writer 每个 section 的 outline 会列出相关 note-card cues；不同章节有不同的起始/最大卡片预算（例如 Related Work 10/14、Introduction 8/12、Analysis 8/12，Abstract 为 0），避免“读得越多越好”的无目标扫描。写作时先用 cues 定位候选，再打开原始 note section 核对 sentence-level claim 和 citation key。不要因为 note card 出现在索引里就自动引用；必须看 `citation_use`、`citation_quality_score` 和 `evidence_level`。若 claim/citation 对不上，固定升级顺序是：已选卡的精确 section → weak/background card 仅作边界线索 → 弱化/删除或 limitation → 只对明确缺口做一次有 query/section/语言/数量上限的补检；新来源必须先进入笔记卡后才能引用。
...
| `survey_visual_manifest` | `drafts/survey/figures/survey_visual_manifest.json` | taxonomy-only 图政策、survey-plan fingerprint、唯一允许的图路径或 skipped 原因 |
...
Agent 调用 `build_survey_figures`，只从 `drafts/survey/survey_plan.json` 的显式 taxonomy tree 生成唯一允许的学术图：`fig_taxonomy_overview.pdf`。工具使用 150 DPI、可用的 serif 字体、英文标签和克制配色，并在 manifest 中记录实际字体，不会假称当前系统安装了 Times New Roman。图只能展示 taxonomy class 和直接记录的 paper-ID 链接；每个直接 ID 都必须解析到 `literature/paper_notes/` 或 `literature/paper_notes_bridge/` 的本地 note card，解析结果写入 `source.paper_link_audit`。不得读取或图示 `comparison_table.csv` 的数值、跨研究 baseline、相对提升、T2 筛选分数或推断性安全/风险。taxonomy 顶层类别不足或存在无法解析的直接 ID 时会生成 `status=skipped` 的 manifest，并删除过期 canonical PDF；这是合法结果，后续 writer 不能为了让 survey “有图”而创建假图。只有 manifest 中 `status=generated` 且唯一图路径存在时才能由 Taxonomy section 解释后插入。

Survey 使用紧凑章节结构而不是 CCF-A research-article 的短篇长度策略。内部 section-depth floor 只防止摘要式拼接，实际篇幅由 taxonomy breadth、已验证证据、比较和边界条件决定；不得为凑字数添加无证据段落。UTD/IS 理论—机制论文与 NeurIPS/ICML/ICLR/KDD 技术论文的 venue profiles 仍只用于 T8 研究论文写作，不替代 T3.6 的 survey 深度契约。
...
```text
latex_compile(tex_path="drafts/survey/survey.tex", engine="xelatex" 或 "pdflatex", bibtex=true)
```

写作语言为中文时使用 `xelatex`；英文稿使用 `pdflatex`。`backend=auto` 会优先使用当前
环境的 `latexmk`、再使用 `tectonic`，宿主机缺失时使用 `config/runtime.yaml` 中允许的
Docker TeX 镜像。Compose 容器则使用镜像内 TeX，不会尝试 Docker-in-Docker。
...
如果当前环境、Docker daemon 或 Docker TeX 镜像都不可用，`latex_compile` 会返回
`waiting_environment_*`，runtime 会暂停；安装/修复对应 backend 后可 resume。`T3.6-COMPILE`
会在 Agent 调用前进行同一套 preflight，避免写作完成后才暴露缺环境。如果是 citation 或
LaTeX 语法错误，应读 log 后修对应 section，再重新 assemble/compile，而不是一口气重写整篇 survey。
...
6. 依次写 `ideation/_candidate_directions.json`、`_family_distribution.md`、`_gate1_candidate_cards.md` 和 `_gate1_selection_brief.md`，保证所有 Pass 1 候选都对用户可见
7. Gate1 卡片完整展示候选评分、机制、prediction、counterfactual、最低实验和 kill criteria；`idea_scorecard.yaml` 不在前半段生成
...
`T8-STYLE-GATE` 是状态机级 immediate gate，不依赖 Writer 是否记得调用 `ask_human`。它读取 `project.yaml` 的 `target_venue` 并展示 IS/UTD、CCF-A、both、基础中文、基础英文和自定义模板选项；选择后由 runtime 写 `drafts/writing_style.json`。该 JSON 必须包含 `human_interaction_id`，并且这个 id 必须能在 `_runtime/human_interactions.jsonl` 中找到；这样可以区分“用户真实选择”和“模型自己编造的默认选择”。它还写 `venue_profile`，由 `config/system_config/venue_writing_profiles.yaml` 解析为内部论证/写作密度档案；这不是官方页数或投稿政策。
...
1. 调用 `build_manuscript_resource_index`，扫描 literature、paper notes、bib、hypotheses、idea scorecard、novelty audit、results summary、ablations、runs、configs、figures、tables 和 code artifacts，写 `drafts/manuscript_resource_index.json`。该索引会同时暴露 `citation_ref_by_note_id`、`note_id_by_bib_key`、`unmapped_note_ids` 和 `paper_note_cards`：T3.5 的 `[note:<id>]` 是 Markdown provenance，T8 TeX 正文必须用这里的 `\cite{bibkey}` 映射；无法映射到 BibTeX key 的 note 只能作为 provenance/补资源提示，不能原样进入 TeX 引用。每张 `paper_note_cards` 会标记 `claim_usable` 和可选 `quality_warning`；`score<0.55`、`do_not_cite`、`citation_allowed=false` 或明显 unrelated 的卡片保留在索引中供审计，但不会进入章节 outline 的 “Relevant indexed note cards”。
...
`T8-WRITE` 读取 resource index、section plan、evidence plan、figure/table plan、CDR ledger、claim ledger、figure registry、alignment matrix、synthesis、hypotheses、results 和 bib 预览，写 `drafts/outline.md` 与 `drafts/writing_storyline.md`。
...
大纲必须包含标题候选、paper thesis、contribution map、section-by-section argument、figure/table plan、claim ledger 和 alignment matrix refinement。`writing_storyline.md` 必须按当前 profile 的 headings 记录问题、why-it-matters/technical bottleneck、closest-prior tension、理论/技术根因、core insight、design/method mapping、claim-to-evidence mapping、alternative explanations、failure/limitations 和 reviewer questions。UTD/IS/INFORMS 需显式说明 theory/rationale -> mechanism -> design principle -> evidence -> bounded theoretical/practical implication；NeurIPS/ICML/ICLR/KDD 需显式说明 technical bottleneck -> insight -> method module -> main result/ablation/analysis/failure evidence。Validator 同时检查 outline 和 storyline 的必需 headings；内部 section word targets 仅是诊断，不会替代当前官方 CFP/template。
...
`initialize_manuscript_state` 生成每章 `drafts/section_outlines/<section>.md` 时，会把 section-specific Note Card Retrieval Plan 写入大纲。这个 plan 只列 `claim_usable=true` 的卡片，并按章节所需字段选择 cue，例如 Related Work 侧重 §2/A/B/§9/§14/§19，Method 侧重 §2/A/§14/§15/§18，Analysis 侧重 §13/§14/§18/§19。若索引里只有弱卡片，outline 会明确提示“有索引但没有可支撑 claim 的卡片”，要求 Writer 直接读 note 并把弱卡片只当背景或 limitation，不得支撑核心结论。
...
T9 进入 LLM 前会先做编译环境 preflight：`auto` 依次验证本机 `latexmk`、本机 `tectonic`、
已允许的 Docker TeX 镜像。宿主机无 TeX 但 Docker 镜像可用时仍可生成真实 PDF 和日志；
Compose 内则要求镜像自身已有 TeX，不使用 Docker-in-Docker。所有 backend 都不可用时任务以
`WAITING_ENVIRONMENT` 暂停，`state.yaml` 保持 `PAUSED`；修复后直接 `resume`，不会先消耗
LLM 步数再失败。
...
4. 尝试编译
   ...
   - `latex_compile` 的 `auto` 顺序是本机 `latexmk`、本机 `tectonic`、已启用的 Docker TeX backend；默认 runtime 已启用 allowlisted Docker fallback，否则等待环境或 `export_only`
...
公共 standalone Skill 使用 `interaction` input/output contract：CLI 先在不调用 LLM 的情况下验证
必需输入并写 `_runtime/skill_sessions/<session-id>.json`。非交互输入不足则停在 `WAITING_INPUT`；
交互终端可先运行受限 intake Agent，反复询问人并只把人提供的内容整理到该 Skill 的
`user_inputs/<skill>/`，重检通过后才进入真正 Skill。intake 不能生成论文、实验或其他最终产物。
新的公共 Skill 启用 `strict_tools: true`，缺工具会明确失败而不是降级。当前论文工作流包括
`paper-outline`、`paper-write`、`paper-polish`、`paper-revision`、`paper-claim-audit` 和
`paper-compile`。完整契约和恢复方式见 [runtime.md](./runtime.md#9-skills-runtime)。
