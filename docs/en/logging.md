...
| --- | --- | --- |
| `citation_diversity` | Hard release check: fewer than the configured 50% minimum of locally traceable, evidence-qualified, survey-plan-scoped bibliography keys are represented, or a key is genuinely over-concentrated. The repeat cap scales with total citation occurrences, so a long survey is not rejected merely because a foundational paper exceeds a fixed count. | Read `repair_guidance.citation_diversity.coverage_contract`, review every `section_review_queue` note at its `source_file`, and add/replace a citation only after exact claim verification. FULL/PARTIAL notes may support claims; ABSTRACT-ONLY notes are limited to background/trend/scope/boundary prose. If no safe source exists, write the repair plan instead of padding citations. |
