...
| --- | --- | --- |
| `citation_diversity` | Reports in-scope traceable availability, actual use, unused candidates, and citation concentration. There is no fixed coverage percentage. A failure means genuinely excessive concentration, not that too few library entries were cited. | Consolidate repeated claims first. Use `section_review_queue` only to locate a semantically valid alternative and open its `source_file` before use. FULL/PARTIAL notes may support verified claims; ABSTRACT-ONLY notes remain limited to background, trend, scope, or boundary prose. Never pad citations. |
