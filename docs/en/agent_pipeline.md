...
| T4 | Which evidence-grounded research idea should be evolved or selected? | P0/P1/P2 Population, Evidence Index, scores, lineage, Portfolio, and one or more Pre-Novelty briefs | Pre-run confirmation; Gate1 directives, independent multi-Proposal selection, composition, rollback |
| T4.5 research-plan audit and formalization | Does each selected Pre-Novelty idea remain differentiated, and can it become a coherent, falsifiable, quality-gated research package? | Phase 1 novelty/collision audit; Phase 2 begins from the compact Pre-Novelty brief, hypothesis brief, and audit, obtains only decision-relevant evidence fragments, then establishes `research_blueprint.yaml` before deriving `claim_registry.yaml` and `exp_plan.yaml`, formal claims and Proposal; Phase 3 orientation-aware review/repair and formalization receipt; multi-track archive when applicable | Novelty human review only for a non-pass audit; with multiple accepted Proposals, a mandatory T5-preselection Gate chooses one; T4's UTD/CCF-A/Hybrid orientation is inherited without another user choice |
| T5 | What can an external executor implement without inventing paper claims, and which resources can be acquired and audited automatically first? | handoff pack, project-specific Skill suite, Phase B resource/provenance reports, executor selection, and the six-file Writer Handoff | Protocol boundary, optional local-material, and executor gates; manual resource upload is not required |
...
- `T3.6-SUPPLEMENT-READ` validates the upgrade receipt by intersecting conservative aliases (`id`, `paper_id`, normalized ID, DOI/arXiv, external IDs, and title), so a receipt from an earlier run may be retained without causing a false missing-paper loop. The reader remains forbidden from `_runtime/logs/` and trace files; an exploratory permission miss is recorded in trace but is not a research failure.
- `T3.6-ASSEMBLE` first creates `survey.tex`, then runs a deterministic audit. Use `audit-survey --workspace <workspace>` after a concrete repair to regenerate `survey_audit.md/json` without contacting a provider. The citation-utilization audit distinguishes the traceable in-scope library from sources actually cited and exposes unrepresented candidates without imposing a fixed coverage percentage. Citation existence, claim alignment, provenance, and evidence-level restrictions remain hard checks. A total-use-scaled concentration guard detects mechanical reliance on one source without forcing unrelated replacement citations.
...
Gate1 shows 1–3 Portfolio Candidates first while retaining 6–8 Active Candidates and the complete Archive. The researcher may proceed with one complete Candidate, or select two or three complete Candidates to write **separate** Proposals, run another Generation, focus a Candidate or Idea Family, create a Crossover, compose selected hypotheses/contributions/genes, inspect score/evidence/lineage, regenerate one Route, rollback, or pause. Multi-Proposal selection is not a merge: each Candidate completes T4.5 sequentially in an isolated context and is archived under `ideation/proposal_portfolio/tracks/`. When more than one Proposal passes, `T4.5-PORTFOLIO-GATE` presents a compact table before T5 and the researcher selects exactly one Proposal for the experiment and manuscript path. A single Proposal goes directly to T5. Read-only actions do not call a model. Every state-changing action explains expected work, whether a model is called, what is retained, rollback availability, and whether T4.5 is reached.
...
Choosing a complete Candidate creates `ideation/hypothesis_brief.yaml`, lineage, search targets, and a Pre-Novelty brief. These artifacts are not an experiment authority. T4 moves directly to T4.5. Its audit runs in a dedicated context and owns collision labels, required baselines, and claim ceilings only. A pass then starts a fresh formalization context that creates the common `research_blueprint.yaml`, `claim_registry.yaml`, formal `hypotheses.md`, `exp_plan.yaml`, and a Proposal covering seven research functions. A third, independent orientation-aware review/repair context checks the same argument framework with UTD, CCF-A, or Hybrid weights. For multi-Proposal selection, every accepted package is copied as an independent immutable track before the next Candidate begins; only the researcher-selected track is materialized as T5's canonical input. The seven functions guide coverage rather than requiring seven literal headings, so adjacent functions may be merged when that improves the prose. T5 is blocked unless all structured sources, Proposal, experiment mappings, and the accepted review agree.

## T5 To T8 External Evidence Path

ResearchOS deterministically compiles the executor handoff, then deterministically runs `T5-SPECIALIZE-EXECUTOR-SKILLS` to publish and independently validate the context/report/13-Skill suite. Neither operation asks an LLM to reconstruct control files or repeat shell diagnostics. The Protocol page separates a compiled handoff from authorization for a full experiment, but it does not require a researcher to find public datasets, code, baselines, benchmarks, or weights manually: when no local material is available, a bounded Phase A/B executor searches, acquires fixed revisions, reviews license/security/protocol fit, records provenance, stops, and is accepted by `resume` before T5 is recompiled. An undeclared seed policy uses a stable auditable default ensemble. `resources/` is an optional existing-material root, while `external_executor/expr/` holds only deployed runnable baseline or method assets. Human confirmation is required only for changing the T4.5-defined task, mechanism, required-baseline set, benchmark scope, or claim/contribution boundary. Full external execution must produce the six-file Writer Handoff: the reader-facing `executor_research_report.md`, `result_pack.json`, `executor_status.json`, `report/run_manifest.json`, and the Phase F facts and validation receipts. After evidence packaging, `writer-handoff` validates those files, their final assets, and their source hashes. ResearchOS accepts the frozen handoff only at `T5-EXTERNAL-WAIT` for the current Candidate, then derives T8 evidence/claim indexes and delegates to the existing full T8 pipeline. A mock dry run verifies the protocol chain only and does not create empirical claims. See the [T5 External Executor Guide](t5_external_executor.md) for commands, the material gate, and A-F artifact paths.
