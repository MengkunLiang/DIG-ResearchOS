...
1. **Mainline forward divergence** from synthesis, cards, user seeds, survey insights, and controlled free reasoning.
2. **Cross-domain bridge candidates** with transferable mechanism, source evidence, mapping, and migration risk.
3. **Coverage supplements**: mechanism challenge, reverse operation, subgroup failure, and missing-area exploration. They are checks for omitted angles, not four compulsory Idea templates.

Each supplement channel can be `unsupported`. A retrieval coverage gap is never silently promoted to a research gap.

Pass1 first persists raw candidates and distribution checks. Pass2 covers every Pass1 candidate with nearest work, grounding, routine risk, feasibility, contribution character, and recommendation without silently deleting it. Only after Pass1, Pass2, the structured pool, and the distribution check are persisted does runtime accept the Gate1 cards and selection brief. Each card presents a short title, innovation, two or three candidate hypotheses, merge opportunities, scoring rationale, supporting note sections, risk, and protocol boundary.

The CLI renders Gate1 through Rich: it first shows a compact candidate index with lane, innovation type, hypothesis count, evidence level, and Pass2 recommendation, then a full card for every candidate. The card includes the complete direction, innovation, H1/H2/H3, falsification, minimum validation, score rationale, risks, and paper-note paths. Long Chinese/English fields wrap inside table cells instead of being manually shortened or split character by character. H2/H3 drafts that still require source binding are visibly marked `proposed_not_verified`; that is a proposal status, not a claimed finding.

During generation, T4 writes a durable progress ledger and emits only bounded public events: context-pack preparation, per-candidate Pass1 persistence, supplement-channel state, Pass2 recommendation, persisted score snapshot, and Gate1-card completion. It never emits hidden model deliberation. The Gate1 selection is fingerprinted against the displayed pool; changed candidates force a new presentation, while `select`, `merge`, `new idea`, and `reanalyze` keep their own durable lineage records.

`minimum_experiment` is a candidate proposal. `supported` and `user_provided` details require `source_refs`; `proposed_not_verified` and `unknown` remain explicitly provisional. A metric such as AUUC/Qini is allowed when source-bound for the current project and must not be guessed because of the research topic.

T4's two machine-readable Gate1/decision artifacts use schema-checked writes. When a write is rejected, the terminal reports the exact object path and rule, not merely `schema_validation_failed`. Common requirements include complete counterfactual/novelty fields for every scorecard candidate, array-form decision reasons, controlled evidence-strength enums, and source references for supported protocol details. The rejected object is not written, so correction is deterministic and does not require deleting candidates or restarting T4.
