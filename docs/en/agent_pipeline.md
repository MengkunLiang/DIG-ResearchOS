...
| T4.5 research-plan audit and formalization | Does the selected Pre-Novelty idea remain differentiated, and can it become a coherent, falsifiable, quality-gated research package? | Phase 1 novelty/collision audit; Phase 2 begins from the compact Pre-Novelty brief, hypothesis brief, and audit, obtains only decision-relevant evidence fragments, then establishes `research_blueprint.yaml` before deriving `claim_registry.yaml` and `exp_plan.yaml`, formal claims and Proposal; Phase 3 orientation-aware review/repair and formalization receipt | Novelty human review only for a non-pass audit; T4's UTD/CCF-A/Hybrid orientation is inherited without another user choice |
