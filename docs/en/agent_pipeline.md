...
- `T3.6-ASSEMBLE` first creates `survey.tex`, then runs a deterministic audit. Use `audit-survey --workspace <workspace>` after a concrete repair to regenerate `survey_audit.md/json` without contacting a provider. Citation diversity is a hard release condition: at least 50% of the bibliography entries that are locally traceable, evidence-qualified, and deterministically linked to the saved survey plan must be represented. The audit excludes unlinked retrieval noise, produces a per-section note-review queue with evidence restrictions, and still applies a total-use-scaled repeat limit. It does not reject a 104-use survey solely because a legitimate citation is used 13 times.
...
Choosing one complete Candidate creates `ideation/hypothesis_brief.yaml`, lineage, search targets, and a Pre-Novelty brief. These artifacts are not an experiment authority. T4 moves directly to T4.5. Its audit runs in a dedicated context and owns collision labels, required baselines, and claim ceilings only. A pass then starts a fresh formalization context that creates the common `research_blueprint.yaml`, `claim_registry.yaml`, formal `hypotheses.md`, `exp_plan.yaml`, and the seven-section Proposal. A third, independent orientation-aware review/repair context checks the exact same template with UTD, CCF-A, or Hybrid weights. T5 is blocked unless all structured sources, Proposal, experiment mappings, and the accepted review agree.
