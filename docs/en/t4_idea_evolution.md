...
Checkpoint reuse is guarded by input and run-config fingerprints. The fingerprint includes project and literature inputs as well as `seed_ideas.md`, `seed_constraints.md`, the normalized `seed_outline_profile.json`, and `seed_external_resources.jsonl`. The latter two are supplied to the Opportunity Planner only as bounded, unverified context for scope and feasibility; they are never promoted to literature evidence. A changed input or confirmed config cannot silently be presented as the result of an older population.
...
`CandidateDossier` additionally normalizes the Genome's scientific package into `contributions` and `hypotheses` for downstream use. An evolved Candidate has one to four non-duplicative contributions and provisional hypotheses, each hypothesis carrying its mechanism, observable prediction, and discriminating test. T4.5, rather than a filler count, subsequently requires a linked challenge--component chain plus counterfactual, competing-explanation, and falsification coverage for every active claim. `CreativeContext` preserves the LLM-authored conceptual leap, competing explanations, surprising prediction, research-program potential, knowledge origin, evidence status, and required reading/validation upgrades.
