...
Normal execution shows only what a researcher needs to act on: the current activity, what completed, what is still needed, and the next action. High-volume Tools such as PDF text, web pages, command output, and structured records retain their full result in the run record while the terminal reports a page range, file, or status summary. Long text wraps naturally in Rich cells instead of using character-level ellipses. T4.5 Formalization creates its shared research contract in dependency order: `research_blueprint.yaml`, then `claim_registry.yaml`, then `exp_plan.yaml`. A blank contract, or a valid already-created prefix waiting for its next source, is rendered as initialization rather than a red repair failure; only an out-of-order partial package, a malformed source, or a cross-file inconsistency requires repair. For a provable compatible-provider transport defect, the runtime losslessly closes up to three outer object delimiters only after strings and arrays are complete, then still applies schema validation; it never inserts a field, string, array element, or research content. A truncation whose completeness cannot be proved remains paused and requires a compact reissue of the same source. T4.5 structured `data` should stay below roughly 5,500 serialized JSON characters so native tool parameters and provider output retain headroom. A model wait uses one in-place heartbeat line that prioritizes the current activity and completed public milestones rather than repeatedly printing the same line. Its elapsed time belongs to the visible logical phase rather than an individual provider request, so a retry in T4.5, T5, T8, or another long task does not restart the displayed clock. T4 retains its more granular controller-owned phase clock. A resumed process starts a new display clock, while valid persisted artifacts remain the only authority for deciding what may be skipped.
...
One request uses the same provider/model configured in `config/model_settings.yaml`. On timeout, connection interruption, 502/503/504, or temporary overload, the runtime applies the same-model recovery allowed by that file. With `retry_after_timeout: false`, reaching the full request deadline does not silently start another full wait. The terminal distinguishes a deadline without a reply from a rate or quota problem, rather than misreporting it as a context or validation error, then offers Retry submission, Retry after waiting, and Pause project. Retrying submission resends only the current request; it does not repeat completed Tools or artifacts. Waiting is excluded from effective Agent work time and does not consume a research step.
