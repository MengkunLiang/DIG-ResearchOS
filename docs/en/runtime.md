...
T4 normally creates 7-9 Gate1 idea cards: 3-5 mainline directions plus at most four supplements. Confirmed bridge coverage that must be visible may increase the total beyond nine; the system must not duplicate a mechanism merely to reach a count. Titles, mechanisms, H1/H2/H3, evidence interpretation, selection advice, risks, and score rationales are LLM-authored from paper reading notes. The runtime only validates the schema and lays out factual metadata in Rich. On `resume`, legacy or incomplete cards are returned to T4 for LLM reanalysis rather than rewritten with a template.
