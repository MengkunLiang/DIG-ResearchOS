...
For T4, model authorship remains the normal path for candidate framing, mechanisms, and H1/H2/H3. The terminal emits a low-frequency Live Runtime panel after 12 seconds and then every 30 seconds while a provider call is in flight. It shows only public candidate/channel activity, persisted Gate1 artifact progress, and the next visible milestone; it never displays model reasoning or hidden drafts.
...
T4 applies a separate compact-context optimization. It injects a lane-diverse set of high-quality note cards first, then writes every other usable card to `ideation/t4_evidence_pool.json`. Those cards remain available for directed `grep_search` and `read_file` when an idea needs a different mechanism, bridge, boundary, or counterexample. For a verified/backlog record that has no note yet, T4 can call `lookup_paper_record` by ID or title and use returned metadata/abstract only as weak idea fuel or a resource-acquisition lead. It may not promote that record to core mechanism or novelty evidence without a supported note/source.
