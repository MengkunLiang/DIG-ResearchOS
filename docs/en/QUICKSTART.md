...
Ordinary `resume` returning to T2/T3 first presents a lightweight Gate: T2 can confirm the current parameters or revise them, while T3 can continue the current queue, run a targeted T2 supplement, or revise parameters. Choosing continue does not start a new search or change scope. Migration and explicit re-entry deliberately reopen the full decision surface: `run --from <source> --start-task T2`, `resume --from <source> --from-task T2`, and `resume --from-task T2` open the full parameter chooser. Their T3 counterparts open the retrieval-coverage decision whenever a reading queue is preserved. A missing historical `search_log.md` or similar summary is explained in that Gate and does not suppress the decision. If no reading queue remains, ResearchOS returns to parameter selection so T2 can be rebuilt safely. None of these paths deletes papers, notes, or previous diagnostics.
