...
T3.6 section, assembly, and review repair are governed by source progress rather than a fixed number of retries. A failed audit supplies the Survey Writer with the precise check, the smallest editable section/plan/state/bibliography scope, and the applicable evidence boundary; the complete audit is rerun only after that scope changes. The runtime pauses when identical diagnostics recur without relevant source progress. For the small class of high-false-positive prose checks, an independent Semantic Adjudicator may assess only equivalent academic wording, such as necessary English technical terms in a Chinese manuscript, compact taxonomy phrasing, or clear bilingual review headings. It cannot modify text and must return one to three verbatim in-artifact quotations bound by SHA-256 to the manuscript, plan/state/template, or review/action/audit inputs. Citation coverage and alignment, BibTeX, provenance/fingerprints, internal runtime language, figures, LaTeX, and PDF compilation remain deterministic hard gates that semantic adjudication cannot waive.
