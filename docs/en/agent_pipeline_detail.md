...
T1 Project initialization
 ...
 -> T8-STYLE-GATE only after the complete Writer Handoff passes independent acceptance at `T5-EXTERNAL-WAIT`
...
The current main chain has deprecated the default semantics of “ResearchOS itself implementing and running long experiments internally during T5–T7”. ResearchOS is responsible for research protocol, executor selection, AGENTS/CLAUDE control instructions, file contracts, and the writing–evidence closed loop; Codex CLI, Claude Code window, human external executors, or mock dry-run are responsible for implementing and running experiments in isolated paths. The normal entry is `T5-REBOOST-GATE`, which deterministically compiles and validates `research-reboost` from the accepted T4.5 package; it does **not** make an LLM recreate the handoff. The separate `T5-SPECIALIZE-EXECUTOR-SKILLS` task then runs the repository `project-skill-specialization` Skill, atomically publishes the project-specific Suite, validates it independently, and records its input fingerprint before executor selection. `T5-HANDOFF` remains a legacy-compatible protocol compiler. The T5-to-T8 boundary is one **six-file Writer Handoff**: `external_executor/executor_research_report.md`, `external_executor/result_pack.json`, `external_executor/executor_status.json`, `external_executor/report/run_manifest.json`, `external_executor/report/phase_F/writer_handoff_facts.json`, and `external_executor/report/phase_F/writer_handoff_validation.json`. The report is the reader-facing summary, but it is not sufficient on its own. Final assets and raw/config/log evidence must be manifest- and hash-valid, and the handoff is accepted only while the state machine is waiting at `T5-EXTERNAL-WAIT`. Re-entering T4, T4-GATE1, T4.5, or an unrelated state rejects old handoff evidence so it cannot be attached to a newly selected Candidate. The old T7/T7.5 nodes have been removed from the main state machine, and ordinary `run-task T7` / `run-task LEGACY-T7-FULL` now fail with a removal message.
...
- A pass enumeration in T4.5’s `Final Gate Verdict`, such as `pass_to_experiment` / `pass_with_required_baselines`, authorizes `T4.5-FORMALIZE`; it does **not** jump directly to `T5-REBOOST-GATE`. T5 becomes available only after `T4.5-REVIEW` accepts the source-bound research package. `return_to_T4_reframe`, `drop_due_to_collision`, `reject`, `collision`, `fail`, missing verdict, or unrecognized verdict all lead to `T4.5-HUMAN-REVIEW`, where the user chooses to continue the external experiment chain, reframe back to T4, or end the project. The system no longer auto-rejects, auto-backtracks, or auto-approves, avoiding T4.5–T4 dead loops and preventing the model from making value judgments on the user’s behalf when novelty is uncertain. T7/T7.5 are no longer main-chain nodes.
- `T8` is not a single node, but a multi-node chain composed of style confirmation, resource indexing, alignment matrix, outline, per-section writing, assembly, self-check, review, and revision. It can ingest external evidence only after `T5-EXTERNAL-WAIT` accepts the hash-valid six-file Writer Handoff for the current Candidate. Any `next_task: T8` / `next_task: T8-WRITE` from old reports or gates will be safely mapped by the state machine to `T8-STYLE-GATE`, and will only directly enter `T8-RESOURCE` if a valid `drafts/writing_style.json` already exists.
...
- the external experiment chain reads existing handoff/result_pack/status/manifest artifacts, but T8 accepts them only as the complete, hash-valid six-file Writer Handoff at `T5-EXTERNAL-WAIT`; after acceptance, T8 reads the report, frozen facts, evidence indexes, and registered supporting `external_executor/` context
...
| `T5-EXTERNAL-WAIT` | `ExperimenterAgent` | `external_wait` | No-LLM Writer Handoff acceptance/recovery boundary; independently validates the report, state, manifest, source-bound facts, validation hashes, terminal status, and real empirical evidence before any T8 entry | `external_executor/wait_acceptance_report.json`, `drafts/t5_t8_handoff.json`, `drafts/experiment_evidence_pack.json`, `drafts/result_to_claim.json` |
...
| `retrieval_scope_plan` | `literature/retrieval_scope_plan.json` | T1's authoritative core/adjacent/Bridge retrieval scope |
...
5. Form `project.yaml`
6. Confirm `literature/retrieval_scope_plan.json` with core, adjacent, and true Bridge lanes; derive the narrow Bridge plan
7. Perform schema/semantic validation on `project.yaml`, retrieval scope, and derived Bridge plan
...
Afterwards, it conducts rounds of interviews via `ask_human`. Each `ask_human.question` must explain three things: which T1 round it is currently in, why the user’s answer is needed, and which fields the user should fill in. Draft confirmation and Bridge Domain Plan selection must directly include the `project.yaml` draft or candidate direction list in the `question`; they cannot just say “please confirm the above.” The three-layer scope confirmation uses a dedicated Rich view that explains the decision before showing vertical labelled blocks for core, adjacent, and Bridge candidates: core lines are required T2 coverage and the T3/T4 mainline; adjacent lines are selective formalization/comparison material; true Bridges are optional, cross-problem-domain targeted exploration. A must-explore Bridge receives minimum guaranteed exploration, whereas a should-explore Bridge is searched only when it adds value. For T1 scope and workflow-setting questions that already have a dedicated Rich page, the CLI does not first render the same Agent Markdown again in the generic stage panel; the source text remains in the trace and resumable human-interaction receipt, while the researcher sees one authoritative decision screen. The terminal renders long reference answers as a wrapped numbered list; ordinary text decisions show at least three editable answer examples, so an Agent suggestion must be a complete, directly submittable answer without its own number and must not turn internal state or missing information such as “no budget supplied” into an option. If the model still writes a short question relying on previous context, the runner will automatically incorporate the same-round Agent body text into the input question, preventing the user from seeing only an input box without seeing the draft/candidates. Typical rounds are:
...
| Scope and Bridge round | T2 needs a mainline literature scope as well as any genuine cross-domain transfer material | LLM separates core research lines, adjacent lines, and true Bridges; the user confirms, edits, adds, or skips the Bridge lane |

If the user provides paper entries, T1 first uses `process_seed_paper` to normalize them and write them into the seed file. The scope round derives a `retrieval_scope_plan` from the `project.yaml` draft, seed papers, and seed ideas. It separates core research lines, adjacent theory/method lines, and genuine cross-domain Bridges. A direct method stack, seed-paper extension, main baseline, or core theory must remain a core or adjacent line, never a Bridge. A Bridge needs a distinct source field, a structural mapping, and a falsifiable transfer question. The user confirms or edits all three lanes and may skip the Bridge lane without dropping the core scope. The Rich view renders each candidate as a vertical labelled block (title, role/priority, rationale, query, and boundary) so long prose cannot collapse into a misleading `—` row; it accepts headings with trailing role/priority metadata such as `**core1 · ...**（research_object · must_cover）`, T1 round-prefixed scope titles, abbreviated `核心线`/`邻接线` group headings, plain Chinese role parentheses, and inline `作用`/`检索`/`边界` fields that a model placed on one line. T1 writes `project.yaml` and `literature/retrieval_scope_plan.json`; the structured writer validates the scope and deterministically projects its confirmed `cross_domain_bridges` into the legacy-compatible `literature/bridge_domain_plan.json`. Thus `must_explore` is available only to genuine Bridges, while core `must_cover` lines guide normal T2 retrieval without consuming Bridge reservations. An empty Bridge projection is legal and leaves T2/T3/T4 free of Bridge-specific obligations. At closeout, `validate_outputs()` checks the project, scope, and derived Bridge contracts; if sensitive research directions are detected, ethical screening prevents completion.
...
Before the material-supplement gate, a new T1 workspace asks the researcher to choose **Copilot** or **Auto**. The Rich menu accepts `1` through `6` directly and also accepts commands such as `Auto survey_ccf deep` or natural language. Copilot leaves all normal research Gates to the researcher and persists no publication orientation: it does not silently mean CCF/CS. T4 asks separately for CCF/CS, UTD/IS, Hybrid, or a custom orientation before idea exploration begins. After either interactive mode choice, T1 shows a Rich default-settings confirmation for T2/T3 coverage, T4 effort, and the number of independent Proposal tracks. The three coverage choices are standard research `40/25/15`, review-balanced `80/40/40`, and review-intensive `90/40/50`, where the numbers mean retained candidates, deep reads, and abstract-level reads. The panel also explains that T4 effort changes Candidate formation/evolution breadth rather than research scope; `one` formalizes only the highest-ranked Candidate, while `top2` independently formalizes the first two before the researcher still chooses one at T5. `1` or `确认` accepts the displayed settings; a light LLM translates other free-form setting requests into a finite proposal which deterministic code validates. A parsed change creates a Rich unsaved preview: after input such as “two Proposals,” the UI displays the `top2` consequence and requires a second `1` or `确认` before writing `_runtime/workflow_mode.json`. The researcher may keep editing that preview, and the durable prior setting remains unchanged until that second confirmation. A clarification question or unrecognized text preserves the current preview, explains the selected mode, and re-prompts within the same run rather than being treated as a mutation or an immediate recovery pause. T2 identifies whether its recommendation came from a saved project default or project-material inference; T4 marks the saved effort as Enter's default and shows the resulting one- or two-Proposal strategy on its Candidate-selection screen. In Copilot these are future Gate recommendations only; they never make a human decision automatic. An Auto preset fixes the publication orientation and survey policy, while `quick`, `standard`, `deep`, or quality-aware `auto` controls T4 effort. After the default-settings confirmation, Auto + CCF/CS immediately presents a numbered menu of locally available conferences and persists the chosen concrete `template_id` (for example, `NeurIPS`) in `_runtime/workflow_mode.json`. T3.6 Survey and T8 reuse that same decision without asking again and never silently downgrade it to `basic_en`. Auto + UTD/IS explicitly persists its one supported INFORMS ISRE template (`template_family=utd, template_id=informs, writing_language=en`) instead of showing a pointless one-option Gate. Conference numbers, labels, and template IDs are finite local enums validated deterministically; the light LLM remains the bounded interpreter for free-form mode and coverage requests. T1 still completes seeds, research boundaries, the project draft, and retrieval-scope confirmation. Auto pre-authorizes only routine subsequent decisions; it never chooses the research question or scope, recovery action, failed novelty verdict, external side effect, or a materially changed research scope. A caller can preconfigure the same settings with `--workflow-mode auto --auto-preset research_utd --auto-t4-mode deep`, or change a future CCF template with `python -m researchos.cli configure-workflow --workspace <workspace> --ccf-template neurips`. The command changes only `_runtime/workflow_mode.json`; it does not delete or silently rerun a literature corpus, Population, Proposal, or manuscript. A noninteractive or `--yes` Auto + CCF/CS change without `--ccf-template` fails explicitly rather than saving an unresolved or basic template. Older silent defaults are deliberately treated as unconfirmed and are asked once.
...
1. **T1 source contract**: `literature/retrieval_scope_plan.json` is the authoritative researcher-confirmed manifest. It records core, adjacent, and genuine cross-domain lanes separately. The structured writer derives `bridge_domain_plan.json` only from the last lane. Thus `source=none + bridge_domains=[]` still means that T2/T3/T4 will not force cross-domain work, but it never removes the confirmed core scope.
  ...
  modes:
    read:
      behavior:
        abstract_sweep:
          enabled: true
          lite_paper_num: 15      # Standard research defaults to 25 deep reads + 15 light reads = 40-paper coverage; review profiles allocate their own totals; `all_readable` is explicit.
...
Survey template selection still uses its explicit two-level state-machine contract. The first level chooses the language/orientation family (Basic English, Basic Chinese, UTD/INFORMS, or CCF/CS); CCF/CS then routes to `T3.6-CCF-TEMPLATE-GATE`, which writes `template_family=ccf`, a concrete `template_id`, and `writing_language=en`. For Auto + CCF/CS, T1 has already captured the researcher's conference choice, so the runtime replays that same template through both nodes to retain normal receipts and validation without reopening a menu. A legacy or unresolved CCF profile is still asked here and is never bypassed with `basic_en`. Auto + UTD/IS reuses T1's explicit INFORMS default (`template_family=utd`, `template_id=informs`, `writing_language=en`). This survey layout choice is not a silent authorization of T4 publication orientation: T3.6 becomes a transparent suggestion source, while the T4 pre-run Gate still explicitly confirms the main research orientation.
...
The double-column layout rule is explicit: In CCF built-in templates such as ICML, NeurIPS, ICLR, KDD, a taxonomy overview using `width=\textwidth` must be placed inside `\begin{figure*}...\end{figure*}`; a regular `figure` may only use `\columnwidth`, `\linewidth`, or a clearly smaller width. A regular `figure` with `\textwidth` often won't cause a LaTeX compile error but will render over the right text column, so it is now blocked directly by the audit before review and compile. Single-column venues are not subject to this double-column rule. T3.6-VISUALS is a deterministic derived artifact: the runtime reads the taxonomy directly and uses the citation map to resolve citation/BibTeX keys, DOI, OpenAlex IDs, or note aliases to canonical local note cards; the model cannot choose whether this resolution is strict. When a taxonomy figure is actually generated, its visual manifest must contain only one figure, resolve every displayed direct paper link to a local note card, and contain no fabricated cross-paper performance graphic. When links are insufficient, the manifest honestly records `skipped`, produces no figure, and retains unresolved IDs as a coverage diagnostic; that is not a T3.6 failure. Resume re-evaluates historical `skipped` manifests without a model so improved citation mapping can produce the figure when warranted.
...
Proposal freshness is a content-lineage contract. `ideation/proposal/proposal_manifest.json` records `novelty_audit_sha256` and `proposal_sha256`; review-time prose edits are followed by a deterministic manifest refresh, while the accepted T5 lineage boundary verifies both digests. Filesystem `mtime` is intentionally not used as a hard gate because coarse timestamp resolution and deterministic rewrites can make semantically current artifacts appear older than the audit.
...
`T4.5-FORMALIZE` runs in a fresh `ResearchFormalizerAgent` context. It creates or repairs only the source artifacts needed to form one consistent research plan: `research_blueprint.yaml`, `claim_registry.yaml`, `hypotheses.md`, `exp_plan.yaml`, and a Proposal covering seven research functions. Those functions are a coverage framework, not a mandatory seven-heading layout; adjacent functions may be merged when that produces a more coherent academic argument. The runtime then deterministically derives the contribution-hypothesis map, validation map, kill criteria, research dossier, and proposal manifest. The Formalizer cannot promote planned results into empirical findings or overwrite the novelty audit.

The T4.5 repair ledger persists across `resume` and binds each diagnosis to the same task, selected Candidate, and upstream evidence. An unchanged source with the same diagnosis pauses immediately. A material change to the relevant research package or its evidence may receive another targeted repair, with no arbitrary cumulative retry ceiling.

Uncertainty in the Proposal and hypotheses must appear as research prose, not runtime metadata. A mechanism or contribution is framed as a reasoned proposal; a hypothesis is a falsifiable prediction with a competing explanation and discriminating test. An unresolved resource, dataset, or operational condition appears once in Risks, Limitations and Execution Plan as a concrete pre-execution check, mitigation, or fallback. Internal tokens such as `verification_required`, `proposed_not_verified`, evidence levels, T4.5, collision labels, and Candidate IDs must never appear in researcher-facing prose.

Every `write_structured_file` of the blueprint, claim registry, or experiment plan first validates that file's schema and then immediately reports whether the **three files together** pass their shared T4.5 contract. A saved file is therefore not presented as a completed formalization when a remaining claim mapping, design rationale, challenge-to-component link, or component ablation/mechanism-test coverage issue still blocks prose. The report lists every independent current failure, so the Formalizer repairs the minimal source sets together rather than discovering one blocked Proposal write per retry.

For the same reason, `researchos validate --task T4.5 --scope outputs` reports two distinct facts: `checks.outputs` is the novelty-audit node result, while `checks.t45_research_package` is the public full-package state. If the audit verdict passes but `t45_research_package.ok=false`, T4.5-FORMALIZE or T4.5-REVIEW still has work and T5 is not authorized. A non-pass audit correctly reports that formalization is not authorized yet; this is not a missing-file error.

The quality gate is deliberately two-layered. Schema, lineage, evidence permission, claim-to-experiment mapping, challenge-to-component coverage, component ablation or mechanism tests, and the novelty audit's Final Gate Verdict are deterministic hard gates: a semantic reviewer cannot waive them. Formalization sees advisory quality cues while it generates a complete source-consistent package, but repair-grade prose quality is assessed only in the separate Review context; the generator is not forced to repair the same quality condition before an independent reviewer can inspect it. A bounded, independent Semantic Adjudicator is used only when a failing check is genuinely about equivalent academic prose, for example a clear bilingual technical heading, a defined abbreviation, or a researcher-readable restatement of a structured claim. It is read-only, must cite one to three verbatim passages from the current artifacts, and binds its receipt to their hashes. Each failure injects the exact diagnostic and smallest source scope into the next Formalizer prompt. Repair is source-aware across resume: the same diagnosis with no source progress pauses instead of producing an uninformative loop, while a material source change remains eligible for another targeted repair.
...
Only at this final boundary does the CLI show the Rich **Research-plan Audit and Formalization Complete** table. It keeps five researcher-facing entry points: the Proposal, hypotheses, research-design constraints (blueprint plus claim registry), experiment plan, and final review/novelty boundary (orientation review plus novelty audit). Each row gives the path, the research constraint it defines, downstream use, and quality-gate state. Contribution/validation maps, kill criteria, the manifest, and formalization receipts remain available in the workspace for T5, `--verbose`, and recovery, but do not crowd the completion page. T5 reads the complete package; it is a plan and falsification contract, not experimental evidence.
...
- Runtime: deterministic T5 reboost compiler
- mode: `reboost`
- Contract: [skills/research-reboost/SKILL.md](../../skills/research-reboost/SKILL.md)
...
`T5-REBOOST-GATE` is the normal entry point of the external experiment chain. It does not write experiment code, run Docker, touch GPUs, select an executor, or treat model-generated natural language as experimental results. It compiles the handoff directly from the accepted T4.5 bundle; the normal pipeline and `run-task T5` do not ask an LLM to recreate a second handoff candidate. The compiler validates a private candidate against the handoff schema, then atomically publishes the validation receipt and executor controls before committing `handoff_pack.json` with `generation_status=completed`. A failed rebuild replaces any old completion marker with `blocked` and removes generated executor controls, so Resume cannot mistake a partial or stale handoff for authorization. Real-run authorization remains separately expressed by `execution_contract.execution_readiness`.
...
| `external_executor/report/reboost_llm_candidate_handoff_pack.json` / `external_executor/report/reboost_llm_candidate_validation_report.json` | Legacy compatibility diagnostics only when an explicitly supplied candidate is audited; they are not normal T5 outputs or downstream inputs |
...
`research-reboost` is the bundled contract used by the compiler, not a normal LLM-writing turn. The compiler reads `project.yaml`, the post-novelty formalization bundle, synthesis and comparison artifacts, and all three Paper Note roots; it deterministically derives the semantic handoff, validates a private candidate, and publishes controls only after that validation passes. It does not publish executor Skills: separating handoff compilation from Suite publication makes each artifact set independently resumable and prevents a partial suite from being advertised as ready.
...
`T5-EXTERNAL-WAIT` independently accepts the modern Writer Handoff: the executor selection, reader-facing report, result pack, executor status, manifest, `report/phase_F/writer_handoff_facts.json`, and `writer_handoff_validation.json`. The validation receipt must bind the current report, state, manifest, and facts hashes; terminal result/status must agree and be `completed` or evidence-bearing `partial`; the facts must contain source-bound comprehensive results; no `mock_only` or `dry_run` value is allowed. Referenced raw/config/log files, final figures/tables, and manifest checksums must also be valid and within `allowed_paths.txt`. On any failure, runtime writes `external_executor/wait_rejection_report.md` with typed issues and pauses recoverably. On success it writes `external_executor/wait_acceptance_report.json` and the ResearchOS-owned `drafts/t5_t8_handoff.json`, `drafts/experiment_evidence_pack.json`, and `drafts/result_to_claim.json`; a later T8 validator checks those derived artifacts again. The normal `research-execution` root invokes `run-task T8` itself after Writer Handoff. A stopped executor's `resume` uses the same acceptance path only as recovery; it never treats a report alone, a mock, a `blocked`/`failed` state, or a result-less partial package as permission to start T8.
...
The required T5-to-T8 evidence interface is the complete modern Writer Handoff package:
...
The report explains the work, while the structured facts bind the report to result state and source files. `raw_results/`, `evidence_package/`, `figure/`, `table/`, and `expr/` remain traceable supporting context. External process files are grouped by ownership under `report/phase_A/` through `phase_F/`; the manifest is the only cross-phase external file kept at the report root. `run-task T8` independently checks the modern schemas, terminal-status consistency, Writer Handoff fingerprints, core hashes, a source-bound empirical result, and manifest registration/checksums for every final figure/table. It then derives the ResearchOS-owned `drafts/t5_t8_handoff.json`, `drafts/experiment_evidence_pack.json`, and `drafts/result_to_claim.json` before entering or resuming `T8-STYLE-GATE`. A rejected handoff does not change `state.yaml`.
...
Template selection has two levels. The first menu chooses Basic English, Basic Chinese, UTD/INFORMS, CCF / CS conference, or custom. Choosing CCF / CS opens a second menu containing the conference templates actually detected locally. The current directory contains AAAI, ACL, ACM, CIKM, CVPR, ECCV, EMNLP, ICASSP, ICCV, ICDE, ICDM, ICLR, ICME, ICML, ICRA, IEEE, IJCAI, IROS, NAACL, NeurIPS, SIGIR, SIGKDD, SIGMOD, VLDB, WSDM, and WWW. When Auto + CCF/CS has selected a venue at T1, the runtime reuses it at both style nodes and retains the normal state-machine receipts; only a legacy or unresolved CCF profile pauses here for a conference. It never reinterprets CCF/CS as `basic_en`. An entry with an official `.tex` source is assembled from that source. A directory containing only a local `.cls`/`.sty` package uses an anonymous writing shell and records a pre-submission notice in `writing_template.json` or `writing_style.json`; it never silently falls back to a generic `article` template. UTD/Management Science/ISR/INFORMS uses T1's explicit `template_family=utd, template_id=informs, writing_language=en` default and the local official ISRE 2024 package. Use `basic_en` or `basic_zh` when only selecting language without applying a venue. To change future CCF output, use `configure-workflow --ccf-template <template_id>`; existing TeX is never silently rewritten.
...
- CLI input for `ask_human` is submitted with a separate line `END`, or with EOF (`Ctrl+D` on POSIX; `Ctrl+Z` then Enter on Windows); if an empty answer is accidentally submitted, the CLI retries up to 3 times; only consecutive empty answers lead to a recoverable pause.
...
T3.6 section, assembly, and review repair are governed by source progress rather than a fixed number of retries. A failed audit supplies the Survey Writer with the precise check, the smallest editable section/plan/state/bibliography scope, and the applicable evidence boundary; the complete audit is rerun only after that scope changes. The runtime persists this source-aware state across `resume`: an identical diagnostic without a relevant source change pauses immediately, while a material change to the literature basis, template, approved scope, or affected source remains eligible for another targeted repair. For the small class of high-false-positive prose checks, an independent Semantic Adjudicator may assess only equivalent academic wording, such as necessary English technical terms in a Chinese manuscript, compact taxonomy phrasing, or clear bilingual review headings. It cannot modify text and must return one to three verbatim in-artifact quotations bound by SHA-256 to the manuscript, plan/state/template, or review/action/audit inputs. Citation coverage and alignment, BibTeX, provenance/fingerprints, internal runtime language, figures, LaTeX, and PDF compilation remain deterministic hard gates that semantic adjudication cannot waive.
...
The 40 independently launchable public Skills now receive corresponding tool combinations through auditable capability profiles; two T5 state-machine modules and 13 external-executor templates are deliberately outside the `run-skill` directory. Each direct Skill has at least `workspace_navigation`: `list_files`, `glob_files`, `grep_search`; these are still constrained by the Skill's own `allowed_read_prefixes`, so they cannot read other projects just because they can list directories. Depending on the task type, the catalog also adds `literature_discovery`, `paper_acquisition`, `paper_curation`, `literature_processing`, `structured_artifacts`, `idea_analysis`, `claim_review`, `manuscript_planning`, `survey_workflow`, `tex_delivery`, or `external_handoff`. The repository-wide `researchos audit-skills --check-interactions` command independently checks all 40 direct interaction surfaces, two pipeline routes, and 13 executor-template routes; it is read-only and does not turn an audit into a new LLM Skill.
