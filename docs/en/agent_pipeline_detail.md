...
Before the material-supplement gate, a new T1 workspace asks the researcher to choose **Copilot** or **Auto**. Copilot leaves all normal research Gates to the researcher and persists no publication orientation: it does not silently mean CCF/CS. T4 asks separately for CCF/CS, UTD/IS, Hybrid, or a custom orientation before idea exploration begins. After an interactive Auto choice, T1 asks once more for the execution setup: the T2/T3 coverage preset and T4 effort. The three coverage choices are standard research `40/25/15`, review-balanced `80/40/40`, and review-intensive `90/40/50`, where the numbers mean retained candidates, deep reads, and abstract-level reads. An Auto preset fixes the publication orientation and survey policy, while `quick`, `standard`, `deep`, or quality-aware `auto` controls T4 effort. Direct input examples are `Auto survey_ccf deep`, `Auto survey_utd standard`, and `Auto survey_exhaustive_utd deep`; the second setup can use `确认`, `standard_research quick`, `survey_balanced standard`, or `survey_exhaustive deep`. T1 still completes seeds, research boundaries, the project draft, and retrieval-scope confirmation. Auto pre-authorizes only routine subsequent decisions; it never chooses the research question or scope, recovery action, failed novelty verdict, external side effect, or a materially changed research scope. A caller can preconfigure the same settings with `--workflow-mode auto --auto-preset research_utd --auto-t4-mode deep`. Older silent defaults are deliberately treated as unconfirmed and are asked once.
