  ...
  modes:
    read:
      behavior:
        abstract_sweep:
          enabled: true
          lite_paper_num: 10      # Standard research defaults to 15 deep reads + 10 light reads = 25-paper coverage; review profiles allocate their own totals; `all_readable` is explicit.
...
The T4.5 targeted-repair window does not become unlimited after `resume`. The runtime binds the same task, selected Candidate, and stable upstream evidence to `_runtime/t45_quality_repair_ledger.json`: an unchanged source with the same diagnosis pauses immediately, and a package that still fails after eight cumulative targeted repairs also pauses. Only a real change to the upstream research decision or evidence baseline, such as reframing and selecting a new Candidate in T4, opens a new repair window.
...
T3.6 section, assembly, and review repair are governed by source progress rather than a fixed number of retries. A failed audit supplies the Survey Writer with the precise check, the smallest editable section/plan/state/bibliography scope, and the applicable evidence boundary; the complete audit is rerun only after that scope changes. The runtime persists this source-aware window across `resume`: an identical diagnostic without a relevant source change pauses immediately, and eight cumulative targeted repairs under the same literature basis, template, and approved survey scope pause before another model call. A changed literature basis, template, or approved scope opens a new window. For the small class of high-false-positive prose checks, an independent Semantic Adjudicator may assess only equivalent academic wording, such as necessary English technical terms in a Chinese manuscript, compact taxonomy phrasing, or clear bilingual review headings. It cannot modify text and must return one to three verbatim in-artifact quotations bound by SHA-256 to the manuscript, plan/state/template, or review/action/audit inputs. Citation coverage and alignment, BibTeX, provenance/fingerprints, internal runtime language, figures, LaTeX, and PDF compilation remain deterministic hard gates that semantic adjudication cannot waive.
