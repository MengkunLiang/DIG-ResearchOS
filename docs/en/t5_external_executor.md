...
The same handoff is intentionally rejected if the workspace has been reopened to `T4`, `T4-GATE1`, T4.5, or another unrelated state. Re-selecting a Candidate creates a new research contract; retained T5 files are audit history, not evidence for that new Candidate. Finish the reselection and its new T4.5/T5 chain before an external executor creates a new Writer Handoff. Do not edit `state.yaml` to bypass this boundary.
