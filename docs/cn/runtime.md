...
人工 Gate 使用异步终端读取。因此，即使终端正在等待回答，`Ctrl+C` 等暂停请求也能被运行循环接收、持久化为可恢复状态并干净退出，而不会留下被输入阻塞的进程。多行回答在所有平台都可用单独一行 `END` 提交，提示会同时显示当前平台适用的 EOF 手势。
...
普通运行只展示研究者需要采取行动的信息：当前正在做什么、已经完成什么、还需要什么以及下一步。高容量 Tool（PDF 文本、网页、命令输出和结构化记录）仍把完整结果保存在运行记录中，终端只给出页码、范围、文件或状态摘要。长文本在 Rich 单元格中自然换行；不会使用字符级省略号截断半句话。T4.5 Formalization 按 `research_blueprint.yaml`、`claim_registry.yaml`、`exp_plan.yaml` 的依赖顺序建立研究契约。三者都缺失，或前一个已创建而后一个仍待创建，都显示为“正在初始化”，不是红色修复错误；乱序残缺、已有来源不一致、缺字段或跨文件不匹配才显示为需要修复。对于兼容 provider 可证明的传输缺陷，若原生工具 JSON 的字符串与数组均完整、仅缺少至多三个外层对象闭合符，runtime 会无损补齐后再做 schema 校验；它绝不补字段、字符串、数组元素或研究内容。无法证明完整性的截断仍会暂停并要求同一来源紧凑重发；T4.5 的结构化 `data` 建议不超过约 5,500 个序列化 JSON 字符，为工具外层参数和 provider 输出留出空间。模型等待会显示一条原位刷新的 heartbeat，优先保留当前活动和已完成的公开里程碑，不重复刷出同一行。等待时间属于研究者可见的逻辑阶段，而不是某一次 provider 请求，因此 T4.5、T5、T8 或其他长任务发生 retry 时，屏幕上的阶段计时不会重新开始。T4 保留由控制器管理的更细粒度阶段时钟。Resume 会启动新的显示时钟，而是否跳过工作仍只由已验证的持久化产物决定。
...
T3.6 与 T4.5 的确定性验证器仍负责必需文件、结构、标识符、来源、指纹等可复现事实。只有当自然语言标签或等价学术表述存在歧义时，运行时才会请求一次独立、可引用原文的语义复核。复核器只接收当前被质疑的文本，并返回简短的结构化结论；其放行记录与文本指纹绑定，绝不覆盖结构或证据方面的硬失败。这样既避免了关键词误判造成的反复改写，也不会放行缺乏依据的研究性主张。

Proposal 的可读性属于审阅质量而非格式硬门。系统保留少量清晰的一级标题，并提示模型在现实问题、计算困难、研究缺口、设计选择、检验与边界之间出现真实论证转换时另起完整段落。它不会为了通过检查生成 `###` 层级树、字段清单或机械空行。
...
T3 的终端验收分为两个明确层次：`[Validation] T3` 的首次提示只代表精读笔记、比较表和引用材料通过结构检查；随后必须完成摘要轻读覆盖，最终 Rich 面板中的“摘要轻读验收”才决定能否进入 T3.5。该面板还会分别显示 PDF 可得性、可升级阅读队列、metadata-only 分诊和关联资源目录。PDF 下载、metadata-only、代码仓库或项目链接都是访问/补资源线索，不会被显示为已读证据，也不会自动支撑 mechanism、causal 或 empirical claim。
