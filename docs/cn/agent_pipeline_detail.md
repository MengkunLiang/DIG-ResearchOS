...
- 会完整体现 `T5-REBOOST-GATE -> T5-SPECIALIZE-EXECUTOR-SKILLS -> T5-EXECUTOR-GATE -> T5-DRY-RUN/T5-EXTERNAL-WAIT -> T8-STYLE-GATE -> T8` 这样的链条
...
| `T5-EXTERNAL-WAIT` | `ExperimenterAgent` | `external_wait` | 无 LLM 的等待/恢复边界；检查外部执行器是否写回必需 T8 报告以及 supporting result pack/status/manifest 文件 | `external_executor/wait_acceptance_report.json`；下游必需输入 `external_executor/executor_research_report.md` |
| `T5-DRY-RUN` | `ExperimenterAgent` | `dry_run` | 用 mock executor 跑通 result_pack/status/manifest/raw/config/log 文件协议；显式 `mock_only=true`；不合成经验性 T8 claim | `external_executor/result_pack.json`, `executor_status.json`, `run_manifest.json`, `heartbeat.json`, `raw_results/`, `configs/`, `logs/` |
...
- `pass_to_experiment` / `pass_with_required_baselines`：进入 `T5-REBOOST-GATE`，由 `research-reboost` 编译并校验实验 handoff、发布 Skill Suite，随后再选择外部执行器；mock dry-run 和真实 Codex/Claude/manual 路径现在都会在外部执行边界后直接进入 `T8-STYLE-GATE`。
...
`T5-REBOOST-GATE` 是外部实验链的常规入口。它不写实验代码、不运行 Docker、不碰 GPU、不选择执行器，也不把模型生成的自然语言当实验结果。它读取受来源约束的 Pre-T5 artifacts，让已配置模型把研究意图和证据边界组织为完整 handoff candidate，再调用 `compile_research_reboost_handoff(handoff_pack=...)`。该工具按 handoff schema 校验 candidate、记录必要修复，并写出可审计的执行器契约。模型 candidate 无效时，确定性 recovery compiler 只能修复结构，不能补造缺失的科研协议事实。
...
`T5-EXTERNAL-WAIT` 读取 `external_executor/report/executor_selection.json`、`external_executor/executor_research_report.md`、`external_executor/result_pack.json`、`external_executor/executor_status.json` 和 `external_executor/run_manifest.json`。如果缺文件、报告为空、`semantics` 不正确、result/status/manifest 的 executor 与 selection 不一致、真实 executor 复用了旧 `mock_only/dry_run` result pack、status/current_state 不是 `done`/`COMPLETED`、metric 为空、引用的 raw/config/log 不存在、sha256 不匹配、路径不在 `allowed_paths.txt` 的 `rw` 范围，或真实执行没有非空 run/raw result 记录，runtime 会写 `external_executor/wait_rejection_report.md` 并抛出 recoverable pause，项目状态保持可 `resume`。外部 Codex/Claude/manual 执行器修复结果后，用户执行 `researchos resume`，runtime 会重新检查并写 `external_executor/wait_acceptance_report.json`，然后进入 `T8-STYLE-GATE`。这避免了“结果还没写完、证据不可审计，或从 mock 切到真实 executor 后误吃旧结果”的隐性错误。
...
T7 结果摄取、完整性审计、实验后 novelty 复核、result-to-claim mapping，以及 T7.5 PI 决策节点，已经从当前主状态机中移除。外部执行器总控 Skill 在完成所有子 Skill 调度和执行后，负责做最终 handoff 输入验证。当前主链是：
...
`external_executor/` 下的其他文件，包括 `result_pack.json`、`executor_status.json`、`run_manifest.json`、`raw_results/`、`configs/`、`logs/` 和 `expr/`，仍作为可追溯上下文供 T8 按需读取，但不能替代核心报告。当前 T5 gate 只定义这个接口并通过下游前置检查验证其存在，不在 ResearchOS 内部合成该报告。报告缺失或为空时，`T8-RESOURCE` 会被 task contract / prerequisite check 阻塞。

原 T7 审计职责仅保留为外部执行器总控 Skill 内的 handoff validation：所有子 Skill 完成后，总控 Skill 运行最终验证脚本，记录报告和 supporting external-executor 目录是否存在。它不写 manuscript artifact，也不批准论文。
...
每个被提升为 Evolved Candidate 的 Seed 都具有 **Idea Genome**：Problem、Opportunity、challenged Assumption、Core Thesis、Mechanism、Design or Artifact、Contribution Package、Draft Hypothesis Bundle、Validation Logic、Boundary、Risk 与 Evidence Map。Controller 会把相关 genome 组织为 **Idea Family** 与 sibling family，但不会把字面相似误判为科研等价。Family 记录共同的 mechanism structure、相关 Candidate ID 与需要保留的内容。`Gene Donor Map` 明确记录通过 Crossover 审核时每个 gene 由哪个 Parent 提供；`Gene Delta` 说明 Child 相对 Parent 改了什么；`Complexity Inflation` 用于识别范围、依赖、contribution 或 validation burden 的无理由膨胀。
