...
### 11.2 T4 Gate1：Rich 决策面板与候选假设状态

T4 Gate1 的 CLI 不再使用等宽字符手工截断的长文本 dump。它先渲染一个 Rich 索引表，用于比较候选 ID、通道、方向、创新类型、假设数、证据等级和 Pass2 建议；随后为每个候选渲染完整 Rich card。卡片包含完整方向、研究问题、创新、技术机制、H1/H2/H3、可观测预测、判别测试、最小验证、评分依据、风险、支撑文献和笔记路径。中英混排在 Rich 表格单元内按词和显示宽度换行，不再按字符切开英文单词，也不使用展示层 `...` 替代正文。

每个 Gate1 可展示候选必须由模型写出 2-3 条彼此不同的候选假设；每条都包含命题、机制、可观测预测、判别测试和 `evidence_status`。`proposed_not_verified` 表示该命题是明确的待验证提案，不是由展示层补上的占位文本；`supported`、`proposed_not_verified` 与 `unknown` 都必须忠实反映证据状态。任何候选若只能写出一条 H1、缺少机制链或只能靠模板补 H2/H3，必须回到 T4 重分析，不能进入 Gate1。
