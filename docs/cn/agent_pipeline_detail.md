...
> [中文](../cn/agent_pipeline_detail.md) | [English](../en/agent_pipeline_detail.md)
...
| 是否适合完整交付 | 最适合 | 不适合 |
| 是否适合测 `T5 external execution -> T8` | 是 | 否 |
...
| `T3` | `ReaderAgent` | `read` | 逐篇精读并形成结构化证据（含 §13 Mechanism Claim、§14-§19 CDR 和 abstract A/B 桥接字段） | `deep_read_notes/`, `shallow_read_notes/`, `metadata_triage.md`, `comparison_table.csv`, `related_work.bib` |
...
| `T5-EXTERNAL-WAIT` | `ExperimenterAgent` | `external_wait` | 无 LLM 的 Writer Handoff 接收/恢复边界；在进入 T8 前独立核验报告、状态、manifest、带来源 facts、validation hash、终态和真实实证结果 | `external_executor/wait_acceptance_report.json`、`drafts/t5_t8_handoff.json`、`drafts/experiment_evidence_pack.json`、`drafts/result_to_claim.json` |
| `T5-DRY-RUN` | `ExperimenterAgent` | `dry_run` | 用 mock executor 跑通 result_pack/status/manifest/raw/config/log 文件协议；显式 `mock_only=true`；不合成经验性 T8 claim | `external_executor/result_pack.json`, `executor_status.json`, `external_executor/report/run_manifest.json`, `heartbeat.json`, `raw_results/`, `configs/`, `logs/` |
...
| `T8-STYLE-GATE` | runtime gate | `style_gate` | 状态机级 immediate gate，确认 IS/CCF-A/both 写作风格、语言和 LaTeX 模板，并记录人工交互 provenance | `drafts/writing_style.json` |
| `T8-RESOURCE` | `WriterAgent` | `resource_index` | 索引写作资源，消费 Pre-T5 材料、`external_executor/executor_research_report.md` 和 supporting external executor artifacts，并生成章节/证据/图表计划和对齐矩阵 seed | `drafts/manuscript_resource_index.json`, `drafts/section_plan.json`, `drafts/evidence_plan.json`, `drafts/figure_table_plan.json`, `drafts/alignment_matrix.json` |
...
| T3 abstract-only 桥接字段 | `abstract_sweep.py` 和 `reader.j2` 要求 abstract-only note 写 `## A. Core Approach / Perspective` 与 `## B. Bridge Point`；`reader.py` 对 `shallow_read_notes/` 以及 `deep_read_notes/` 中 `[ABSTRACT-ONLY]` note 都做结构校验，并兼容旧中文标题读取 | 已落地 |
...
| T4 原生多路径生成，并兼容 Pass 1/Pass 2 | 默认 Controller 依次执行 Evidence Routing、Opportunity Map、多路径 P0 formation、独立评分、O0 offspring 与 P1/Portfolio selection，并把结果投影为既有状态机继续使用的 Pass 1/Pass 2 Gate1 artifacts。只有成功接纳的 Child 改变 Population 时才 union re-score；否则保留已实际取得的 Parent 独立评分，并写入明确的复用回执，既不合成分数，也不额外调用 provider。初始 Route 可提交显式 conjectural 的最小 `IdeaSeed`，随后再补全；Route/评分/Child 的局部失败会落盘诊断、修复或降级，不能取消其他 Candidate。`researchos/prompts/ideation.j2` 只保留为显式 legacy fallback，绝不是默认 T4 路径，也不能覆盖 native Artifact。 | 已落地 |
| T4 四类 constraint route 保持补充定位 | `mechanism_challenge`、`reverse_operation`、`subgroup_failure`、`gap_exploration` 是原生 Population 中明确的覆盖 route。它们可以发现有价值的边界、反事实或失败模式，但不能替代 evidence-routed Literature 与 informed-brainstorm 主线；`_candidate_directions.json` 区分 `mainline/supplement/bridge/not_supported_by_current_evidence`。 | 已落地 |
...
- LLM connection：全局唯一的 `api_base` / `api_key` / `model`
- 主要工具：
  - `read_file`
  - `write_file`
  - `ask_human`
...
随后它会通过 `ask_human` 分轮访谈。每个 `ask_human.question` 必须说明三件事：当前处于 T1 第几轮、为什么需要用户回答、用户应该补哪些字段。草案确认和 Bridge Domain Plan 选择必须把 `project.yaml` 草案或候选方向清单直接写进 `question`，不能只写“请确认以上”。三层范围确认会以专用 Rich 页面先解释决策含义，再按核心、邻接、Bridge 三层显示纵向字段块：核心线是 T2 必须覆盖、T3/T4 主线使用的材料，邻接线用于选择性形式化/比较，真正 Bridge 是可为空的跨问题域定向探索；重点交叉保底探索，普通交叉仅在有增益时检索。对于这类已有专用 Rich 页的 T1 范围和工作流设置问题，CLI 不会先在通用阶段面板重复渲染同一份 Agent Markdown；原始内容仍写入 trace 和可恢复的人机交互记录，用户只看到一份权威决策页面。终端会把长的参考回答渲染为可换行的编号列表；普通文本决策至少显示三条可编辑的回答示例，因此 Agent 提供的 suggestion 应是可直接提交的完整回答，不带自编号，也不能把内部状态或“未提供预算”等缺失信息当作一个选项。如果模型仍写了依赖前文的短问题，runner 会把同一轮 Agent 正文自动并入输入问题，避免用户只看到输入框却看不到草案/候选。典型轮次是：
...
在材料补充 Gate 之前，新建 T1 workspace 会先让研究者选择 **Copilot** 或 **Auto**。Rich 菜单可直接输入 `1` 至 `6`，也兼容 `Auto survey_ccf deep` 这类命令或自然语言。Copilot 保留所有普通科研 Gate 供研究者确认，并且不持久化任何投稿取向；它不等于默认 CCF/CS。进入 T4 前会单独询问 CCF/CS、UTD/IS、Hybrid 或自定义取向。无论交互式选择 Copilot 还是 Auto，T1 随后都会以 Rich 面板确认一次默认执行设置：T2/T3 文献覆盖、T4 探索力度和独立 Proposal 数量。三种覆盖档分别是标准研究 `40/25/15`、综述均衡 `80/40/40`、综述强覆盖 `90/40/50`，三个数字依次表示保留候选、精读和摘要级轻读。面板还会解释：T4 探索只控制 Candidate 形成/演化力度，不改变研究范围；`one` 只正式化排序最高的 Candidate，`top2` 将前两条分别正式化为独立 Proposal，之后仍由研究者选择哪一条进入 T5。`1` 或“确认”接受当前显示的设置；自由表达先由轻量 LLM 翻译为有限枚举提案，再由本地代码校验。一个已解析的修改只会生成 Rich 未保存预览，例如输入“两个 Proposal”后系统显示 `top2` 的实际含义；必须在下一屏再次输入 `1` 或“确认”才写入 `_runtime/workflow_mode.json`。在该确认前可继续描述修改，且磁盘上的原设置不变。输入的是澄清问题或无法映射的文字时，系统会保留当前预览、说明已选模式后在同一轮重新询问，不会将它当作修改或直接暂停。T2 会明确说明推荐来自项目默认设置还是项目材料推断，T4 会将保存的探索力度标成回车默认，并在 Candidate 选择页提示一份或两份独立 Proposal 的后续策略。Copilot 中这些只是后续 Gate 的建议，绝不会令人工决策自动通过；Auto 预设才会决定写作取向与综述策略，`quick`、`standard`、`deep` 或质量感知的 `auto` 决定 T4 力度。Auto + CCF/CS 在默认设置确认后会立刻展示本地可用会议的编号菜单，研究者选择一个具体 `template_id`（例如 `NeurIPS`）；该决定写入 `_runtime/workflow_mode.json`，供 T3.6 Survey 和 T8 复用，既不在后来重复提问，也绝不静默降级为 `basic_en`。Auto + UTD/IS 则明确持久化唯一受支持的 INFORMS ISRE 模板（`template_family=utd, template_id=informs, writing_language=en`），因此不增加只有一个选项的 Gate。会议名称、模板 ID 和编号是有限本地枚举，优先由确定性解析核验；模式和覆盖等自由表达仍可由轻量 LLM 受限翻译。T1 仍会完成种子、研究边界、项目草案和检索范围确认，不会被跳过。Auto 只预授权后续常规决策，不会替用户决定研究问题或范围、恢复操作、失败的新颖性 verdict、外部副作用，或发生实质变化的研究范围。也可以在命令行预设，例如 `--workflow-mode auto --auto-preset research_utd --auto-t4-mode deep`；修改未来 CCF 模板使用 `python -m researchos.cli configure-workflow --workspace <workspace> --ccf-template neurips`。该命令只更新 `_runtime/workflow_mode.json`，不会删除或静默重跑既有文献库、Population、Proposal 或论文；在非交互或 `--yes` 的 Auto + CCF/CS 更新中，缺少 `--ccf-template` 会明确失败而非写入空模板。历史版本静默写入的默认值会被视为未确认，并在 T1 首次进入时补问一次。

中文 Auto 另提供 `research_zh`（中文研究论文）和 `survey_zh`（中文综述）两个预设：前者使用 `basic_zh` 模板并按研究论文规模检索中英文文献，后者在同一中文写作与中英文检索策略下进入 Survey 支线。它们不冒充 CCF/CS 或 UTD/IS 投稿取向；T4 默认采用均衡研究评估，后续仍可由研究者另行指定目标 venue。来源、证据与引用的一致性检查和其他文献一样在后台执行，不作为前台额外选择项。

### 单独运行 vs 完整运行

- 单独运行 `run-task T1` 只会完成初始化文件，不会自动进入 T2
- 完整运行 `run` T1 成功后会自动进入 T2
...
它是 T3.5、T4 会继续用到的重要输入，不是可有可无的附带产物。 但它不是人工精读后的“真实研究空白”结论，只能作为补检和复核线索。
...
`seed_outline_profile.json` 来自用户 Markdown 种子提纲。它的 `framework`、`sections`、 `query_profile` 和 `representative_literature_directions` 只作为 query/taxonomy prior； `representative_literature_directions` 不是已验证 citation，不能写入 `seed_papers.jsonl`， 也不能在 T3.6/T8 中直接引用。
...
如果 inspection 发现真实 Markdown seed outline，而 `user_seeds/seed_outline_profile.json` 尚不存在，Scout 会调用 `normalize_seed_outline`，runtime 也会在 prompt 渲染前做一次兜底规范化。 综述 profile 下，query 设计必须覆盖中英文检索、管理/IS/OR、human-AI decision-making、 AI governance/model risk management、XAI/fairness/accountability 和中文“智能算法风险/ 管理决策/算法治理”等角度。当前没有 CNKI/万方官方 API；中文文献覆盖不足时应写入 `search_log.md`/`missing_areas.md`，并要求用户提供中文 PDF、DOI 或题录作为 seed。 EU AI Act、NIST AI RMF、ISO/IEC 标准和中国算法治理法规属于 external resources / official-source verification 线索，不是 scholarly paper，不应进入 `papers_dedup.jsonl`。
...
- `access_score_estimate` 基于 metadata 估计可读性
- `access_level_hint` 基于 arXiv、PDF URL、DOI、abstract、本地 PDF 等可验证 metadata 生成的可读性 hint，例如 `LIKELY_FULL_TEXT` / `POSSIBLE_FULL_TEXT`
- `evidence_level` Reader 最终阅读状态。T2 阶段不再根据 access score 推断 `FULL_TEXT` / `PARTIAL_TEXT`；没有 Reader 覆盖记录时只保守写 `ABSTRACT_ONLY` 或 `METADATA_ONLY`，并标记 `_needs_reader_evidence_level`
...
当前核心参数优先来自 workspace-local `literature/literature_params.json`，该文件由 `T2-PARAM-GATE` 写入；没有该文件时才回退到 `config/system_config/agent_params.yaml -> agents.reader.modes.read.behavior`。Gate 面向用户使用三层语义：保留候选数、精读篇数、摘要轻读篇数。
...
7. **T3 bridge 笔记路径**：`save_paper_note` 会把未通过 core screen 的 `bridge_deep` 论文写入 `literature/bridge_notes/{bridge_id}/{id}.md`；通过 core screen 的论文仍写入 `literature/deep_read_notes/{id}.md`，避免同一论文在主线和 bridge 目录重复阅读。T3 manifest、T3 recovery、T3.5 workbench、T8 resource index 都会同时扫描 `deep_read_notes/` 与 `bridge_notes/`，并统一过滤 `_DIR_GUIDE.md`、README、template、example、空占位文件。
8. **T4 上桌和逃生舱**：有 confirmed bridge 且 `source!=none` 时，T4 必须检查 `cross_domain_catalogs/{bridge_id}/` 的检索上下文、`bridge_notes/{bridge_id}/` 中实际已读的 Bridge note、主线 note 的 `contributed_bridges`、`synthesis_workbench.bridge_transfer_drafts`。catalog-only 轨道可以提出结构类比、反例、验证问题和阅读升级，但不能伪装成已读机制证据。有足够依据时生成 `idea_origin=bridge_synthesis`、`constraint_status=bridge`、`cross_domain_sources=[...]` 的候选，并放入 Gate1 可见池；没有足够依据时不能硬编已证实 idea，必须在 `ideation/bridge_coverage_review.json` 写 `escape_hatch.status=no_candidate_available`、证据缺口和回访条件。
...
10. **Gate1 语义**：进入候选池不等于进入 `hypotheses.md`。`bridge_synthesis` 候选和被 Pass 2 不推荐的候选都必须在 `_candidate_directions.json` 与 `_gate1_selection_brief.md` 可见；研究者可以选择、并行保留、组合、重构、延后或拒绝。完成选择后只会生成 Pre-Novelty brief；必须经过 T4.5 的 novelty/collision audit，才会创建正式 hypothesis 与 experiment bundle。
...
T3 与其他阶段共用 `config/model_settings.yaml` 的 provider/model，不再受用户维护的 step/token/wall budget 表控制。PDF Tool timeout、输出 validator、workspace 权限、provider timeout/retry 以及持久化的 deep-read queue 仍会约束执行，并保证恢复路径可复核。
...
| 输出 key | 文件 | 含义 |
| --- | --- | --- |
| `deep_read_notes_dir` | `literature/deep_read_notes/` | 每篇论文一份结构化笔记；包含 `Reading Coverage` 阅读覆盖记录 |
| - | `literature/shallow_read_notes/` | abstract sweep 补读的精简笔记（可选，由 orchestrator hook 自动生成） |
...
新写入的详细笔记采用 Note Card schema v2。除既有的 problem、approach、claim/evidence、mechanism、design rationale、boundary、limitation 与 cross-paper tension 字段外，§20 还记录 Scientific、Engineering/Deployment，以及仅在确有依据时才写的 Practical/Managerial/Business implication。每条 implication 都标记为 `author_stated`、`system_inference`、`weak_inference` 或 `not_applicable`，并保留来源或推理边界。理论论文只写 Scientific implication 完全合理；系统绝不为了字段完整而编造商业价值。已有 version-1 note 仍可用于 resume，新保存的 note 则会在写入边界检查 v2 字段。

详细 Markdown note 才是证据记录。面向用户的 Compact View 只展示 Problem、Mechanism、Finding、适用的 Implication 与 Evidence Status；它只是导航入口，不能替代完整 note 或其中的页码/section 锚点。
...
10. 用 `save_paper_note(queue_rank=..., content=...)` 保存完整 markdown；工具自动生成 `deep_read_notes/{id}.md`、即时校验并刷新 `notes_manifest.json`

普通文本工具 `read_file` 的默认分页大小会按本轮 agent 的模型上下文窗口动态调整； 显式传入 `max_chars` 时以显式值为准。PDF 正文读取仍以 `extract_pdf_text` 返回的 `preview_truncated_by_max_chars`、`complete_pdf_read` 和页码覆盖为准，不能因为单次 文本预览没有截断就假设整篇 PDF 已读完。

### 实际执行过程

`ReaderAgent(read)` 进入时会先列出 `literature/deep_read_notes/`，再读取或刷新 `literature/notes_manifest.json`；如果存在 `literature/deep_read_queue_pending.jsonl`，就按 pending queue 的 queue rank 继续，否则读 `deep_read_queue.jsonl`，再回退到 `papers_verified.jsonl`，最后才回退到 `papers_dedup.jsonl`。处理每篇论文前，它优先用 `lookup_paper_record(queue_rank=...)` 取该论文 metadata；只有没有 queue rank 的旧 workspace 才用 `paper_id` / `title` 查找。已有 note 只有在结构、Evidence 锚点和 `Reading Coverage` 都合格时才算完成。`notes_manifest.json` 会把每条队列记录标为 `complete`、`incomplete` 或 `missing`：同名 note 存在但缺章节时是 `incomplete`，不会再被误报成“没读”。
...
任何 `[ABSTRACT-ONLY]` note（无论在 `deep_read_notes/` 还是 `shallow_read_notes/`）还必须包含：
...
- 根据已有且通过结构校验的 `deep_read_notes/` 自动裁出 `deep_read_queue_pending.jsonl`
...
- `deep_read_notes/` 中历史重复 stub 或坏 note 不再直接拖死整体验证；只有结构合格 note 会计入完成数。若合格数量/queue 覆盖不足，validator 会区分 `missing` 与 `incomplete`：如果 note 已匹配但缺 `Claims vs Evidence`、`Key Quotes`、CDR 等结构，会直接列出文件名和缺失章节
...
- `related_work.bib` 存在
- 如果 `shallow_read_notes/` 存在，每篇 abstract note 必须通过结构校验（5 节 + §13，Evidence type = `abstract_claim_hint`；旧产物的 `claimed_untested` 兼容但不推荐）
...
- 跳过 `deep_read_notes/` 已覆盖的论文，匹配使用 ID、canonical_id、DOI、arXiv、title overlap 等多 key，不只看文件名
- 跳过 `shallow_read_notes/` 已覆盖的论文
...
产出：
- `literature/shallow_read_notes/` — 含 abstract 候选的精简 note（5 节 + §13 Mechanism Claim）
...
`build_synthesis_workbench` 会在 `literature/synthesis_workbench.json` 中保留 `notes`（deep/bridge notes）、`shallow_read_notes`（粗读 notes）和 `all_note_cards`（合并卡片）。`build_manuscript_resource_index` 会在 `drafts/manuscript_resource_index.json.paper_note_cards` 中写入同样的 section 摘录，并在每个 `drafts/section_outlines/*.md` 的 Note Card Retrieval Plan 中列出当前 section 最相关的 1-2 条 section cues。`IdeationAgent` 也会把这些 section cues 作为紧凑摘要注入 prompt，避免只依赖 `synthesis.md` 的概括。

后续阶段读取 note cards 的协议：

1. **先读结构化索引，再回查原始 note**。T3.5/T3.6/T4 优先读 `literature/synthesis_workbench.json.all_note_cards`；T8 优先读 `drafts/manuscript_resource_index.json.paper_note_cards` 和当前 `drafts/section_outlines/<section>.md` 的 Note Card Retrieval Plan。索引只提供摘录和路由；真正写 claim 或引用前必须回查 `path` 指向的原始 `.md` section。
2. **按 section 取字段，不按标题关键词猜用途**。Introduction 主要看 `problem_motivation/gaps/boundary_conditions`；Related Work 看 `method_overview/core_approach_view/bridge_point/gaps/design_rationale/cross_paper_tension`；Methodology 看 `method_overview/core_approach_view/design_rationale/artifact_design`；Experiments 看 `key_results/reading_coverage/data_view`；Analysis 和 idea generation 看 `mechanism_claim/design_rationale/boundary_conditions/cross_paper_tension`。
3. **区分证据等级**。`evidence_level=FULL_TEXT/PARTIAL_TEXT` 且 citation quality 足够的卡片可支撑正文 claim；`ABSTRACT_ONLY` 卡片只能作为 coverage hint、bridge hint、resource-upgrade hint 或 idea fuel。`raw_abstract` 只帮助理解 abstract-only 候选，不能替代全文 evidence。
4. **T3.6 survey 使用方式**。Survey plan 和 section writer 应把 `all_note_cards` 作为 taxonomy class、comparison stream、challenge 和 future agenda 的证据路由表；taxonomy/comparison 不应只复述 synthesis.md，而应回查代表卡片的 A/B、§13、§14、§18、§19。
5. **T4 idea 使用方式**。IdeationAgent 会收到紧凑 note-card 摘要。候选 idea 的 `source.idea_origin` 可以来自 `evidence_driven`、`mechanism_challenge`、`cross_domain_analogy` 或 `bridge_synthesis`，但如果依赖 abstract-only card，必须在 provenance 中标明 low-evidence/upgrade-needed，不能把它写成已验证机制。
6. **T8 写作使用方式**。Writer 每个 section 的 outline 会列出相关 note-card cues；写作时先用 cues 定位候选，再打开原始 note section 核对 sentence-level claim 和 citation key。不要因为 note card 出现在索引里就自动引用；必须看 `citation_use`、`citation_quality_score` 和 `evidence_level`。
...
| `deep_read_notes_dir` | `literature/deep_read_notes/` | 是 | T3 的逐篇笔记 |
...
| `synthesis_workbench` | `literature/synthesis_workbench.json` | 从 deep_read_notes 抽取的结构化证据、方法家族、frontier、趋势和问题候选 |
...
- 单篇笔记（`deep_read_notes/*.md` + `shallow_read_notes/*.md`）
...
工具内部逻辑：
1. 解析 `deep_read_notes/*.md`、`bridge_notes/**/*.md` 和 `shallow_read_notes/*.md`，提取每篇 note 的标题、年份、方法概述、关键结果、局限、问题、§13 Mechanism Claim、§14-§19 CDR 字段、abstract A/B 桥接字段和 Raw Abstract 摘录；deep/bridge notes 写入 `notes`，粗读 notes 写入 `shallow_read_notes`，合并索引写入 `all_note_cards`
...
- `deep_read_notes/*.md` / `shallow_read_notes/*.md` 的 `## A. Core Approach / Perspective`
- `## B. Bridge Point`
...
T3.5 没有像 T3/T5 那样复杂的专门恢复文件，但它天然是 artifact-first 的：

- 只要 `deep_read_notes/`、`comparison_table.csv` 还在
- 重跑 T3.5 的成本主要是重新综合
...
| `deep_read_notes_dir` | `literature/deep_read_notes/` | deep-read 证据 |
| `shallow_read_notes_dir` | `literature/shallow_read_notes/` | abstract-only 桥接提示，不能当 FULL-TEXT 证据 |
...
综述模板选择仍按两级状态机契约处理：第一层选择正文语言/取向（Basic English、Basic Chinese、UTD/INFORMS 或 CCF/CS），CCF/CS 后进入 `T3.6-CCF-TEMPLATE-GATE` 写入最终 `template_family=ccf`、具体 `template_id` 和 `writing_language=en`。对 Auto + CCF/CS，具体会议已在 T1 默认设置确认后由研究者选择；runtime 会在这两级节点复用同一个 `template_id`，保持正常 receipt 和校验，但不再次弹出菜单。旧 workspace 或未确认具体会议的 CCF profile 仍在这里询问，绝不以 `basic_en` 自动答案跳过。Auto + UTD/IS 会复用 T1 明确持久化的唯一 INFORMS 默认（`template_family=utd, template_id=informs, writing_language=en`）。该文件是综述排版选择，不会替代 T4 的 publication orientation 确认；T3.6 的人工选择只作为 T4 建议来源，T4 Gate 仍会明确询问并确认主研究取向。
...
- 图是否符合 taxonomy-only visual manifest，且在已选双栏 CCF 模板中不会跨栏覆盖正文；失败项名为 `survey_graphics_layout`
...
如果当前环境缺少 `latexmk`/`tectonic` 等可用 PDF backend，`latex_compile` 会返回 `waiting_environment_*`，runtime 会暂停；安装 backend 后可 resume。如果是 citation 或 LaTeX 语法错误，应读 log 后修对应 section，再重新 assemble/compile，而不是一口气重写整篇 survey。
...
| 输出 key | 文件 | 含义 |
| --- | --- | --- |
| `pre_novelty_brief` | `ideation/hypothesis_brief.yaml` 与 `ideation/selected/` | 已选 Candidate、Draft Hypotheses、谱系、证据边界与 T4.5 search scope；不是可执行 protocol |
| `formal_hypotheses` | `ideation/hypotheses.md` | 正式 post-T4.5 hypothesis bundle；只有 novelty/collision audit 通过后才会创建或更新 |
| `exp_plan` | `ideation/exp_plan.yaml` | 正式 post-T4.5 experiment plan；由 T5 编译为外部执行契约 |
...
| `gate_decisions` | `ideation/gate_decisions.json` | Gate1 的用户反馈、选择/淘汰和决策理由，并保留可迁移的历史决策记录 |
...
### Legacy Pass1/Pass2 兼容投影参考

以下详细字段参考为 legacy artifact reader 与 migration diagnosis 保留。native T4 的真实执行以 11.2 节的 Population workflow 为准：构建 Evidence Index 与 Opportunity Map，经非对称 Route 形成 P0，使用职责分离的 scoring，在 Standard mode 完成 P0->P1，之后才投影兼容文件。这些 legacy-shaped 文件不能在 T4.5 之前授权正式实验。

1. 读取 `synthesis.md`
2. 将 native T4 的非对称 Route 结果投影到 legacy 主线候选字段。历史上的 `3-5` 仅用于解释旧 Artifact；native T4 使用配置的 Route quota 与 Population size。
...
12. 选择后的输出：
   - `hypothesis_brief.yaml` 与已选 Candidate 的 lineage，供 T4.5 使用
   - 正式 `hypotheses.md` 和 `exp_plan.yaml` 只会在 T4.5 audit 通过后生成
   - `idea_scorecard.yaml`
   - `rejected_ideas.md`
   - `gate_decisions.json`
   - `idea_rationales.json`
   - `risks.md`
   - 条件产物：如果 T1 已确认非空 bridge domain plan，还必须写 `bridge_coverage_review.json`，记录 Bridge 候选是否被考虑、escape hatch 决策，以及已选 Candidate 在提出 strong claim 前是否需要补读。

### 历史兼容流程

本节说明历史 Pass1/Pass2 projection 的字段级行为，并不代表 native T4 有第二个 Gate，也不授权生成正式执行 Artifact。当前流程以 11.2 节的 Population lifecycle 为准，pre-mortem 只是可记录的 T4 限定复核。
...
4. **生成候选方向**：
   - 主线候选：从 Workspace 材料分别获得 evidence-routed Literature 分配和独立的 Brainstorm 分配。native Route quota，而不是固定的历史数量，决定尝试生成多少个 Seed。
   ...
   - native controller 保留完整 Archive，维持配置的 Active Population，并展示 1-3 个 Candidate Portfolio。主线候选不能被四类补充候选覆盖掉。
   ...
   - `cross_domain_sources` / `cross_domain_relation`（如果使用 `bridge_transfer_drafts`、`domain_map.theory_bridge`、`cross_domain_catalogs/`、`bridge_notes/` 或 bridge-domain plan 作为类比素材，必须记录对应 `bridge_id` 数组和关系类型；未使用时为空数组；旧 `cross_domain_source` 单数只作兼容 alias）
...
10. **Gate1 暂停**：T4 不会在同一次 LLM run 内写入正式 hypothesis 或执行计划。runtime 校验 native Population Artifact 及其 Pass1/Pass2 兼容 projection 后，返回 `completion_mode=t4_gate1_ready` 并转入 `T4-GATE1`。
...
12. **更新决策链**：T4 resume 读取 `_gate1_user_selection.json`，更新已选 Candidate、评分/决策历史和 Pre-Novelty brief，然后将 Candidate 送入 T4.5。只有 T4.5 audit 通过，才会编译正式 hypothesis 与 experiment plan。
...
如果用户选择 routine risk 或 `reject_recommended` 候选，T4 不能把它当作成熟 Candidate 直接送入 T4.5。系统必须先重构 CDR tuple、mechanism、prediction、counterfactual 和 contribution character，独立评分后再次在 Gate1 展示；原始风险保留在 Archive 与决策历史中。
...
2. **历史字段清单**：旧 Workspace 可能带有以下文件；native T4 仅在 T4.5 通过后生成正式版本：
   ...
   - `ideation/exp_plan.yaml`：正式实验计划（schema 校验）
   - `ideation/idea_scorecard.yaml`：所有候选 idea 的完整记录
   - `ideation/rejected_ideas.md`：被淘汰 idea 的可读原因
   - `ideation/gate_decisions.json`：Gate1 决策日志和可迁移的历史决策
   - `ideation/idea_rationales.json`：legacy 的机器可读 rationale projection
   - `ideation/risks.md`：Top 3 风险

3. **当前确认方式**：Gate1 确认 Candidate 选择或 Human-composed Candidate。T4.5 处理 novelty/collision 结论，并决定是否允许 formalization。

4. **完成**：T4 以绑定 fingerprint 的交接进入 T4.5；在 audit 通过前，正式 Artifact 不会被改动。
...
- `gate_decisions.json` 记录 Gate1 决策与可迁移的历史决策；native T4 不要求第二个 Gate2。
...
2. `literature/deep_read_notes/*.md` 的 §13 Mechanism Claim：找 `evidence_type` 为 `claimed_untested` 或 `empirical_correlation` 的机制
3. `literature/shallow_read_notes/*.md` 的 §13：abstract-only 笔记（`abstract_claim_hint`，引用时标注 "based on abstract only"）
...
- `literature/deep_read_notes/*.md` 的 §2 Method Overview
...
**数据来源**：
- `literature/deep_read_notes/*.md` 的 §5 Limitations
...
- 后续执行与写作阶段会按这些 anchor 追踪正式 hypothesis

### T4 如何记录 idea 的依据

native T4 用四层保存证据链。下面列出的旧 `idea_scorecard.yaml`、`rejected_ideas.md` 与 `idea_rationales.json` 仍可读取，作为 compatibility projection；它们不能替代绑定 fingerprint 的 Population Artifact。

#### 层1：Pre-Novelty brief — 已选 Candidate 的依据

`hypothesis_brief.yaml` 中的每条 Draft Hypothesis 都有：
- **背景**：为什么这个假设重要
...
#### 层2：Candidate dossier 与 Population snapshot — 完整候选记录

native Candidate dossier 与带版本的 Population snapshot 记录所有 Candidate，不只记录选中项。`idea_scorecard.yaml` 镜像适用的 legacy 字段，每项包括：
...
- `source`：来源追溯
  ...
  - `constraint_status`：`mainline` / `supplement` / `bridge` / `not_supported_by_current_evidence`
  - `cross_domain_sources`：如果 idea 使用了 `bridge_transfer_drafts`、`domain_map.theory_bridge`、`cross_domain_catalogs/`、`bridge_notes/` 或 bridge-domain plan，填对应 `bridge_id` 数组；否则为空数组
...
#### 层3：Archive — 未进入 Portfolio 或暂缓的原因

Archive 和可选的 `rejected_ideas.md` compatibility projection 会用人能读懂的方式说明：
- 为什么 pass 掉其他方向
- 什么条件下可以重访
- 最接近的已有工作是什么

#### 层4：Round Artifact 与 legacy 决策 projection — 机器可读依据

- `ideation/evidence/`、`ideation/evolution/`、`ideation/scoring/` 与 selected lineage 会保存 Evidence Permission、Gene Delta、评分、survival 与已选来源的 provenance。
- `gate_decisions.json` 和 `idea_rationales.json` 为 legacy reader 保留兼容的 decision/rationale 字段；正式 hypothesis mapping 只在 T4.5 后完成。

这四层让 T4.5、T5 与 T8 可以回看 Candidate 为什么存在、哪些证据能够支持它，以及其他 Candidate 为什么被暂缓或归档。
...
| `hypothesis_brief` | `ideation/hypothesis_brief.yaml` | 是 | Gate1 选择后生成的 Candidate、Draft Hypotheses、Evidence boundary 与 T4.5 search scope |
| `selected_candidate` | `ideation/selected/selected_candidate.json` | 是 | Gate1 选择、并绑定 fingerprint 的 Candidate |
| `hypotheses` | `ideation/hypotheses.md` | 否；仅通过后生成 | 只有 T4.5 formalization 通过后才创建的正式 hypotheses |
| `synthesis` | `literature/synthesis.md` | 是 | 文献综合 |
| `comparison_table` | `literature/comparison_table.csv` | 否 | 已有方法对比表 |
| `idea_scorecard` | `ideation/idea_scorecard.yaml` | 可选 compatibility input | legacy Candidate projection；native T4.5 会优先读取 selected Candidate 与 lineage |

### 输出文件

| 输出 key | 文件 | 含义 |
| --- | --- | --- |
| `novelty_audit` | `ideation/novelty_audit.md` | 针对已选 Candidate 与 Draft Hypotheses 的 novelty/collision audit |
...
- `continue_to_t5`：接受风险，继续进入外部实验链入口 `T5-REBOOST-GATE`。
...
T4.5 的定向修复状态会跨 `resume` 持久化。runtime 将同一 task、已选 Candidate 和上游证据绑定到 `_runtime/t45_quality_repair_ledger.json`：同一诊断且关联 source 未改变会立即暂停；只要相关研究包或证据发生实质变化，便可继续下一次定向修复，不设置任意累计次数上限。这样既保留必要的多文件定向修复，也避免没有任何修改的 resume 空转。
...
| `external_executor/report/reboost_report.json` | 发布与来源使用回执；指向 handoff 和 validation report |
| `external_executor/report/reboost_validation_report.json` | research-reboost validator 结果；成功的 T5-REBOOST 要求 `valid=true` |
| `external_executor/paper_card_evidence_index.json` | 紧凑的 Paper Note 与 Cross-domain catalog 索引，记录允许/禁止的证据用途 |
| `external_executor/expected_outputs_schema.json` | 外部执行器必须写出的 T8 报告、result/status/manifest/artifact 字段与文件 |
...
如果 reboost outputs 与其 fingerprint 仍有效，`resume` 会保留它们，而不会重建 handoff。专属化节点会单独检查已发布 Suite 的完整 source/template fingerprint 与独立校验结果；只有两者仍匹配时才复用，否则会原子重建。formalization 变更后需要重新整理研究意图时，应从 `T5-REBOOST-GATE` 重新进入；只需重建过期 Suite 时，应从 `T5-SPECIALIZE-EXECUTOR-SKILLS` 重新进入。用户要切换真实执行器时，应回到 `T5-EXECUTOR-GATE` 重新选择，而不是在后续 ingest 阶段改写结果来源。
...
- `codex_cli`：进入 `T5-EXTERNAL-WAIT`；Codex 在 workspace 根目录启动，runnable method/baseline 资产部署在 `external_executor/expr`；
- `manual`：进入 `T5-EXTERNAL-WAIT`，由人工或其它执行器写回结果；
- `revise_handoff`：返回 `T5-REBOOST-GATE`，只写入 deferred selection marker。

最终确定执行器选择后，runtime 写 `external_executor/report/executor_selection.json`，字段包括 `selected_executor`、`real_experiment_allowed`、`requires_user_copy_paste`、`next_state`、`selected_by`、`selected_at` 和 fallback，同时写 `external_executor/report/executor_capabilities.json`。随后 runtime 调用确定性 patch，把 AGENTS/CLAUDE/README（若存在）中的 `dry_run: UNSET`、`mock_only: UNSET`、`real_experiment_allowed: UNSET` 以及 executor placeholder 替换成真实值。对当前 research-reboost handoff pack，它会在 `execution_contract.authority_rules` 下追加 executor-selected authority rule；只有 legacy handoff pack 才会写顶层 `executor` 与 `execution_mode`。

交互细节：CLI 交互式运行时，在 `T5-EXECUTOR-GATE` 直接回车会选择默认的 `codex_cli`；选择 `codex_cli` 后必须再输入 `yes` 才允许真实实验，否则会降级为 `claude_code_window` 并把降级原因写入 `external_executor/report/executor_selection.json.notes`。非交互 stdin/管道关闭时不会默认选 mock，也不会默认继续，而是把项目暂停为可 `resume` 状态，避免“没有真实用户输入却推进实验”的隐性错误。
...
`T5-EXTERNAL-WAIT` 会独立接收现代 Writer Handoff：执行器选择、研究者可读报告、result pack、executor status、manifest、`report/phase_F/writer_handoff_facts.json` 与 `writer_handoff_validation.json`。validation receipt 必须绑定当前报告、状态、manifest 与 facts 的 hash；result/status 终态必须一致且为 `completed` 或带实证结果的 `partial`；facts 必须包含可追溯的 comprehensive results；不得出现 `mock_only` 或 `dry_run`。引用的 raw/config/log、最终 figure/table 和 manifest checksum 也必须正确，并位于 `allowed_paths.txt` 授权范围内。任何失败都会写入带 typed issue 的 `external_executor/wait_rejection_report.md` 并可恢复暂停。成功时会写入 `external_executor/wait_acceptance_report.json`，以及 ResearchOS 所有的 `drafts/t5_t8_handoff.json`、`drafts/experiment_evidence_pack.json`、`drafts/result_to_claim.json`；后续 T8 validator 会再次检查这些派生产物。正常的 `research-execution` 根 Skill 会在 Writer Handoff 后自行调用 `run-task T8`。执行器已经停止时，`resume` 只可作为同一接收路径的恢复入口；它绝不会把仅有报告、mock、`blocked`/`failed`，或没有结果的 partial 包当作启动 T8 的许可。
...
`T5-DRY-RUN` 是协议联调节点，不是真实实验。它证明外部执行器 handoff、result pack、status、run manifest、raw result、config、log、heartbeat 这些文件协议能端到端跑通。所有输出必须带 `dry_run=true` / `mock_only=true`，后续 claim audit 会阻止它们被写成论文实证 claim。成功后会回到 `T5-EXECUTOR-GATE`，不能进入 T8。
...
| `external_executor/executor_status.json` | 执行器状态；`accepted=false`，handoff validation/T8 后续决定写作接收度 |
| `external_executor/report/run_manifest.json` | raw/config/log artifact 列表及 sha256 |
...
已经持久化的扩展或旧 Workspace 仍可能保留完成后的 `T7.5` 节点，并使用 `next_on_success: __parse_from_output__`。系统保留仅供旧状态恢复的 `t75_human_review_gate` 来展示这份已保存的决策。Resume 不会重新启用已移除的内部执行器，而是读取保存的 `evaluation/evaluation_decision.md`。其中旧的 `T5`、`T6`、`T7` 建议会映射到当前外部实验入口；`T8`、`T8-WRITE` 建议会先经过 `T8-STYLE-GATE` 与 `T8-RESOURCE`，只有已存在合法写作风格记录时才跳过风格 Gate。

T5 到 T8 的必需证据接口是完整的现代 Writer Handoff 包：
...
研究报告解释工作内容，结构化 facts 则把报告与结果状态和源文件绑定。`raw_results/`、`evidence_package/`、`figure/`、`table/` 与 `expr/` 仍是可追溯副输入。外部过程文件按所有权放入 `report/phase_A/` 至 `phase_F/`；manifest 是唯一保留在 report 根目录的跨 Phase 外部文件。`run-task T8` 会独立检查现代 schema、终态一致性、Writer Handoff 指纹、核心文件 hash、可追溯实证结果，以及全部最终 figure/table 的 manifest 登记与 checksum；随后生成 ResearchOS 所有的 `drafts/t5_t8_handoff.json`、`drafts/experiment_evidence_pack.json` 和 `drafts/result_to_claim.json`，再进入或恢复 `T8-STYLE-GATE`。handoff 被拒绝时不会修改 `state.yaml`。
...
- `researchos run-task T5 --workspace ...`：报 retired，并提示使用 `T5-REBOOST-GATE`。
- `researchos run-task T6 --workspace ...`：报 retired；旧 Pilot 后 novelty 路径不再属于当前主链。
- `researchos run-task T7 --workspace ...`：直接以 removed 报错，并指向 T5 external-execution -> T8 路径。
- `researchos run-task LEGACY-T7-FULL --allow-legacy --workspace ...`：直接以 removed 报错；旧内部完整实验节点不再是受支持的兼容入口。
...
模板选择分两层：第一层只选择英文基础、中文基础、UTD/INFORMS、CCF / CS 会议或自定义；选择 CCF / CS 后，第二层会列出当前本地实际检测到的会议模板。当前目录包含 AAAI、ACL、ACM、CIKM、CVPR、ECCV、EMNLP、ICASSP、ICCV、ICDE、ICDM、ICLR、ICME、ICML、ICRA、IEEE、IJCAI、IROS、NAACL、NeurIPS、SIGIR、SIGKDD、SIGMOD、VLDB、WSDM 和 WWW。Auto + CCF/CS 已在 T1 确认具体会议时会沿这两级 Gate 自动复用该选择，保留正常 state-machine receipt；旧 workspace 或未确认会议时才在这里请求研究者补选，它不会把“CCF/CS”偷换成普通 `basic_en`。带官方 `.tex` 入口的条目会直接用于组装；只有 `.cls`/`.sty` 的本地模板包会生成匿名写作外壳，并在 `writing_template.json` 或 `writing_style.json` 写入提交前核对官方入口的提示，不会悄悄回退到普通 `article`。UTD/Management Science/ISR/INFORMS 使用 T1 明确持久化的 `template_family=utd, template_id=informs, writing_language=en` 和本地官方 ISRE 2024 模板包；只想确定语言、不套 venue 时使用 `basic_en` 或 `basic_zh`。之后需要改动未来模板可运行 `configure-workflow --ccf-template <template_id>`，已经产生的 TeX 不会被静默改写。
...
| 其余本地 `ccf/*` | 是，二级会议清单 | 有 `.tex` 入口时直接组装；只有 class/style 包时生成匿名外壳 | 同左 | 复制同目录 class/style/bst，以及需要的 preamble | 扫描并复制已引用的 support files |
...
`T8-SEC-EXPERIMENTS` 只写 `drafts/sections/experiments.tex`。Writer 读取 `alignment_matrix.json` 的 experiment 列、`external_executor/executor_research_report.md`、`external_executor/raw_results/`、configs/logs、`ablations.csv`、seed ensemble、`exp_plan.yaml` 和 `figure_table_plan.json`。
...
`T8-SELF-CHECK` 读取 `paper.tex`、`manuscript_audit.md`、`craft_audit.md`、`alignment_matrix.json`、`external_executor/executor_research_report.md` 和 `related_work.bib`，写 `drafts/self_check.md`。
...
Reviewer 先读取 `paper_state.json`、`sections/*.tex`、`paper.tex`、`manuscript_audit.md`、`craft_audit.md`、`alignment_matrix.json`、`self_check.md`、`external_executor/executor_research_report.md` 和 `.bib`。它对 7 个 section 逐章生成：abstract、introduction、related_work、methodology、experiments、analysis、conclusion。没有独立 `limitations.md` review；Conclusion review 必须检查 Limitations 子段。
...
T9 使用 `config/model_settings.yaml` 的全局 provider/model 与 retry 策略，不再有用户维护的 step/token/wall budget。`config/system_config/agent_params.yaml` 中的 `submission.behavior.max_compile_attempts` 是 LaTeX 编译 attempt/cache 的系统行为参数，不是普通 LLM budget；匿名化 pre-hook 由 `submission.behavior.enforce_anonymization_precheck` 显式控制。
...
T9 进入 LLM 前会先做编译环境 preflight：如果本机已有 `latexmk` 或 `tectonic`，可以直接 生成真实 PDF 和日志；如果没有可用 backend，任务会以 `WAITING_ENVIRONMENT` 暂停， `state.yaml` 保持 `PAUSED`，安装 TeX backend 后可直接 `resume`，不会先消耗 LLM 步数再失败。

例外：runtime 会在环境 preflight 和 LLM 前先尝试 `t9_submission_prefinalize`。如果 `submission/bundle/` 已有 `main.tex`、`references.bib`、`main.pdf`、`main.log`、 `bundle_manifest.json`，且 `submission/compile_report.json` 与当前文件 hash/mtime/size 全部一致，`SubmissionAgent.validate_outputs()` 会直接通过并跳过环境检查和 LLM。这个 prefinalize 不是单纯复用旧 PDF：`bundle_manifest.json` 必须证明 bundle 来源仍对应当前 `drafts/paper.tex` 和 `literature/related_work.bib`。
...
4. 尝试编译
   ...
   - `latex_compile` 的 `auto` 顺序是本机 `latexmk`、本机 `tectonic`、显式允许时的 Docker backend、否则等待环境或 `export_only`
   - `export_only` 只表示已导出 TeX/BibTeX，不是 PDF 编译成功证据
...
`validate --task T9` 现在也复用 `SubmissionAgent.validate_outputs()`，不是只看 `bundle/` 和报告文件是否存在。
...
跨 workspace 重跑不是 `resume`。如果旧 workspace 的 T2 出了问题，但 T1、用户 seed 和 bridge plan 仍可信，应创建新 workspace 并从 T2 启动完整主链：
...
省略 `--start-task` 时，`run --from` 默认从 `T2` 开始。它只复制目标 task 的前置输入， 不会复制旧 T2 输出；目标 workspace 已有 `state.yaml` 时会拒绝覆盖。单 task 调试才用 `run-task <TASK> --from <workspace>`。

## 7.2 Runtime Recovery Gate

当前系统不再把普通 ResearchOS step/token 计数当成面向用户的运行上限。Runtime recovery 用于可恢复的 provider、environment、validation、tool 和人机输入不可用中断。旧 workspace 可能仍保存 bounded-budget 恢复记录；这些记录只作为兼容诊断读取，除非开发者显式启用 bounded override。

主要覆盖：

- provider/runtime 中断
- environment readiness
- output validation 与 artifact validation
- unavailable human input
- legacy bounded-budget 恢复记录
- tokens
- wall_seconds
...
- Human Gate 等待时间会从 active runtime accounting 中扣除，避免“等用户输入”看起来像 agent 工作时间
- 新运行统一使用 `config/model_settings.yaml` 的 provider/model，不施加普通 ResearchOS step/token 上限。step/token/cost 仍会记录，便于追踪；provider context window、provider timeout/retry、Tool timeout、Docker/TeX 专用 timeout、workspace 权限、输出校验和项目级实验预算检查仍然生效。`config/system_config/agent_params.yaml` 是工具、权限、prompt 与 behavior capability 的系统声明，不是研究者日常配置入口。
- 旧 workspace 可能仍保存提到 `max_steps` 或 `max_tokens` 的恢复记录；当前 runtime 只把它们当兼容诊断，除非开发者显式启用 bounded budget override。
- 如果进程异常退出导致 `state.yaml` 停在 `RUNNING`，`resume` 会把最近一次 run 标为 `INTERRUPTED` 并自动转回 `PAUSED` 后继续，避免“当前状态不是 PAUSED/WAITING_HUMAN， 无法 resume”的死状态。
...
- `docker_exec` 和 `latex_compile` 不再受普通工具 `max_tool_call=180s` 的小上限截断： `docker_exec` 使用 `global_timeout.docker_operation`，`latex_compile` 使用 `global_timeout.latex_compile`。

## 7.3 LLM Connection 与 Recovery

唯一面向用户的 LLM 文件是本地 `config/model_settings.yaml`，可由 [model_settings.example.yaml](../../config/model_settings.example.yaml) 创建，也可运行 `researchos configure-llm` 生成。它声明一个 `provider`、`api_base`、`api_key`、`model` 和 `fallback` 区块。

- 所有 Agent 和 Skill 共用同一对 provider/model，不存在 primary/secondary model chain，也不再区分 heavy/medium/light routing。
- `fallback.max_attempts`、`initial_wait_seconds`、`max_wait_seconds` 与 `retry_after_timeout` 只会在临时故障后重试同一条连接。
- 认证错误、URL 错误和 model 配置错误不会被误当成服务拥堵反复重试，CLI 会直接指向 `configure-llm`。
- provider recovery 耗尽后，workspace 保持可恢复状态，终端走正常的 retry、延迟 retry 或 pause 决策；`resume` 从已落盘 artifact 继续。
- 普通 CLI 隐藏 secret 和 SDK trace；更详细的非 secret 诊断留在 `_runtime/logs/researchos.log` 与 trace。

## 7.4 MCP

`config/mcp.yaml` 是唯一的可选 MCP 配置文件。对于每个启用的 `transport: stdio` server，ResearchOS 会在启动时运行声明的 command、初始化官方 MCP session、发现 Tool schema，并在整个 run 中保持进程连接。普通 stdio server 不需要自定义 connector。已发现 Tool 默认对所有 Agent 和 Skill 可用；在单个 server 条目中设置 `allowed_agents` 可限制范围。`--mcp-connector` 只作为非 stdio transport 的高级兼容入口保留。

MCP 可增加外部能力，例如特定来源的检索服务、仓库浏览器或机构数据库 adapter。它是可选项：内置论文检索、PDF 获取、解析和结构化论文阅读笔记不依赖 MCP。MCP 输出与其他 Tool result 一样保留证据边界：metadata 或检索命中不会自动成为科学 claim 的全文证据。

## 7.5 Skills

Skill 由 `skills/<name>/SKILL.md` 的 frontmatter 发现，并在运行前接受严格的输入/输出/权限契约检查。`list-skills`、`browse-skills`、`describe-skill`、`run-skill` 与 `skill-status` 共用同一套 session 和 workspace 语义；Claude 风格工具别名只在兼容层翻译，`strict_tools: true` 的公共 Skill 若声明工具未注册会在启动前明确失败，不会静默降级为无效能力。

当前公共 Skill 分为原子能力和集成工作流两层。原子能力包括 PDF/标识解析、section 证据、文献比较、证据矩阵、citation library、gap map、写作、审稿、润色、编译和提交检查；集成工作流包括 `domain-synthesis-studio`、`literature-comparison-studio`、`literature-review-studio`、`survey-evidence-package`、`cross-domain-idea-studio`、`paper-reading-workbench`、`research-landscape-report`、`related-work-builder` 与 `draft-evidence-repair`。准确目录以 `researchos list-skills --workspace <workspace>` 的实时输出为准。

guided intake 不再只能等待上传。`pdf-note-card`、`paper-comparison` 与 `literature-comparison-studio` 可在用户明确提供 DOI、arXiv/OpenAlex ID、直接 PDF URL、精确标题，或“主题 + 目标篇数”时调用受限来源解析工具。下载只允许写入 `user_inputs/<skill>/` 的声明路径；每次尝试都会写 `_source_resolution.md`，记录用户输入、检索/下载工具、落盘路径、结果和访问限制。metadata、搜索命中和摘要不会因此升级为 section 级或全文证据。

暂停语义是确定的：材料未齐时提示 `[1] 继续收集缺失材料` 与 `[2] 暂停并保留会话`，输入 `1`/“继续”才进入下一轮；输入 `2`/“暂停”立即把 session 保留为 `WAITING_INPUT` 并返回 shell。intake Agent 只能通过 `ask_human` 发问，不能用普通自然语言输出伪造一个未调用工具的提问。
...
这一节记录容易在演示、恢复和长任务中出现的高影响行为，避免把“某个 Agent 看起来在重复”误解成普通模型不稳定。
...
T3.6 的 section、assemble 和 review 校验遵循来源进展而不是固定修复轮次：每次失败都会把 audit 中的具体检查、最小可修改的 section/plan/state/bibliography 范围和证据边界注入 Survey Writer；只要相关来源确有变化，就重新执行完整校验。该来源感知状态会跨 `resume` 持久化：相同诊断再次出现且相关来源未变时立即暂停；只要文献基础、模板、已确认范围或受影响来源出现实质变化，便可继续下一次定向修复，不设置累计次数上限。对少数高误判风险的自然语言检查，系统可使用独立 Semantic Adjudicator：仅限中文稿中保留必要英文术语导致的语言统计、compact taxonomy content 的等价学术表述，以及 review 中清晰的中英维度标题。裁决器不能修改文本，必须给出当前 artifact 内 1--3 条逐字证据；回执写入 `_runtime/t36_semantic_adjudications.json` 并绑定 `survey.tex`、plan/state/template 或 review/action/audit 的 SHA-256。引用覆盖、citation alignment、BibTeX、来源/指纹、内部运行术语、图表、LaTeX 和 PDF 编译永远是确定性硬门，不能由语义裁决放行。
...
正式运行前，同一确认界面还会询问 Publication Orientation。ResearchOS 会复用 `project.yaml`、已有 writing style 和可配置的 venue profile，建议 `UTD / INFORMS`（theory and phenomenon）、`CCF / CS`（technical and computational）或 `Hybrid / Cross-disciplinary`；选择运行 mode 后直接回车即可接受建议，也可以用一句自然语言覆盖。这些是可配置的内部研究评价 lens，并非对任何 venue 当前官方审稿规则的声明。确认后的 profile 写入 `ideation/t4_target_profile.json` 和 `ideation/t4_run_config.json`，因此会参与 T4 configuration fingerprint 和 resume。它只改变 Opportunity/Generation/Scoring 的侧重点以及 final card 的叙事顺序，绝不改变来源事实、Evidence Permission、citation 或 Candidate lineage。

Evidence Routing 会召回主线和 Bridge 中所有允许阅读层级的笔记。Evidence Permission 决定被召回的信息可以承担什么角色：全文/部分全文笔记可以在已读范围内锚定机制或设计理由；abstract-only 材料可以扩展召回、提供 taxonomy 或 Bridge 线索、提示候选机制并触发 reading upgrade，但不能确认已建立机制、强边界 Claim、可执行实验或外部 novelty。Controller 会在 `ideation/evidence/` 保存 Evidence Index、Opportunity Map、按 Route 划分的 Evidence Bundle、permission warning、来源路径、阅读等级和 fingerprint。缺少足够依据的 Route 会保留为 `unsupported`；Cross-domain/Bridge 还会写入 escape-hatch decision，而不是编造迁移关系。

证据约束的是认证，不是想象。Evidence Index 用于 grounding 与校准，并不是 Generator 可以提及的机制清单。在每一条正常 Route 中，Generator 都可以通过通用学术知识、反事实推演或结构性跨域类比提出非显然假设。它必须在 `CreativeContext` 中保留概念跃迁、竞争解释、反直觉预测、研究纲领潜力、来源以及验证/阅读升级需求。由参数知识或类比得到的内容始终是 `conjectural` 且需要验证；即使它对发现很有帮助，也不能因此变成来源、已证实机制、结果、已确认数据集、指标或新颖性结论。

Generator、Scorer 和 Evolver 的权限彼此分离。Generator 只产出按 Route 划分的 Seed 和 Candidate 文案，不评分也不选择。Scorer 接收已脱敏的 Candidate，返回五维 readiness 与 diagnosis，但不生成或改写 Idea。Evolver 依据明确的 Evolution Plan 创建 Mutation Child 或通过 Compatibility Check 的 Crossover Child；Parent artifact 保持不变。当单个 Plan 在不制造措辞型改写、不编造支持或不掩盖 Crossover 冲突的前提下无法形成实质 Child 时，Evolver 可以返回可审计的 `no_improvement`、`deferred` 或 Crossover `incompatible`，并给出复审条件。Controller 会保留 Parent 并持久化这一正常科研结果。成功接纳的 Child 才会触发 union rescoring，随后经过 Idea Contract、Complexity Inflation、Family-level Survival Selection 和 quality-diversity Population update。若没有 Child 被接纳，Controller 会保留真实的 Parent 独立评分并写入评分复用回执，不重复相同评分请求，也不合成替代分数。P0、每一轮 artifact、active 的 P1/P2/... snapshot 与 Archive 都可恢复并绑定 fingerprint。
...
Gate1 的操作都有明确后果并尽可能可逆。研究者可以选择一个 Candidate、继续一个 Generation、聚焦一个 Candidate 或 Idea Family、请求 Crossover、组合指定 Hypothesis/Contribution/Gene、并行保留完整 Candidate、查看 Score/Evidence/Lineage、重跑一条 Route、rollback 或 pause。自然语言先经可选 LLM interpreter 解析，再由确定性规则核对 Candidate ID、组件、Population fingerprint 和确认要求。一次提到多个完整 Candidate 默认是 `keep_parallel`，只有明确要求才会进入 composition。组件级请求会先生成 Compatibility Check 与 Gene Donor Map；随后必须第二次确认，才能生成 Human-composed Candidate、独立评分并重新比较。系统绝不会把用户选择的文本直接拼接进 hypothesis 文件。

研究者也可以在 Gate1 修改 Publication Orientation。确认后，ResearchOS 会保留旧 Population 与它的五维 Core Scientific Score，独立重评 Profile Fit，并写入新的 profile-revision Population snapshot 与更新后的 Portfolio。这个操作不会重跑文献检索、改变 Evidence Status、删除旧 snapshot 或改写 Candidate 内容；当研究问题不变但目标贡献叙事改变时适合使用。

选择一个完整 Candidate 后，T4 会生成 `hypothesis_brief.yaml`、selected lineage、T4.5 search targets 和可读的 Pre-Novelty brief。这些 artifact 固定已选 Candidate 并为定向 novelty/collision review 提供范围，但不是 T5 的执行授权。T4.5 必须先完成审计；只有明确通过的 T4.5 formalization 才能创建或更新 `hypotheses.md`、`exp_plan.yaml`、Contribution-Hypothesis Mapping、Validation Map、Kill Criteria 和供 T5 使用的 formalization manifest。legacy Gate1 文件仍作为兼容投影保留；旧 workspace 可迁移为清楚标注为 partial 的 P0，而无需重跑 T2/T3。

#### 正式算法与职责边界

T4 的正式算法是 **Evidence-Routed Multi-Route Idea Formation + Population-based Evolution + Independent Scoring + Human-guided Portfolio Selection**。它包含三个负责科研语义的 Agent 和一个确定性 Controller。`IdeaGeneratorAgent` 负责规划机会点并形成按 Route 划分的 Seed，不负责评分或选择；`IdeaScoringAgent` 独立评分匿名 Candidate，并审查 Crossover 或 Human-composition 的兼容性，不生成或改写 Candidate；`IdeaEvolverAgent` 只创建明确 Evolution Plan 授权的 Child，不选择 Parent 或 survivor。Opportunity Planning、Crossover Review、Pairwise Arbitration 和 Final Idea Card compilation 是这些 Agent 的 Prompt Mode 或 Controller 调度任务，不是额外的自治 Agent。Controller 负责 Evidence Routing、Population Management、Family construction、Parent Selection、Evolution budget、Crossover pair selection、Survival Selection、MMR Portfolio selection、Human directive application、fingerprint、resume 与 rollback。

Evidence substrate 包含 `literature/deep_read_notes/`、`literature/shallow_read_notes/`、`literature/bridge_notes/`、独立的 `literature/cross_domain_catalogs/`、`literature/synthesis_workbench.json`、contribution space、tensions、mechanism clusters、transfer drafts 与研究者提供的约束。Evidence Routing 从三类笔记与独立 catalog 中建立 Evidence Index，再为每条 Route 选择精简的 **Anchor**、**Expansion** 与 **Bridge** atom。Deep / partial reading 可以在已提供的范围内支撑 mechanism 或 design rationale；catalog-only、shallow / abstract material 仍可用于结构类比、coverage、taxonomy、trend、comparison、Bridge discovery 和 candidate discovery，但其 permission 不能被悄悄提升为已验证机制或 strong Claim。完整 Index 始终可被定向回查；初始 prompt 为控制体积做的压缩不会删除笔记，也不会使其不可用。

初始 Population `P0` 是非对称的。系统默认配置尝试形成三条 Literature Candidate、二到三条 Informed Brainstorm Candidate、Mechanism Challenge / Reverse Operation / Subgroup Failure / Gap Exploration 四类补充 Route 各一条，以及在存在已确认 Bridge 时一到两条 Cross-domain / Bridge Candidate。这些是 Route quota，不代表每个 Route 都必须编造一个 idea；证据不足的 Route 可以写 `unsupported` 并给出 evidence-upgrade 条件。Bridge 必须留下明确的 escape-hatch decision，而不是依据关键词相似生成迁移方案。Active Population 的数量大于展示 Portfolio：通常保留六到八个有效 Candidate，但最终只展示一到三个。

每个被提升为 Evolved Candidate 的 Seed 都具有 11 个科研 Gene 构成的 **Idea Genome**：Problem、Opportunity、challenged Assumption、Core Thesis、Mechanism、Design or Artifact、Contribution Package、Draft Hypothesis Bundle、Validation Logic、Boundary 与 Risk。每个 Gene 同时保存来源 Route、可追溯 SourceRef、阅读等级、证据角色、信心与升级要求。当前 schema 没有独立的 Evidence Map Gene，证据映射由逐 Gene provenance 和 Dossier 级 `evidence_composition` 共同承担。摘要、元数据、综合推断和模型知识只能形成启发或猜想，不能升格为机制锚点或直接支持。Controller 会把相关 Genome 组织为 **Idea Family** 与 sibling family，但不会把字面相似误判为科研等价。Family 记录共同的 mechanism structure、相关 Candidate ID 与需要保留的内容。`Gene Donor Map` 用 `donors: {gene: parent_id}` 明确记录通过 Crossover 审核时每个移植 Gene 的 Parent，并只将真正合成的字段写入 `synthesized_genes`。`Gene Delta` 说明 Child 相对 Parent 改了什么，`Complexity Inflation` 用于识别范围、依赖、contribution 或 validation burden 的无理由膨胀。

当 `rounds=1` 时，T4 必须完成完整生命周期，不能只停在 brainstorming：

1. 建立 Evidence Index 与 Opportunity Map，然后形成 multi-route `P0`。
2. 构造 genome 与 family，对匿名 Candidate 执行 **Independent Scoring**，评分维度为 Research Value、Mechanism Integrity、Contribution Distinctiveness、Evidence Calibration 与 Validation Tractability；`Profile Fit` 单独报告。
3. Controller 按质量、多样性、Family coverage、repairability 与 high-upside preservation 选择 Parent，生成二到四个 diagnosis-driven Mutation plan；只有 Compatibility Check 批准一个 coherent thesis 和 Gene Donor Map 时，才生成零到两个 Crossover plan。
4. 在能够形成实质改进时，`IdeaEvolverAgent` 根据 plan 创建 Offspring `O0`。Mutation 保留指定优势并修复已诊断 bottleneck；Crossover 必须形成一个 thesis 和一条 mechanism chain，不能堆叠模块或拼接 Parent hypothesis。单 Plan 的 `no_improvement` / `deferred`，或 Crossover `incompatible`，会连同复审条件被 checkpoint，保留 Parent，而不是制造措辞型 Child。
5. 若有 `O0` Child 被接纳，则对已改变的 `P0` 与 `O0` union 再次匿名评分；若没有 Child 被接纳，则保留已实际取得的 `P0` Parent 独立评分并写入明确的评分复用回执。无论哪种情况，随后都校验 Idea Contract、检查 Gene Delta 与 Complexity Inflation、保留 Family coverage、执行 Family-level Survival Selection，并写入 `P1` 和完整 Archive。
6. 在 `P1` 上执行 MMR Portfolio Selection，平衡当前 readiness、scientific upside、多样性与配置的 Profile Fit weight。只为选出的一个到三个 Candidate 编译可读的 Final Idea Card。

Local optimization 指通过某个 Candidate 自己的诊断结果和 Mutation plan 改进它，同时保留其他 Population 成员作为比较对象；Global optimization 指跨 Family、多样性、evidence strength 与项目约束选择 Portfolio 和下一代 Population。局部更强的 Child 不必然在全局 survivor 中胜出；高潜力但暂时不确定的 Family 可以在 revisit condition 清楚时保留在 Archive 或 Active Population。这个区分避免系统因一连串看似更顺的改写而过早压缩研究选择空间。

#### Publication Orientation 与 Final Idea Card

pre-run 的 Publication Orientation 是 `utd_is`、`ccf_cs`、`hybrid` 或 `custom`；为兼容已有 workspace，`management_is` 与 `technical_cs` 仍可读取。`utd_is` 强调 phenomenon、theoretical tension、explanatory mechanism、identification credibility、boundary conditions 与 organizational implications；`ccf_cs` 强调精确的 computational problem、algorithmic or system mechanism、technical contribution、evaluation discipline、efficiency、robustness 与 reproducibility。它影响 Opportunity / Generation 的强调方向、单独报告的 Profile Fit、MMR Portfolio weighting 与 Final Idea Card 的解释顺序；它绝不改变 Evidence Index、Evidence Permission、source citation、Candidate Genome、lineage、五维 Core Scientific Score 或验证结果。后续修改 profile 时，系统保留原始 Population 和 Core Scientific Score，写入带新 fingerprint 的 snapshot，只重算 profile-sensitive 的展示和选择字段。
...
每张成熟的 Final Idea Card 都是对不可变 Candidate 的 LLM-authored 解释。它包含一句话 core thesis、overall readiness、五项评分解释、research question、core innovation、mechanism chain、二到四条一句话 contribution、二到四条一句话 draft hypothesis、risk、Evidence Composition、系统推荐理由以及全部相关 artifact path。它可以按 profile 加入 why it matters、representative scenario、带 Evidence Status 与条件的 implication，以及不应作出的 Claim。compiler 必须保留完全一致的 core thesis、contribution ID、hypothesis ID、mechanism 和 Evidence Status；任何改变科学内容的 translation 都会被校验拒绝。卡片的格式或覆盖问题不会使 Population 失效。单次运行内 compiler 的调用仍受限，但会保存失败诊断；之后 `resume` 会携带该上下文，并可在 provider 或相关 source 发生变化后再次定向编译，不再设置跨 `resume` 的累计展示修复次数上限。

#### 研究者交互、恢复与 legacy 边界

运行结束后的 Gate 会用研究者能理解的语言说明每个动作：将执行什么、预计耗时类别、原版本是否保留、能否 rollback、适合什么情况。支持的动作包括：select、继续一轮 Evolution、focus、请求 Crossover、查看 score/evidence/lineage、重新生成 Route、保留 Candidate 并行推进、组合选定的 Gene/Contribution/Hypothesis、rollback 与 pause。自然语言 directive 先由语义解析器处理，再由确定性逻辑校验 Candidate ID、component reference、Population fingerprint 和 confirmation 条件。跨 Candidate 的 component 请求先产生 compatibility report；只有已批准的 Gene Donor Map 加第二次明确确认，才能产生 Human-composed Candidate。新 Candidate 会独立评分并重新进入 Population，禁止把用户选择的字符串直接拼接到 hypothesis 文件。

默认 runtime mode 是 `evolutionary`。兼容 prompt `ideation.j2` 不会在默认 T4 路径中被渲染或执行。管理员只能为恢复没有 native T4 Population、state、Portfolio 或 final-card artifact 的早期 workspace 显式设置 `agent_behavior.allow_legacy_t4_fallback: true`。runtime 会在 `ideation/evolution/execution_mode.json` 中记录 `evolutionary` 或 `legacy_fallback`、触发原因和 artifact-protection 规则。只要存在 native artifact，legacy fallback 就会被阻止，因此失败或 resume 的 native T4 不会静默回退并覆盖 Population。
...
正常 CLI 命令默认展示一次 `DIG · BUAA / ResearchOS` 启动页；`--no-banner` 和 `--quiet` 只服务脚本/CI，不应作为日常演示默认。阶段面板、工具结果、Gate、Skill 状态、trace 和 events 都链接到 workspace，但不会暴露模型内部推理。排障顺序应是：`status` 确认当前状态 -> `validate` 或 `audit-survey` 确认 artifact -> `trace`/日志查看上一次工具与错误 -> 修复对应输入 -> `resume`。最终是否成功只由当前 workspace 的可验证产物、输入指纹、编译报告和状态机决定，而不是控制台中的自然语言承诺。

Survey taxonomy 图是当前唯一允许自动生成的论文图。它采用 `Times New Roman` 优先字体链，依次 fallback 到 Times、Nimbus Roman、Liberation Serif、TeX Gyre Termes、STIX 和 DejaVu Serif；实际选用字体写入 `survey_visual_manifest.json`，因此同一环境下可复核。图中只展示 `survey_plan.json` 的显式 taxonomy 类、直接本地笔记链接和必要的来源注记，不展示跨论文性能、相对提升、T2 筛选分、风险热图或装饰元素。画布高度和坐标范围根据 taxonomy 行数收紧，避免少量方法类在下方留下大面积空白；字体、卡片留白、边框和 muted 多色标签服务于论文阅读，而非 dashboard 式展示。

在 ICML、NeurIPS、ICLR、KDD 等双栏模板中，`width=\textwidth` 的总览图必须使用 `figure*` 跨两栏；普通 `figure` 使用 `\textwidth` 会发生“PDF 可生成、右栏正文被图遮住”的静默版式错误。`audit-survey` 的 `survey_graphics_layout` 现在会在编译前拒绝这种写法，并在 audit 报告中给出 `figure*` 或 `\columnwidth` 的修复建议。展示前应始终运行 audit 和真实 compile，而不是只确认 PDF 文件存在。

### 11.4 文档的双语结构

`docs/cn/README.md` 是中文导航，`docs/en/README.md` 是英文导航，它们互相链接，并共同指向本文档、根 README 和各主题参考页。中英文配对文档保留语义等价的完整说明；需要提高准确性的 English technical term 会被保留。文档不应复制 workspace 运行产物、秘密、原始 trace 或 model prompt；这些仍属于具体项目的 workspace。

### 11.5 T4 结构化产物：字段级错误、完整候选与可恢复重试

T4 后半段的 `ideation/idea_rationales.json` 与 `ideation/idea_scorecard.yaml` 不是普通文本文件，必须通过 `write_structured_file` 写入。工具在写盘前调用 JSON Schema 校验；失败时不会覆盖上一次有效文件，也不会留下半个 YAML/JSON。过去终端只显示 `schema_validation_failed`，Agent 看不到是哪个候选、哪个字段出错，容易发生“同一阶段看起来反复重试”的假象。当前工具会把有限条字段诊断同时写到 ToolResult 和 CLI 事件中，格式包括实例路径、规则和可执行提示，例如：

```text
$.ideas[3].decision.rejection_reason [type]: 类型不匹配，需要 ['array']，当前为 str。
$.ideas[0].counterfactual_check [required]: 缺少必填字段。
$.ideas[0].basis.literature_observations[0].strength [enum]: `supporting` 不在 direct/indirect/weak 中。
```

`idea_scorecard` 的每个候选都必须保留 `idea`、`source`、`selection_rationale`、七维 `scores`、`decision`、风险、最小验证、`counterfactual_check`、`counterfactual_note`、`nearest_prior_work` 和 `novelty_signal`。这适用于 selected、rejected、deferred 和 merged 记录，不能为了让 schema 通过而删除 Pass1/Pass2 中已经展示给用户的候选。`selected_reason` / `rejection_reason` 都是字符串数组；哪怕只有一个原因也必须写列表。`minimum_experiment.evidence_status=supported|user_provided` 时必须有可追溯 `source_refs`；否则诚实写 `proposed_not_verified` 或 `unknown`。这些是结构完整性与证据状态约束，不是让工具替代 LLM 判断创新性的模板。
...
### 11.7 T3.6-REVIEW：section 是源，`survey.tex` 是派生产物

`T3.6-REVIEW` 可以发现标题、taxonomy 结构、比较公允性、语言一致性、coverage 或 prose craft 的问题，但它不能用普通 `write_file` 直接覆盖 `drafts/survey/survey.tex`。整篇 TeX 是 sections、state、template 与 bibliography 的拼装结果；若模型只读了前几千字符，直接整篇写回会把未读 section 静默截断。Review 必须改 `drafts/survey/sections/<id>.tex`，需要标题/模板修正时用 `assemble_survey(title="...")`，再运行 audit；修订和原因写入 `survey_review.md` / `survey_review_actions.json`，最后由 `bind_survey_review` 写入当前输入 fingerprint。

一旦 review 重新拼装，旧 `survey.pdf` 与 `survey_compile_report.json` 会按设计变成 stale。T3.6-COMPILE validator 可能报告 `pdf_mtime 早于当前 survey.tex`、TeX hash 或依赖 fingerprint 不一致；这不是 T3.6-ASSEMBLE 的审计循环，而是要求调用真实 `latex_compile` 更新 PDF、log、hash 和 report 的单次编译步骤。禁止手工修改 compile report；修复 TeX/环境后 compile，再 `validate --task T3.6-COMPILE`，通过后 resume 进入 T3.6-FEED。
