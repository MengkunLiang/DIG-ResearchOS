...
T1 项目初始化
 ...
 -> T5-SKILL-CUSTOMIZATION-GATE 审阅已发布 Skill Suite
...
当前主链已经废弃“ResearchOS 自己在 T5-T7 内部实现并长时间跑实验”的默认语义。ResearchOS 负责研究协议、执行器选择、handoff prompt、文件契约、结果摄取、诚信审计、实验后 novelty 复核、result-to-claim 和写作证据闭环；Codex CLI、Claude Code 窗口、人工外部执行器或 mock dry-run 负责在隔离路径中实现和运行实验。常规入口是 `T5-REBOOST-GATE`：它通过已配置模型运行 `research-reboost`，校验 handoff，并在执行器选择前发布项目专属 Skill Suite。`T5-HANDOFF` 保留为 legacy-compatible 协议编译入口。旧 `T5` Pilot、`T6` Pilot 后 novelty 复核和旧 `T7` 内部完整实验仍保留为 `LEGACY-T5-PILOT` / `LEGACY-T6-NOVELTY` / `LEGACY-T7-FULL` 兼容入口；普通 `run-task T5/T6/T7` 会报 retired 并提示新版入口，旧 workspace 的 `next_task: T7` 恢复语义会安全映射到 `T5-REBOOST-GATE`。
...
- 会完整体现 `T5-REBOOST-GATE -> T5-SKILL-CUSTOMIZATION-GATE -> T5-EXECUTOR-GATE -> T5-DRY-RUN/T5-EXTERNAL-WAIT -> T7-INGEST -> T7-AUDIT -> T7-POST-NOVELTY -> T7-CLAIMS -> T7.5 -> ask_human -> T8` 这样的链条
...
| `T5-REBOOST-GATE` | `SkillAgent(research-reboost)` | `reboost` | 用已配置模型编译受来源约束的实验 handoff、校验并发布项目专属 Skill Suite；不运行真实实验 | `external_executor/handoff_pack.json`、`reboost_report.json`、AGENTS/CLAUDE、prompt、schema、`project_skill_context.yaml`、`skill_specialization_report.json`、`skills/` |
| `T5-SKILL-CUSTOMIZATION-GATE` | `ExperimenterAgent` | `skill_customization` | 校验已发布的 context、report 和 13 个项目专属 Skill；`incomplete` 记录未决 context，不会编造内容 | `external_executor/skill_specialization_report.json`、`project_skill_context.yaml`、`skills/` |
...
| T4 原生多路径生成，并兼容 Pass 1/Pass 2 | 默认 Controller 依次执行 Evidence Routing、Opportunity Map、多路径 P0 formation、独立评分、O0 offspring、union re-scoring 与 P1/Portfolio selection，并把结果投影为既有状态机继续使用的 Pass 1/Pass 2 Gate1 artifacts。`researchos/prompts/ideation.j2` 只保留为 legacy fallback，绝不是默认 T4 路径。 | 已落地 |
...
8. **T4 上桌和逃生舱**：有 confirmed bridge 且 `source!=none` 时，T4 必须检查 `bridge_notes/{bridge_id}/`、主线 note 的 `contributed_bridges`、`synthesis_workbench.bridge_transfer_drafts`。有足够证据时生成 `idea_origin=bridge_synthesis`、`constraint_status=bridge`、`cross_domain_sources=[...]` 的候选，并放入 Gate1 可见池；没有足够证据时不能硬编 idea，必须在 `ideation/bridge_coverage_review.json` 写 `escape_hatch.status=no_candidate_available`、证据缺口和回访条件。
...
4. **生成候选方向**：
   ...
   - `cross_domain_sources` / `cross_domain_relation`（如果使用 `bridge_transfer_drafts`、`domain_map.theory_bridge`、`bridge_notes/` 或 bridge-domain plan 作为类比素材，必须记录对应 `bridge_id` 数组和关系类型；未使用时为空数组；旧 `cross_domain_source` 单数只作兼容 alias）
...
1. **当前行为**：把已选 Candidate 编译为带 2-4 条 Draft Hypotheses 的 Pre-Novelty brief，并生成 T4.5 search targets。
...
- Pre-Novelty brief 必须有 2-4 条非模板化 Draft Hypotheses 和合法的 Evidence Permission。正式 `hypotheses.md` 只会在 T4.5 audit 通过后生成，并包含 `H1/H2/...` anchor。
...
- `source`：来源追溯
  ...
  - `constraint_status`：`mainline` / `supplement` / `bridge` / `not_supported_by_current_evidence`
  - `cross_domain_sources`：如果 idea 使用了 `bridge_transfer_drafts`、`domain_map.theory_bridge`、`bridge_notes/` 或 bridge-domain plan，填对应 `bridge_id` 数组；否则为空数组
...
- Standard mode 必须完成完整的 P0 -> P1：独立评分、合法 Evolution Plan、union rescoring、Contract validation、Survival Selection 与 Population update 都不能跳过。
- Portfolio 展示 1-3 个 Candidate，同时配置的 Active Population 与 Archive 必须可恢复。
- 每个 Gate1 Candidate 都要有非模板化的 mechanism/prediction/counterfactual/family 和 2-4 条 Draft Hypotheses；正式 hypothesis 与 experiment plan 必须等待 T4.5 audit 通过。
...
- `pass_to_experiment` / `pass_with_required_baselines`：进入 `T5-REBOOST-GATE`，由 `research-reboost` 编译并校验实验 handoff、发布 Skill Suite，随后再选择外部执行器；mock dry-run 路径可自动到 `T7-INGEST`，真实 Codex/Claude/manual 路径会先经过 `T5-EXTERNAL-WAIT`。
...
- `continue_to_t7`：接受风险，继续进入外部实验链入口 `T5-REBOOST-GATE`。
...
| `external_executor/reboost_report.json` / `reboost_validation_report.json` | source 使用回执、handoff 校验结果和确定性结构修复记录 |
| `external_executor/project_skill_context.yaml` / `skill_specialization_report.json` / `skills/` | schema 校验过的项目 context、专属化报告和已发布的 13-Skill executor Suite |

### 实际执行过程

`research-reboost` Skill 会在 system prompt 中得到 bundled contract 与 references，因此只读取 workspace artifacts。它依据 `project.yaml`、post-novelty formalization bundle、synthesis/comparison artifacts 和三类 Paper Note 根目录准备语义 handoff candidate。编译器会落盘 candidate、进行校验、生成控制文件，并原子发布项目专属 Suite。发布后的 Suite 状态可以是 `ready` 或 `incomplete`；`incomplete` 表示部分字段仍受证据边界约束，后续由对应的 runtime Skill 解决，不能用通用知识填充。

此时执行模式仍为 `UNSET`；`T5-SKILL-CUSTOMIZATION-GATE` 校验 context 和全部 13 个 Skill 后，`T5-EXECUTOR-GATE` 才选择执行器，runtime 随后确定性 patch 面向执行器的文件。validator 会检查 handoff schema、required control files、allowed paths、project context、marker/template integrity 和完整的 Skill 数量；它会拒绝未发布的 Suite、错误的结果契约，以及把自然语言总结伪装成实验事实的内容。`T5-HANDOFF` 继续作为旧 workspace 或显式恢复路径的 legacy-compatible 协议编译器，但不是 T4.5 的默认目的地。

### 恢复逻辑

如果 reboost outputs 与其 fingerprint 仍有效，`resume` 会保留它们，而不会重建 handoff 或 Suite。formalization 变更后需要重新整理研究意图时，应从 `T5-REBOOST-GATE` 重新进入；在新的 Suite 成功发布前，原有 Suite 仍保持可审计。用户要切换真实执行器时，应回到 `T5-EXECUTOR-GATE` 重新选择，而不是在后续 ingest 阶段改写结果来源。

### T5-SKILL-CUSTOMIZATION-GATE

这个短审阅节点不会再次改写 13 个 Skills。它校验 `project_skill_context.yaml`、`skill_specialization_report.json` 与已发布的 `external_executor/skills/` 树。`ready` 表示 required context 完整；`incomplete` 会保留显式未决字段，等待后续 context alignment。报告为 `failed` 或文件缺失时，流水线会在选择执行器前停止。可选的 `researchos specialize-executor-skills` 命令可以为已发布的 Suite 添加 LLM guidance，但绝不是本 Gate 的隐藏前置条件。
...
- `researchos run-task T5 --workspace ...`：报 retired，提示改用 `T5-REBOOST-GATE`。
- `researchos run-task T6 --workspace ...`：报 retired，提示改用新版 `T7-POST-NOVELTY`。
- `researchos run-task T7 --workspace ...`：报 retired，提示改用 `T5-REBOOST-GATE` 外部实验链。
...
完整 `run/resume` 默认不会从 T4.5 进入这些 legacy 节点。T4.5 pass 路由到 `T5-REBOOST-GATE`；T7.5 的旧推荐 `next_task: T7` 会被状态机安全映射到 `T5-REBOOST-GATE`。`T5-HANDOFF` 保持为显式兼容节点，而不是静默 fallback。这样可以保留旧功能，又不会让手动调试或主链误回“ResearchOS 自己长时间跑实验”的旧设计。
...
- `T5-REBOOST-GATE` 到 `T7-CLAIMS` 负责把外部实验协议和证据链跑通
...
1. 调用 `build_manuscript_resource_index`，扫描 literature、paper notes、bib、hypotheses、idea scorecard、novelty audit、results summary、ablations、runs、configs、figures、tables 和 code artifacts，写 `drafts/manuscript_resource_index.json`。该索引会同时暴露 `citation_ref_by_note_id`、`note_id_by_bib_key` 和 `unmapped_note_ids`：T3.5 的 `[note:<id>]` 是 Markdown provenance，T8 TeX 正文必须用这里的 `\cite{bibkey}` 映射；无法映射到 BibTeX key 的 note 只能作为 provenance/补资源提示，不能原样进入 TeX 引用。
...
- 新运行统一使用 `config/model_settings.yaml` 的 provider/model，不暴露普通的 step/token/wall budget。step/token/cost 仍会记录；provider timeout/retry、Tool timeout、Docker/TeX 专用 timeout、workspace 权限、输出校验和项目级实验预算检查仍然生效。`config/system_config/agent_params.yaml` 是工具、权限、prompt 与 behavior capability 的系统声明，不是研究者日常配置入口。
...
在不改变公开状态路径 `T4 -> T4-GATE1 -> T4 -> T4.5` 的前提下，T4 是一个 artifact-first 的 Evolutionary Idea Formation 工作流。第一次进入 T4 时会显示 Rich pre-run confirmation：说明已有全文/部分全文与摘要层论文阅读笔记、非阻塞的 Bridge warning、当前 mode、预计工作量以及已有 Population snapshot 是否可 rollback。Standard mode 完成一次完整的 `P0 -> P1`：Evidence Routing -> Opportunity Map -> 非对称 Multi-route Generation -> Idea Genome 与 Idea Family -> Independent Scoring -> Parent Selection -> Mutation/Crossover plan -> Offspring 与 union rescoring -> Idea Contract -> Family-level Survival Selection -> Population Update -> Portfolio Selection。它不是一次 Prompt 改写，也不会增加公开 state machine node。

正式运行前，同一确认界面还会询问 Publication Orientation。ResearchOS 会复用 `project.yaml`、已有 writing style 和可配置的 venue profile，建议 `UTD / Management & IS`、`CCF A / Technical` 或 `Hybrid / Cross-disciplinary`；选择运行 mode 后直接回车即可接受建议，也可以用一句自然语言覆盖。确认后的 profile 写入 `ideation/t4_target_profile.json` 和 `ideation/t4_run_config.json`，因此会参与 T4 configuration fingerprint 和 resume。它只改变 Opportunity/Generation/Scoring 的侧重点以及 final card 的叙事顺序，绝不改变来源事实、Evidence Permission、citation 或 Candidate lineage。
...
Generator、Scorer 和 Evolver 的权限彼此分离。Generator 只产出按 Route 划分的 Seed 和 Candidate 文案，不评分也不选择。Scorer 接收已脱敏的 Candidate，返回五维 readiness 与 diagnosis，但不生成或改写 Idea。Evolver 只能依据明确的 Evolution Plan 创建 Mutation Child 或通过 Compatibility Check 的 Crossover Child；Parent artifact 保持不变。所有 Parent 和 Child 都会先 union rescoring，再经过 Idea Contract、Complexity Inflation、Family-level Survival Selection 和 quality-diversity Population update。P0、每一轮 artifact、active 的 P1/P2/... snapshot 与 Archive 都可恢复并绑定 fingerprint。

五维 Core Scientific Score 在所有项目中保持一致：Research Value、Mechanism Integrity、Contribution Distinctiveness、Evidence Calibration 与 Validation Tractability。Scorer 还会针对已确认的取向单独返回 `Profile Fit`。Portfolio 和 Parent selection 可以按配置把它作为次级输入，但它不能替代 Core Scientific Score，更不能升级弱证据。T4 Prompt 由共享 Scientific Constitution、role contract、task mode、紧凑的 Target Profile 摘要、runtime evidence context、output contract 和 failure protocol 组合而成。workspace 文本一律作为数据处理，不能成为指令。

Gate1 先用 Rich 比较 1–3 个 Candidate 的 Portfolio，同时保留 6–8 个 Active Candidates 和完整 Archive 供按需查看。Lead card 与 compact alternative 使用由 LLM 根据 Workspace 生成的科研文本；renderer 只负责排版已验证字段。成熟 Candidate 包含一句话核心、Overall Readiness 与五维解释、Problem、Core Innovation、Mechanism Chain、2–4 项 Contribution、2–4 条 Draft Hypotheses、validation snapshot、Evidence Composition、风险、推荐理由、谱系，以及说明每个文件用途的路径。`proposed_not_verified` 表示假设是清晰的待验证提案，不是已经证实的证据。假设少于两条或多于四条、机制链不完整、Evidence Permission 无效，或科研字段由模板填充的 Candidate 不能进入 Gate1。
...
4. `IdeaEvolverAgent` 根据 plan 创建 Offspring `O0`。Mutation 保留指定优势并修复已诊断 bottleneck；Crossover 必须形成一个 thesis 和一条 mechanism chain，不能堆叠模块或拼接 Parent hypothesis。
5. 对 `P0` 与 `O0` 的 union 再次匿名评分。随后校验 Idea Contract、检查 Gene Delta 与 Complexity Inflation、保留 Family coverage、执行 Family-level Survival Selection，并写入 `P1` 和完整 Archive。
6. 在 `P1` 上执行 MMR Portfolio Selection，平衡 readiness、多样性与配置的 Profile Fit weight。只为选出的一个到三个 Candidate 编译可读的 Final Idea Card。
