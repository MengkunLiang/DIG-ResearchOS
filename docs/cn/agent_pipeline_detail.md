...
在材料补充 Gate 之前，新建 T1 workspace 会先让研究者选择 **Copilot** 或 **Auto**。Copilot 保留所有普通科研 Gate 供研究者确认，并且不持久化任何投稿取向；它不等于默认 CCF/CS。进入 T4 前会单独询问 CCF/CS、UTD/IS、Hybrid 或自定义取向。无论交互式选择 Copilot 还是 Auto，T1 随后都会以 Rich 面板确认一次默认执行设置：T2/T3 文献覆盖、T4 探索力度和独立 Proposal 数量。三种覆盖档分别是标准研究 `40/25/15`、综述均衡 `80/40/40`、综述强覆盖 `90/40/50`，三个数字依次表示保留候选、精读和摘要级轻读。T2 会明确说明推荐来自项目默认设置还是项目材料推断，T4 会将保存的探索力度标成回车默认，并在 Candidate 选择页提示一份或两份独立 Proposal 的后续策略。Copilot 中这些只是后续 Gate 的建议，绝不会令人工决策自动通过；Auto 预设才会决定写作取向与综述策略，`quick`、`standard`、`deep` 或质量感知的 `auto` 决定 T4 力度。模式/参数既可用紧凑菜单，也可用自然语言：轻量 LLM 只把自由表达翻译为有限枚举提案，本地代码随后校验枚举/范围，最终仍须由研究者确认后才保存。可直接输入 `Auto survey_ccf deep`、`Auto survey_utd standard`、`协作模式，综述均衡覆盖，深入探索` 或 `standard_research deep top2`。T1 仍会完成种子、研究边界、项目草案和检索范围确认，不会被跳过。Auto 只预授权后续常规决策，不会替用户决定研究问题或范围、恢复操作、失败的新颖性 verdict、外部副作用，或发生实质变化的研究范围。也可以在命令行预设，例如 `--workflow-mode auto --auto-preset research_utd --auto-t4-mode deep`；任意阶段都可运行 `python -m researchos.cli configure-workflow --workspace <workspace>` 查看或调整未来默认值。该命令只更新 `_runtime/workflow_mode.json`，不会删除或静默重跑既有文献库、Population、Proposal 或论文。历史版本静默写入的默认值会被视为未确认，并在 T1 首次进入时补问一次。
