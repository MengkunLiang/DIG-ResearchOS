...
- T1 会在 Auto + CCF/CS 默认设置确认后立即选择具体 CCF 模板；T3.6 沿两级 Gate（CCF/CS 方向与 `T3.6-CCF-TEMPLATE-GATE`）复用该选择，以保留正常 receipt 而不重复弹菜单。旧 workspace 或未确认会议的 CCF profile 仍在此选择，绝不回退为 `basic_en`；UTD 明确持久化并复用 `utd/informs`。T3.6 选择只作为 T4 的透明建议，T4 仍单独确认 publication orientation。
