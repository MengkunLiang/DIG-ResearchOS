...
| 失败类型 | 含义 | 修复范围 |
| --- | --- | --- |
| `citation_diversity` | 报告范围内可追溯材料、正文实际使用、未使用候选和引用集中度，不设置固定覆盖百分比。失败只表示单一来源确实过度集中，不表示“引用库中论文数量不足”。 | 先合并重复论断。只有存在语义匹配的替代来源时才查看 `section_review_queue` 并打开其 `source_file` 核验。FULL/PARTIAL 笔记可支持经核验的具体论断；ABSTRACT-ONLY 仅用于背景、趋势、范围或边界。不得堆砌引用。 |
...
Review 只以三份固定输入开始：`ideation/hypotheses.md`、`ideation/proposal/research_proposal.md` 与 `ideation/orientation_config.yaml`。第一轮 checkpoint 会在任何改写前检查研究包。因此，checkpoint 已通过后出现超时，只表示模型服务没有在时限内返回，**不**表示 Proposal、研究蓝图或实验计划未通过校验。

只有确实需要诊断时才使用 `trace <run-id>`。若 trace 显示模型读取了 `proposal.md` 等裸文件名，或把 `ideation/` 这样的目录交给 `read_file`，请更新 ResearchOS 后直接恢复，而不要手工改写产物。运行时现在只会将一小组已知的历史 Proposal 别名转到规范路径，并在 trace 中记录 `canonicalized_from`；目录和新编造的文件名绝不会被当作有效来源。这样可避免一串无害的 404 在真正审阅前耗尽模型回合。
